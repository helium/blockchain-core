...
election_info(Block) ->
    {Block#blockchain_block_v1_pb.election_epoch, Block#blockchain_block_v1_pb.epoch_start}.
...
verify_signatures(#blockchain_block_v1_pb{} = Block, ConsensusMembers, [], _Threshold, Key) when
    % force the other path for old tests :/
    ConsensusMembers /= []
->
    EncodedBlock =
        case is_list(Key) of
...
verify_normal_signatures(Artifact, ConsensusMembers, Signatures, Threshold) ->
    ValidSignatures0 =
        blockchain_utils:pmap(
            fun({Addr, Sig}) ->
                case
                    lists:member(Addr, ConsensusMembers) andalso
                        libp2p_crypto:verify(Artifact, Sig, libp2p_crypto:bin_to_pubkey(Addr))
                of
                    true -> {Addr, Sig};
                    false -> error
                end
            end,
            lists:sublist(blockchain_utils:shuffle(Signatures), Threshold)
        ),
    ValidSignatures =
        case
            lists:any(
    ...
    case
        length(Signatures) =< (3 * F) + 1 andalso
            ...
            length(ValidSignatures) >= Threshold
    of
        true ->
...
verified_signees(Block) ->
    ...
    lists:foldl(
        fun({Signer, Signature}, Acc) ->
            case
                libp2p_crypto:verify(EncodedBlock, Signature, libp2p_crypto:bin_to_pubkey(Signer))
            of
                false -> Acc;
                true -> [Signer | Acc]
            end
...
new_genesis_test() ->
    ...
    Txs = [
        blockchain_txn_gen_gateway_v1:new(<<"gateway1">>, <<"owner">>, Loc, 1),
