...
new_group_v1(Ledger, Hash, Size, Delay) ->
    ...
    lager:debug(
        "scored old group: ~p scored gateways: ~p",
...
condense_votes(Sz, Seen0) ->
    SeenMaps = [
        blockchain_utils:bitvector_to_map(Sz, S)
     || {_Idx, S} <- Seen0
    ],
    %% 2f + 1 = N - ((N - 1) div 3)
...
determine_sizes_v2_math(
    ...
    case Size == OldLen of
        true ->
            ...
            % smallest remainder we will allow
            MinSize = (2 * ((OldLen - 1) div 3)),
            % initial remove size
            BaseRemove = ceil(Size / ReplacementFactor),
            Removable = OldLen - MinSize - BaseRemove,
            %% use tanh to get a gradually increasing (but still clamped to 1) value for
            %% scaling the size of removal as delay increases
            %% vihu argues for the logistic function here, for better
        ...
        fun({Addr, BinGw}, Acc) ->
            ...
            case
                blockchain_ledger_gateway_v2:is_valid_capability(
                    Mode, ?GW_CAPABILITY_CONSENSUS_GROUP, Ledger
                )
            of
                true ->
...
has_new_group(Txns) ->
    ...
    of
        [Txn] ->
            ...
            {true, lists:member(MyAddress, blockchain_txn_consensus_group_v1:members(Txn)), Txn,
                {Height, Delay}};
        [_ | _] ->
            lists:foreach(
                fun(T) ->
...
election_info(Height, Ledger) when is_integer(Height) ->
    ...
    {ok, #block_info_v2{election_info = {Epoch, StartHeight0}}} = blockchain_ledger_v1:get_block_info(
        Height, Ledger
    ),
...
val_dedup(OldGroup0, Validators0, Ledger) ->
    ...
    {Group, OfflineGroup, Pool} =
        maps:fold(
            fun(
                Addr,
                Val = #val_v1{heartbeat = Last, prob = Prob},
                        ...
                        case Offline of
                            ...
                            _ ->
                                case PenaltyFilter - Prob of
                                    ...
                                    NewProb -> {Old, Off, [Val#val_v1{prob = NewProb} | Candidates]}
                                end
                        end
                end
            end,
    ...
    case length(Group ++ OfflineGroup) < length(OldGroup0) of
        ...
        true ->
            ...
            {Group, OfflineGroup ++ [#val_v1{addr = MAddr, prob = 1.0} || MAddr <- Missing], Pool}
    end.
...
val_dedup_v2(OldGroup0, Validators0, Ledger) ->
    ...
    {Group, OfflineGroup, Pool} =
        lists:foldl(
            fun(
                {Addr, Val = #val_v1{heartbeat = Last, prob = Prob}},
                {Old, Off, Candidates} = Acc
            ) ->
                Offline = (Height - Last) > (HBInterval + HBGrace),
                case lists:member(Addr, OldGroup0) of
