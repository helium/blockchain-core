    ...
    case ?MODULE:head_hash(Blockchain) of
        ...
        {ok, Hash} ->
            case get_block(Hash, Blockchain) of
                ...
                {ok, Block} -> {ok, blockchain_block:height(Block)}
            end
    end.
    ...
    case blockchain_ledger_v1:current_height(Ledger) of
        ...
        {ok, CurrentHeight} ->
            ...
            case blockchain_ledger_v1:current_height(DelayedLedger) of
                ...
                {ok, DelayedHeight} when Height > DelayedHeight andalso
...
fold_blocks(Chain0, DelayedHeight, DelayedLedger, Height) ->
    ...
    {HighestSnapHeight, HighestLedger} = lists:foldl(
        fun
                    ...
                    {ok, Block} ->
                        case blockchain_txn:absorb_block(Block, ChainAcc) of
                            {ok, Chain1} ->
                                ...
                                ok = blockchain_ledger_v1:refresh_gateway_witnesses(
                                    Hash,
                                    Ledger0
                                ),
                                ok = blockchain_ledger_v1:maybe_recalc_price(
                                    Chain1,
                                    Ledger0
                                ),
...
has_block(
    Block,
    #blockchain{db = DB, blocks = BlocksCF, plausible_blocks = PlausibleBlocks}
) ->
    Hash = blockchain_block:hash_block(Block),
    case rocksdb:get(DB, BlocksCF, Hash, []) of
        {ok, _} ->
...
%% @end
%%--------------------------------------------------------------------
-spec add_block(blockchain_block:block(), blockchain()) ->
...
add_block(
    Block,
    #blockchain{db = DB, blocks = BlocksCF, heights = HeightsCF, default = DefaultCF} =
        Blockchain,
    Syncing
) ->
    blockchain_lock:acquire(),
                        ...
                        case blockchain_block:prev_hash(NextBlock) of
                            Hash ->
                                ...
                                ok = rocksdb:batch_put(
                                    ...
                                    BlocksCF,
                                    Hash,
                                    blockchain_block:serialize(Block)
                                ),
...
can_add_block(Block, Blockchain) ->
    Hash = blockchain_block:hash_block(
        Block
    ),
    {ok, GenesisHash} =
    ...
    case blockchain_block:is_genesis(
             Block
         ) of
        true when Hash =:=
                      GenesisHash ->
        ...
        true ->
            {error,
                unknown_genesis_block};
        false ->
            case blockchain:head_block(
                     Blockchain
                 ) of
                {error, Reason} = Error ->
                ...
                {ok, HeadBlock} ->
                    ...
                    case blockchain_block:prev_hash(Block) =:= HeadHash andalso
                             Height =:= blockchain_block:height(HeadBlock) + 1 of
                            ...
                            case ?MODULE:get_block(Hash, Blockchain) of
                                ...
                                _ ->
                                    ...
                                    case Height > (ChainHeight + 1) of
                                        true ->
                                            lager:warning(
                                                "block doesn't fit with our chain,\n"
                                                "                                                block_height: ~p, head_block_height: ~p",
                                                [blockchain_block:height(Block), blockchain_block:height(HeadBlock)]
                                            ),
                                            case is_block_plausible(
                                                     Block,
                                                     Blockchain
                                                 ) of
                                                true -> plausible;
                        ...
                        true ->
                            ...
                            case blockchain_ledger_v1:consensus_members(
                                     Ledger
                                 ) of
                                {error, _Reason} = Error ->
                                ...
                                {ok, ConsensusAddrs} ->
                                    ...
                                    case blockchain_block:verify_signatures(
                                             Block,
                                             ConsensusAddrs,
                                             Sigs,
                                             N - F,
...
process_snapshot(ConsensusHash, MyAddress, Signers, Ledger, Height, Blockchain) ->
    case lists:member(MyAddress, Signers) of
        true ->
    ...
    lists:foldl(
        fun
            (H, ok) ->
                ...
                case blockchain_txn:absorb_and_commit(
                         B,
                         Chain,
                         fun () -> ok end,
...
absorb_temp_blocks_fun(
    Block,
    Blockchain = #blockchain{temp_blocks = TempBlocksCF},
    Syncing
) ->
    ...
    case blockchain_block:prev_hash(OldestTempBlock) of
        ...
        Res ->
            ...
            lager:warning(
                "Saw assumed valid block, but cannot connect it to the main chain ~p",
                [Res]
            ),
...
absorb_temp_blocks_fun_([BlockHash | Chain], Blockchain, Syncing) ->
    ...
    case blockchain_txn:unvalidated_absorb_and_commit(
             Block,
             Blockchain,
             BeforeCommit,
...
delete_block(
    Block,
    #blockchain{db = DB, default = DefaultCF, blocks = BlocksCF, heights = HeightsCF} =
        Chain
) ->
    {ok, Batch} = rocksdb:batch(),
    Hash = blockchain_block:hash_block(Block),
    ...
    case HeadHash =:= Hash of
        false -> ok;
        true -> ok = rocksdb:batch_put(Batch, DefaultCF, ?HEAD, PrevHash)
    end,
...
%% @end
%%--------------------------------------------------------------------
-spec fees_since(non_neg_integer(), blockchain()) ->
...
%% @end
%%--------------------------------------------------------------------
-spec build(blockchain_block:block(), blockchain(), non_neg_integer()) ->
            ...
            case rocksdb:get(DB, CF, ParentHash, []) of
                {ok, BinBlock} ->
                    build_hash_chain_(StopHash, CF, Blockchain, [
                        blockchain_block:prev_hash(blockchain_block:deserialize(BinBlock))
                        | Acc
                    ]);
                _ ->
                    Acc
...
reset_ledger(
    ...
    case length(HashChain) == Height - 1 andalso
             ...
             GenesisHash ==
                 blockchain_block:prev_hash(LastKnownBlock) of
        false ->
            ...
            case head_hash(Chain) == {ok, blockchain_block:hash_block(TargetBlock)} of
                false ->
                    ...
                    ok = rocksdb:put(
                        ...
                        DefaultCF,
                        ?HEAD,
                        blockchain_block:hash_block(TargetBlock),
                        [{sync, true}]
                    );
                true ->
                    %% nothing to do
...
check_recent_blocks(Blockchain) ->
    ...
    {ok, LedgerHeight} = blockchain_ledger_v1:current_height(Ledger),
    {ok, DelayedLedgerHeight} =
        blockchain_ledger_v1:current_height(blockchain_ledger_v1:mode(delayed, Ledger)),
                    ...
                    case get_block(hd(Hashes), Blockchain) of
                        {ok, FirstBlock} ->
                            case blockchain_block:prev_hash(FirstBlock) == DelayedHeadHash of
                                ...
                                false ->
                                    case length(Hashes) ==
                                             (LedgerHeight - DelayedLedgerHeight) of
                                        true ->
                                            %% this is real bad
                                            {error,
                        ...
                        _ ->
                            case get_block(hd(tl(Hashes)), Blockchain) of
                                {ok, Block} ->
                                    {error,
                                        {missing_block, blockchain_block:height(Block) - 1}};
                                _ ->
                                    %% what the hell is happening
                                    {error, space_wolves}
                            end
                ...
                ok ->
                    ...
                    case LedgerHeight - DelayedLedgerHeight of
                        Lag when Lag > BlockDelay ->
                            {error,
                                {ledger_delayed_ledger_lag_too_large, Lag}};
                        Lag ->
                            ...
                            case ledger_at(
                                ...
                                {ok, RecalcLedger} ->
                                    ...
                                    {ok, RecalcFP} =
                                        blockchain_ledger_v1:raw_fingerprint(RecalcLedger, true),
                                    case FP == RecalcFP of
                                        false ->
                                            Mismatches =
                                                [
                                                    K
                                                    || K <- maps:keys(FP),
                                                       maps:get(K, FP) /= maps:get(K, RecalcFP),
                                                       K /= <<"ledger_fingerprint">>
                                                ],
                                            MismatchesWithChanges = lists:map(
                                                fun (M) ->
...
repair(#blockchain{db = DB, default = DefaultCF} = Blockchain) ->
    case crosscheck(Blockchain) of
        ...
        {error, {missing_block, Height}} ->
            ...
            case {get_block(Height - 1, Blockchain),
                     find_upper_height(
                         Height + 1,
                         Blockchain,
                         MaxHeight
                ...
                {{ok, Block}, TopHeight} when TopHeight == Height + 1 ->
                    ...
                    case application:get_env(blockchain, enable_missing_block_refetch, false) of
                        true ->
                            ...
                            ok = rocksdb:put(DB, DefaultCF, ?MISSING_BLOCK, blockchain_block:hash_block(Block), [
                                {sync, true}
                            ]),
        ...
        {error, {chain_ledger_height_mismatch, ChainHeight, LedgerHeight}}
                when ChainHeight > LedgerHeight ->
...
add_snapshot(Snapshot, #blockchain{db = DB, snapshots = SnapshotsCF}) ->
    try
        ...
        %% lexiographic ordering works better with big endian
        ok =
            rocksdb:batch_put(Batch, SnapshotsCF, <<Height:64/integer-unsigned-big>>, Hash),
...
add_gateway_txn(OwnerB58, PayerB58, Fee, StakingFee) ->
    ...
    Txn = blockchain_txn_add_gateway_v1:staking_fee(
        blockchain_txn_add_gateway_v1:fee(Txn0, Fee),
        StakingFee
    ),
...
assert_loc_txn(H3String, OwnerB58, PayerB58, Nonce, StakingFee, Fee) ->
    ...
    Txn = blockchain_txn_assert_location_v1:staking_fee(
        blockchain_txn_assert_location_v1:fee(Txn0, Fee),
        StakingFee
    ),
...
open_db(Dir) ->
    ...
    DefaultCFs =
        ["default", "blocks", "heights", "temp_blocks", "plausible_blocks", "snapshots"],
    ExistingCFs =
...
save_temp_block(
    Block,
    #blockchain{db = DB, temp_blocks = TempBlocks, default = DefaultCF} = Chain
) ->
    Hash = blockchain_block:hash_block(Block),
...
init_assumed_valid(
    Blockchain,
    HashAndHeight = {Hash, Height}
) when is_binary(Hash),
       is_integer(Height) ->
    case ?MODULE:get_block(
             Hash,
             Blockchain
         ) of
        {ok, _} ->
                ...
                false ->
                    ...
                    case rocksdb:get(
                        ...
                        {ok, BinBlock} when IsInSync == true ->
                            ...
                            Block = blockchain_block:deserialize(
                                BinBlock
                            ),
                            case blockchain_block:height(Block) ==
                                     Height of
                                true ->
                                    absorb_temp_blocks(
                                        Block,
...
init_blessed_snapshot(Blockchain, _HashAndHeight = {Hash, Height})
        when is_binary(Hash), is_integer(Height) ->
...
maybe_continue_resync(Blockchain, Blocking) ->
    case {blockchain:height(Blockchain),
        ...
        {{ok, ChainHeight}, {ok, LedgerHeight}} when ChainHeight > LedgerHeight ->
            lager:warning(
                "blockchain height ~p is ahead of ledger height ~p~n",
        ...
        {{ok, ChainHeight}, {ok, LedgerHeight}} when ChainHeight < LedgerHeight ->
            ...
            lager:warning(
                "ledger height ~p is ahead of blockchain height ~p",
...
resync_fun(
    ...
    case get_block(LedgerHeight, Blockchain) of
        {error, _} when LedgerHeight == 0 ->
            ...
            ok = blockchain_worker:notify(
                {integrate_genesis_block, GenesisHash}
            ),
            case blockchain_ledger_v1:new_snapshot(
                     blockchain:ledger(Blockchain)
                 ) of
                {error, Reason} = Error ->
                    lager:error(
                        "Error creating snapshot, Reason: ~p",
                        [Reason]
                    ),
                    Error;
                {ok, NewLedger} ->
                    ok = blockchain_worker:notify(
                        {add_block, GenesisHash, true, NewLedger}
                    )
            end,
        ...
        {error, _} ->
            ...
            lager:warning(
                "cannot resume ledger resync, missing block ~p",
                [LedgerHeight]
            ),
            blockchain_lock:release();
        {ok, LedgerLastBlock} ->
            ...
            LastKnownBlock =
                case get_block(hd(HashChain), Blockchain) of
                    ...
                    {error, not_found} ->
                        {ok, LKB} =
                            get_block(hd(tl(HashChain)), Blockchain),
                        LKB
                end,
            %% check we have a contiguous chain back to the current ledger head from the blockchain head
            case length(HashChain) ==
                     (ChainHeight - LedgerHeight) andalso
                     EndHash ==
                         blockchain_block:prev_hash(
                             LastKnownBlock
                         ) of
                    ...
                    try
                        lists:foreach(
                            fun (Hash) ->
                                ...
                                lager:info("absorbing block ~p", [
                                    blockchain_block:height(Block)
                                ]),
                                case application:get_env(
                                         blockchain,
                                         force_resync_validation,
                                         false
                                     ) of
                                    true ->
                                        ok = blockchain_txn:absorb_and_commit(
...
save_plausible_block(Block, #blockchain{db = DB, plausible_blocks = PlausibleBlocks}) ->
    ...
    case rocksdb:get(DB, PlausibleBlocks, Hash, []) of
        {ok, _} ->
            ...
            exists;
        _ ->
            rocksdb:put(DB, PlausibleBlocks, Hash, blockchain_block:serialize(Block), [
                {sync, true}
            ])
    end.
...
check_plausible_blocks(#blockchain{db = DB, plausible_blocks = CF} = Chain) ->
    ...
    SortedBlocks = lists:sort(
        fun (A, B) -> blockchain_block:height(A) =< blockchain_block:height(B) end,
        Blocks
    ),
    {ok, Batch} = rocksdb:batch(),
    lists:foreach(
...
get_plausible_blocks(Itr, {ok, _Key, BinBlock}, Acc) ->
    NewAcc =
        try blockchain_block:deserialize(BinBlock) of
            ...
            What:Why ->
                lager:warning(
                    "error when deserializing plausible block at key ~p: ~p ~p",
                    [_Key, What, Why]
                ),
                Acc
        end,
...
blocks_test_() ->
    {timeout, 30000, fun () ->
        ...
        Txn = blockchain_txn_vars_v1:new(
            Vars,
            1,
            #{master_key => BinPub}
        ),
        Proof =
            blockchain_txn_vars_v1:create_proof(
                Priv,
                Txn
            ),
        VarTxns = [
            blockchain_txn_vars_v1:key_proof(
                Txn,
                Proof
            )
        ],
        ...
        GenBlock =
            blockchain_block:new_genesis_block(
                VarTxns
            ),
        GenHash = blockchain_block:hash_block(
            GenBlock
        ),
        TmpDir =
            test_utils:tmp_dir("blocks_test"),
        {ok, Chain} = new(
            TmpDir,
            GenBlock,
            undefined,
        ...
        ?assert(
            meck:validate(blockchain_ledger_v1)
        ),
        meck:unload(blockchain_ledger_v1),
        ?assert(
            meck:validate(blockchain_block)
        ),
        meck:unload(blockchain_block),
        ?assert(
            meck:validate(blockchain_worker)
        ),
        meck:unload(blockchain_worker),
        ?assert(
            meck:validate(blockchain_election)
        ),
        meck:unload(blockchain_election),
        ?assert(
            meck:validate(blockchain_swarm)
        ),
