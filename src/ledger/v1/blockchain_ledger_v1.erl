...
lookup_gateways_from_hex(Hex, Ledger) when is_integer(Hex) ->
    H3CF = h3dex_cf(Ledger),
    case cache_get(Ledger, H3CF, <<"gws-", (h3_to_key(Hex))/binary>>, []) of
        {ok, Bin} ->
            binary_to_term(Bin);
        not_found ->
            ...
            case get_context(Ledger) of
                ...
                _ ->
                    ...
                    cache_put(Ledger, H3CF, <<"gws-", (h3_to_key(Hex))/binary>>, term_to_binary(GWs)),
...
clear_h3dex_gws(Hex, Ledger) ->
    ...
    H3CF = h3dex_cf(Ledger),
    cache_fold(Ledger, H3CF,
               fun({<<"gws-", _/binary>> = Key, _Val}, Acc) ->
                    cache_delete(Ledger, H3CF, Key),
                    Acc
               end, ok, [

                        {start, {seek, <<"gws-", (find_lower_bound_hex(Hex))/binary>>}},
                        {iterate_upper_bound, <<"gws-", (increment_bin(h3_to_key(Hex)))/binary>>}
                        ]).
