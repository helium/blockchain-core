...
find_data_credits_entry(Address, Ledger) ->
    EntriesCF = entries_cf(Ledger),
    case cache_get(Ledger, EntriesCF, data_credits_entry_key(Address), []) of
        {ok, BinEntry} ->
...
credit_data_credits(Address, Amount, Ledger) ->
    ...
    case ?MODULE:find_data_credits_entry(Address, Ledger) of
        {error, not_found} ->
            ...
            cache_put(Ledger, EntriesCF, data_credits_entry_key(Address), Bin);
        {ok, Entry} ->
...
%% @end
%%--------------------------------------------------------------------
-spec data_credits_entry_key(binary()) -> binary().
data_credits_entry_key(Bin) ->
    <<Bin/binary, "/data_credits">>.
