...
-export([
    ...
    find_entry/2,
    credit_account/3,
    debit_account/3, debit_account/4,
    debit_fee_from_account/3,
    check_balance/3,
...
find_poc(OnionKeyHash, Ledger) ->
    ...
    erlang:display("----- OnionKeyHash find_poc-----"),
...
request_poc_(OnionKeyHash, SecretHash, Challenger, BlockHash, Ledger, Gw0, PoCs) ->
    erlang:display("----- OnionKeyHash request_poc-----"),
    erlang:display(OnionKeyHash),
    erlang:display("----- OnionKeyHash request_poc-----"),
...
-spec debit_account(libp2p_crypto:pubkey_bin(), integer(), ledger()) -> ok | {error, any()}.
debit_account(Address, Amount, Ledger) ->
    case ?MODULE:find_entry(Address, Ledger) of
        {error, _}=Error ->
            Error;
        {ok, Entry} ->
