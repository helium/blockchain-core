...
release_snapshot(#ledger_v1{snapshot=_Snapshot}) ->
    lager:warning("releasing snapshot").
...
cache_fold(Ledger, {CFName, DB, CF}, Fun0, OriginalAcc, Opts) ->
    ...
    case context_cache(Ledger) of
        {undefined, undefined} ->
            %% fold rocks directly
            rocks_fold(DB, CF, Opts, Fun0, OriginalAcc);
        {Cache, _GwCache} ->
