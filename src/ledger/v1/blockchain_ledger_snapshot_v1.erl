...
-type snapshot_v5() ::
    #{
        ...
        % TODO More specific
        delayed_vars => [{integer(), [{Hash :: term(), TODO :: term()}]}],
        % According to spec of blockchain_ledger_v1:snapshot_threshold_txns
        threshold_txns => [{binary(), binary()}],
        master_key => binary(),
        multi_keys => [binary()],
        vars_nonce => pos_integer(),
        % TODO What is the term()?
        vars => [{binary(), term()}],
    ...
    load_into_ledger_(
        [
            ...
            [
             ...
             || maps:is_key(hnt_burned, Snapshot)
            ] ++
...
binary_to_list_of_binaries(
    <<109, Length:32/integer-unsigned-big, Bin:Length/binary, Rest/binary>>, Acc
) ->
...
v2_to_v3(#blockchain_snapshot_v2{
    ...
    #blockchain_snapshot_v3{
        ...
        dc_accounts = kvl_map_vals(
            fun blockchain_ledger_data_credits_entry_v1:serialize/1, DCAccounts
        ),

        security_accounts = kvl_map_vals(
            fun blockchain_ledger_security_entry_v1:serialize/1, SecurityAccounts
        ),
                ...
                _ ->
                    case Field of
                        ...
                        accounts ->
                            ...
                            [
                                {Field, {
                                    libp2p_crypto:bin_to_b58(K),
                                    blockchain_ledger_entry_v1:deserialize(V),
                                    case proplists:get_value(K, BI) of
                                        undefined -> undefined;
                                        V2 -> blockchain_ledger_entry_v1:deserialize(V2)
                                    end
                                }}
                             || {K, V} <- AUniq
                            ] ++
                                [
                                 ...
                                 || {K, V} <- BUniq, not lists:keymember(K, 1, AI)
                                ] ++ Acc;
                        gateways ->
                            AUniq = AI -- BI,
                            BUniq = BI -- AI,
                        ...
                        blocks ->
                            ...
                            case
                                {AHeightAndHash -- BHeightAndHash, BHeightAndHash -- AHeightAndHash}
                            of
                                {[], []} ->
                                    Acc;
                                {ADiffs, BDiffs} ->
                                    [
                                        {Field, [Height || {Height, _Hash} <- ADiffs], [
                                            Height
                                         || {Height, _Hash} <- BDiffs
                                        ]}
                                        | Acc
...
minimize_gw(A0, B0) ->
    ...
    case minimize_witnesses(AWits, BWits) of
        [] -> Compare;
        MiniWit -> [{witnesses, MiniWit} | Compare]
    end.
...
deserialize_field(hexes, Bin) ->
    ...
    %% future.
    try
        binary_to_proplist(Bin)
    catch
        What:Why ->
            lager:warning(
                "deserializing hexes from snapshot failed ~p ~p, falling back to binary_to_term", [
                    What, Why
                ]
            ),
            binary_to_term(Bin)
...
mk_bin_iterator(<<>>) ->
    fun() -> ok end;
mk_bin_iterator(
    <<SizK:32/little-unsigned-integer, K:SizK/binary, SizV:32/little-unsigned-integer,
        V:SizV/binary, Rest/binary>>
) ->
    fun() ->
...
bin_pairs_from_bin(
    <<
        SizK:32/little-unsigned-integer,
        K:SizK/binary,
        SizV:32/little-unsigned-integer,
        V:SizV/binary,
        Rest/binary
    >>,
    Pairs
