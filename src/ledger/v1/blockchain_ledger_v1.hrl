-record(hook, {
    ...
    %% fun called for each incremental/partial update for the relevant CF
    hook_inc_fun :: fun(),
    %% fun called after all incremental/partial updates are complete for the relevant cf
    hook_end_fun :: fun(),
...
-record(validator_stake_v1, {
    ...
    %% aka `target_block'
    stake_release_height = 0 :: non_neg_integer()
}).
...
%% stores the current calculated price
-define(CURRENT_ORACLE_PRICE, <<"current_oracle_price">>).
%% stores a rolling window of prices
-define(ORACLE_PRICES, <<"oracle_prices">>).
...
%% biggest unsigned number in 25 bits
