...
block_height(Entry = #oracle_price_entry{}, Value) when
    is_integer(Value) andalso
        Value >= 0
->
...
-spec public_key(
    Entry :: oracle_price_entry(),
    Value :: libp2p_crypto:pubkey_bin()
) -> Updated :: oracle_price_entry().
...
price(Entry = #oracle_price_entry{}, Value) when
    is_integer(Value) andalso
        Value >= 0
->
