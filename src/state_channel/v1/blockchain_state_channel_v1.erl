...
-spec old_merge(SCA :: state_channel(),
            SCB :: state_channel(),
            MaxActorsAllowed :: pos_integer()) -> state_channel().
old_merge(SCA, SCB, MaxActorsAllowed) ->
...
merge(SCA, SCB, MaxActorsAllowed) ->
    ...
    summaries(merge(SC1, SC2, MaxActorsAllowed, []), SCB2).

merge([], [B | T], Max, Acc) when length(Acc) =< Max ->
    merge([], T, Max, [B | Acc]);
merge([A | T], [], Max, Acc) when length(Acc) =< Max ->
    merge(T, [], Max, [A | Acc]);
merge(
