...
packet(Packet, DefaultRouters, Region) ->
    gen_server:cast(
        ?SERVER, {packet, Packet, DefaultRouters, Region, erlang:system_time(millisecond)}
    ).

-spec get_known_channels(SCID :: blockchain_state_channel_v1:id()) ->
...
-spec purchase(
    Purchase :: blockchain_state_channel_purchase_v1:purchase(),
    HandlerPid :: pid()
) -> ok.
...
-spec banner(
    Banner :: blockchain_state_channel_banner_v1:banner(),
    HandlerPid :: pid()
) -> ok.
...
-spec reject(
    Rejection :: blockchain_state_channel_rejection_v1:rejection(),
    HandlerPid :: pid()
) -> ok.
...
%% ------------------------------------------------------------------
init(Args) ->
    SCClientTransportHandler = application:get_env(
        blockchain, sc_client_transport_handler, blockchain_state_channel_handler
    ),
        ...
        BannerSC ->
            case is_valid_sc(BannerSC, State) of
                {error, causal_conflict} ->
                    lager:error("causal_conflict for banner sc_id: ~p", [
...
handle_cast({reject, Rejection, HandlerPid}, State) ->
    ...
    NewState =
        case dequeue_packet(HandlerPid, State) of
...
handle_info({dial_fail, AddressOrOUI, _Reason}, State0) ->
    ...
    lager:error("failed to dial ~p: ~p dropping ~p packets", [
...
handle_info(
    {blockchain_event, {add_block, BlockHash, false, Ledger}},
    #state{chain = Chain, pubkey_bin = PubkeyBin, sig_fun = SigFun, pending_closes = PendingCloses} =
        State
    ...
    case Block of
        ...
        Block ->
            Txns = lists:filter(
                fun(T) -> blockchain_txn:type(T) == blockchain_txn_routing_v1 end,
                    ...
                    fun(T, Acc) ->
                        case blockchain_txn:type(T) == blockchain_txn_state_channel_close_v1 of
                            true ->
                                ...
                                case
                                    lists:member(SCID, PendingCloses) orelse
    ...
    ExpiringChannels = lists:foldl(
        fun([H | _] = SC, Acc) ->
            ...
            case
                (not lists:member(SCID, PendingCloses ++ ClosingChannels)) andalso
                            ...
                            case blockchain_ledger_v1:find_state_channel(SCID, SCOwner, Ledger) of
                                ...
                                {ok, LSC} ->
                                    case blockchain_ledger_state_channel_v2:is_v2(LSC) of
                                        ...
                                        true ->
                                            case
                                                blockchain_ledger_state_channel_v2:close_state(LSC)
                                            of
                                                undefined ->
                                                    Txn = blockchain_txn_state_channel_close_v1:new(
                                                        H, PubkeyBin
                                                    ),
                                                    SignedTxn = blockchain_txn_state_channel_close_v1:sign(
                                                        Txn, SigFun
                                                    ),
    ...
    {noreply, State#state{
        pending_closes = lists:usort(PendingCloses ++ ClosingChannels ++ ExpiringChannels)
    }};
...
handle_info({'DOWN', _Ref, process, Pid, _}, #state{streams = Streams, packets = Packets} = State) ->
    ...
    FilteredPackets = maps:filter(
        fun(StreamPid, _PacketQueue) ->
            StreamPid /= Pid
        end,
        Packets
    ),
...
handle_packet(
    ...
    lager:info("handle_packet ~p to ~p with handler ~p", [
            ...
            case StreamKey of
                {address, Address} ->
                    case find_stream(Address, StateAcc) of
                        ...
                        dialing ->
                            lager:debug("stream is still dialing queueing packet, address: ~p", [
                                Address
                            ]),
                            add_packet_to_waiting(
                        ...
                        {unverified, _Stream} ->
                            ...
                            lager:debug("unverified stream, add_packet_to_waiting, address: ~p", [
                                Address
                            ]),
                            add_packet_to_waiting(
                                Address, {Packet, Region, ReceivedTime}, StateAcc
                        ...
                        Stream ->
                            lager:debug("stream ~p, send_packet_when_v1, address: ~p", [
                                Stream, Address
                            ]),
                ...
                {oui, OUI} ->
                    case find_stream(OUI, StateAcc) of
                        ...
                        {unverified, _Stream} ->
                            ...
                            add_packet_to_waiting(OUI, {Packet, Region, ReceivedTime}, StateAcc);
                        Stream ->
                            lager:debug("got stream: ~p, send_packet_or_offer, oui: ~p", [
                                Stream, OUI
                            ]),
                            send_packet_or_offer(
                                Stream, OUI, Packet, Region, ReceivedTime, StateAcc
                            )
                    end
            end
        end,
...
handle_purchase(
    ...
    case is_valid_sc(PurchaseSC, State) of
        {error, causal_conflict} ->
            lager:error("causal_conflict for purchase sc_id: ~p", [
        ...
        ok ->
            ...
            case blockchain_ledger_v1:is_state_channel_overpaid(PurchaseSC, Ledger) of
                true ->
                    lager:error("insufficient dcs for purchase sc: ~p: ~p - ~p = ~p", [
                        PurchaseSC, DCBudget, TotalDCs, RemainingDCs
                    ]),
                    _ = libp2p_framed_stream:close(Stream),
                ...
                false ->
                    case dequeue_packet(Stream, State) of
                        {undefined, State0} ->
                            ...
                            lager:debug("failed dequeue_packet, stream: ~p, purchase: ~p", [
                                Stream, Purchase
                            ]),
                            State0;
                        {{Packet, ReceivedTime}, NewState} ->
                            Payload = blockchain_helium_packet_v1:payload(Packet),
                            PacketDCs = blockchain_utils:calculate_dc_amount(
                                Ledger, byte_size(Payload)
                            ),
                            case RemainingDCs >= PacketDCs of
                                false ->
                                    lager:error(
                                        "current packet (~p) (dc charge: ~p) will exceed remaining DCs (~p) in this SC, dropping",
                                ...
                                true ->
                                    ...
                                    case (TotalDCs - PrevTotal) >= PacketDCs of
                                        true ->
                                            Region = blockchain_state_channel_purchase_v1:region(
                                                Purchase
                                            ),
                                            lager:debug(
                                                "successful purchase validation, sending packet: ~p",
                                        ...
                                        false ->
                                            ...
                                            lager:error(
                                                "purchase not valid - did not pay for packet: ~p, dropping.",
                                                [Packet]
                                            ),
                                            _ = libp2p_framed_stream:close(Stream),
...
-spec add_stream(
    AddressOrOUI :: non_neg_integer() | string(), Stream :: pid() | dialing, State :: state()
) -> state().
...
add_packet_to_waiting(
    AddressOrOUI, {Packet, Region, ReceivedTime}, #state{waiting = Waiting} = State
) ->
...
enqueue_packet(Stream, Packet, ReceivedTime, #state{packets = Packets} = State) ->
    ...
    State#state{
        packets = maps:update_with(
            Stream,
            fun(PacketList) -> queue:in(Value, PacketList) end,
            queue:in(Value, queue:new()),
            Packets
        )
    }.

-spec dequeue_packet(Stream :: pid(), State :: state()) ->
    {undefined | {blockchain_helium_packet_v1:packet(), non_neg_integer()}, state()}.
...
maybe_send_packets(
    AddressOrOUI, HandlerPid, #state{pubkey_bin = PubkeyBin, sig_fun = SigFun} = State
) ->
    Packets = get_waiting_packet(AddressOrOUI, State),
    case AddressOrOUI of
        OUI when is_integer(OUI) ->
            lager:debug("dial_success sending ~p packets or offer depending on OUI", [
                erlang:length(Packets)
            ]),
            State1 = lists:foldl(
        ...
        Address when is_list(Address) ->
            State1 =
                case blockchain:config(?sc_version, blockchain:ledger(State#state.chain)) of
                    {ok, N} when N >= 2 ->
                        lager:debug("valid banner for ~p, sending ~p packets", [
                            AddressOrOUI, length(Packets)
                        ]),
                        lists:foldl(
                            fun({Packet, Region, ReceivedTime}, Acc) ->
                    ...
                    _ ->
                        lists:foreach(
                            fun({Packet, Region, ReceivedTime}) ->
                                ok = send_packet(
                                    PubkeyBin, SigFun, HandlerPid, Packet, Region, ReceivedTime
                                )
                            end,
                            Packets
                        ),
    ...
    case get_state_channels(SCID, State) of
        ...
        {ok, [KnownSC]} ->
            ...
            case Check /= conflict of
                ...
                false ->
                    lager:notice(
                        "causality check: ~p, sc_id: ~p, same_sc_id: ~p, nonces: ~p ~p",
                        [
                            Check,
                            SCID,
                            SCID == blockchain_state_channel_v1:id(KnownSC),
                            blockchain_state_channel_v1:nonce(SC),
                            blockchain_state_channel_v1:nonce(KnownSC)
                        ]
                    ),
                    false
            end;
...
is_overspent_sc(SC, State = #state{chain = Chain}) ->
    ...
    case get_state_channels(SCID, State) of
        ...
        {error, _} ->
            ...
            false;
        {ok, KnownSCs} ->
            lists:any(fun(E) -> blockchain_ledger_v1:is_state_channel_overpaid(E, Ledger) end, [
                SC | KnownSCs
            ])
    end.
...
get_previous_total_dcs(SC, State) ->
    ...
    case get_state_channels(SCID, State) of
        ...
        {ok, PrevSCs} ->
            lager:error(
                "multiple copies of state channels for id: ~p, returning current total",
                [PrevSCs]
            ),
...
-spec close_state_channel(SC :: blockchain_state_channel_v1:state_channel(), State :: state()) ->
    ok.
close_state_channel(SC, State = #state{pubkey_bin = PubkeyBin, sig_fun = SigFun}) ->
    ...
    case get_state_channels(SCID, State) of
        {ok, [SC0]} ->
            ...
            ok = blockchain_worker:submit_txn(SignedTxn),
            lager:info(
                "closing state channel on conflict ~p: ~p",
        ...
        {ok, SCs} ->
            ...
            case lists:filter(fun(E) -> conflicts(E, SC) end, SCs) of
                ...
                Conflicts ->
                    ...
                    Txn = blockchain_txn_state_channel_close_v1:new(
                        lists:last(SortedConflicts), SC, PubkeyBin
                    ),
                    SignedTxn = blockchain_txn_state_channel_close_v1:sign(Txn, SigFun),
                    ok = blockchain_worker:submit_txn(SignedTxn),
                    lager:info(
...
conflicts(SCA, SCB) ->
    case blockchain_state_channel_v1:compare_causality(SCA, SCB) of
        conflict ->
            lager:info(
                "sc_client reports state_channel conflict, SCA_ID: ~p, SCB_ID: ~p",
                [
...
-spec debug_multiple_scs(
    SC :: blockchain_state_channel_v1:state_channel(),
    KnownSCs :: [blockchain_state_channel_v1:state_channel()]
) -> ok.
