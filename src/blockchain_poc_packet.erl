...
-spec decrypt(Packet :: binary(), ECDHFun :: libp2p_crypto:ecdh_fun(), BlockHash :: binary()) -> error | {Payload :: binary(), NextLayer :: binary()}.
decrypt(<<IV0:16/integer-unsigned-little, OnionCompactKey:33/binary, Tag:4/binary, CipherText/binary>>, ECDHFun, BlockHash) ->
    try libp2p_crypto:bin_to_pubkey(OnionCompactKey) of
        PubKey ->
            SharedKey = ECDHFun(PubKey),
            IV = <<0:80/integer, IV0:16/integer-unsigned-little>>,
            BlockKey = crypto:hash(sha256, <<SharedKey/binary, BlockHash/binary>>),
...
-spec build(OnionKey :: libp2p_crypto:key_map(),
            ...
            BlockHash :: binary()) -> {OuterLayer :: binary(), Layers :: [binary()]}.
build(#{secret := OnionPrivKey, public := OnionPubKey}, IV, PubKeysAndData, BlockHash) ->
    ...
    DataMatrix = lists:foldl(fun({Row, Col=1}, Acc) ->
                                ...
                                ({Row, Col}, Acc) ->
                                     ...
                                     setelement(((Row-1)*N)+Col, Acc, encrypt_cell(Row+1, Col-1, N, Acc, OnionCompactKey, ECDHFun, IVs, PubKeysAndData, BlockHash))
    ...
    PaddingMatrix = lists:foldl(fun({Row, Col}, Acc) when Col == N ->
                                        ...
                                        case Row > N of
                                            false ->
                                                ...
                                                <<_:ExtraTagBytes/binary, Cell/binary>> = encrypt_cell(Row, Col, N, TempMatrix, OnionCompactKey, ECDHFun, IVs, PubKeysAndData, BlockHash),
    ...
    EncryptedMatrix = lists:foldl(fun(R, Acc) ->
                                          ...
                                          Row = encrypt_row(R, N, Acc, OnionCompactKey, ECDHFun, IVs, PubKeysAndData, BlockHash),
    ...
    [FirstRow|_] = PacketRows = lists:map(fun(RowNumber) ->
                                                  case RowNumber > N of
                                                      true ->
                                                          ...
                                                          list_to_binary(Bins);
                                                      false ->
                                                          encrypt_row(RowNumber, N, EncryptedMatrix, OnionCompactKey, ECDHFun, IVs, PubKeysAndData, BlockHash)
                                                  end
                                          end, lists:seq(1, N+1)),
...
encrypt_cell(Row, Column, N, Matrix, OnionCompactKey, ECDHFun, IVs, KeysAndData, BlockHash) ->
...
encrypt_row(Row, N, Matrix, OnionCompactKey, ECDHFun, IVs, KeysAndData, BlockHash) ->
    SecretKey = ECDHFun(element(1, lists:nth(Row, KeysAndData))),
...
encrypt_decrypt_test() ->
    ...
    {OuterPacket, Rows} = build(OnionKey, IV, KeysAndData, BlockHash),
    ...
    {<<"abc">>, Remainder1} = decrypt(OuterPacket, libp2p_crypto:mk_ecdh_fun(PrivKey1), BlockHash),
    ?assert(lists:all(fun(E) -> E == error end, [ decrypt(OuterPacket, libp2p_crypto:mk_ecdh_fun(PK), BlockHash) || PK <- PrivKeys -- [PrivKey1]])),
    ...
    {<<"def">>, Remainder2} = decrypt(Remainder1, libp2p_crypto:mk_ecdh_fun(PrivKey2), BlockHash),
    ?assert(lists:all(fun(E) -> E == error end, [ decrypt(Remainder1, libp2p_crypto:mk_ecdh_fun(PK), BlockHash) || PK <- PrivKeys -- [PrivKey2]])),
    {<<"ghi">>, Remainder3} = decrypt(Remainder2, libp2p_crypto:mk_ecdh_fun(PrivKey3), BlockHash),
    ?assert(lists:all(fun(E) -> E == error end, [ decrypt(Remainder2, libp2p_crypto:mk_ecdh_fun(PK), BlockHash) || PK <- PrivKeys -- [PrivKey3]])),
    {<<"jhk">>, Remainder4} = decrypt(Remainder3, libp2p_crypto:mk_ecdh_fun(PrivKey4), BlockHash),
    ?assert(lists:all(fun(E) -> E == error end, [ decrypt(Remainder3, libp2p_crypto:mk_ecdh_fun(PK), BlockHash) || PK <- PrivKeys -- [PrivKey4]])),
    {<<"lmn">>, Remainder5} = decrypt(Remainder4, libp2p_crypto:mk_ecdh_fun(PrivKey5), BlockHash),
    ?assert(lists:all(fun(E) -> E == error end, [ decrypt(Remainder4, libp2p_crypto:mk_ecdh_fun(PK), BlockHash) || PK <- PrivKeys -- [PrivKey5]])),
    {<<"opq">>, Remainder6} = decrypt(Remainder5, libp2p_crypto:mk_ecdh_fun(PrivKey6), BlockHash),
    ?assert(lists:all(fun(E) -> E == error end, [ decrypt(Remainder5, libp2p_crypto:mk_ecdh_fun(PK), BlockHash) || PK <- PrivKeys -- [PrivKey6]])),
    {<<"rst">>, Remainder7} = decrypt(Remainder6, libp2p_crypto:mk_ecdh_fun(PrivKey7), BlockHash),
    ?assert(lists:all(fun(E) -> E == error end, [ decrypt(Remainder6, libp2p_crypto:mk_ecdh_fun(PK), BlockHash) || PK <- PrivKeys -- [PrivKey7]])),
    {<<"uvw">>, Remainder8} = decrypt(Remainder7, libp2p_crypto:mk_ecdh_fun(PrivKey8), BlockHash),
    ?assert(lists:all(fun(E) -> E == error end, [ decrypt(Remainder7, libp2p_crypto:mk_ecdh_fun(PK), BlockHash) || PK <- PrivKeys -- [PrivKey8]])),
