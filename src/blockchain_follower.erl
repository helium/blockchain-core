...
handle_info(
        ...
        fun() ->
            ...
            case BlockHeight of
                ...
                X when X > Height + 1 ->
                    ...
                    lager:info("trying to absorb missing blocks [~p..~p]", [
                        hd(BlockHeights), lists:last(BlockHeights)
                    ]),
                    lists:foldl(
                        fun(MissingHeight, {ok, FS}) ->
                            {ok, MissingBlock} = blockchain:get_block(MissingHeight, Chain),
                            MissingHash = blockchain_block:hash_block(MissingBlock),
                            {ok, MissingLedger} =
                                case RequiresLedger of
                                    true ->
                                        blockchain:ledger_at(MissingHeight, Chain);
                                    false ->
                                        {ok, undefined}
                                end,
                            FollowerMod:load_block(
                                MissingHash, MissingBlock, true, MissingLedger, FS
                            )
                        end,
                        {ok, State#state.follower_state},
                        BlockHeights
    ...
    case MaybePlaybackBlocks() of
        {error, already_loaded} ->
            {noreply, State};
        {ok, FollowerState} ->
            {ok, NewFollowerState} = FollowerMod:load_block(
                Hash, Block, Sync, Ledger, FollowerState
            ),
