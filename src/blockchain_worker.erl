...
-record(state, {
    blockchain ::
        undefined | {no_genesis, blockchain:blockchain()} | blockchain:blockchain(),
    swarm :: undefined | pid(),
    ...
    case Chain of
        ...
        _ ->
            ...
            try
                ...
                FPMD =
                    case blockchain:sync_height(Chain) ==
        ...
        case application:get_env(blockchain, honor_quick_sync, false) of
            true ->
                case QuickSyncMode of
                    ...
                    blessed_snapshot ->
                        {ok, Hash} =
                            application:get_env(blockchain, blessed_snapshot_block_hash),
                        {ok, Height} = application:get_env(
                            blockchain,
...
handle_cast(
    ...
    #state{blockchain = {no_genesis, Blockchain}, swarm_tid = SwarmTid} = State
) ->
    case blockchain_block:is_genesis(GenesisBlock) of
        ...
        true ->
            ...
            lager:info(
                ...
                [lager:pr(Blockchain, blockchain), ConsensusAddrs]
            ),
    ...
    NewState =
        case {blockchain:head_hash(Chain), blockchain:head_block(Chain)} of
            ...
            {{ok, LocalHead}, {ok, LocalHeadBlock}} ->
                ...
                case LocalHeight < Height orelse
                         (LocalHeight == Height andalso Head /= LocalHead) of
                    false ->
                        ok;
                    true ->
                        start_sync(State)
...
handle_info({blockchain_event, {add_block, _Hash, _Sync, _Ledger}}, State) ->
    ...
    {noreply, State};
handle_info(
    {'DOWN', SyncRef, process, _SyncPid, _Reason},
...
handle_info(
    ...
    #state{gossip_ref = GossipRef, blockchain = Blockchain, swarm_tid = SwarmTID} = State
) ->
...
handle_info(
    ...
    case Reason of
        normal ->
            ...
            {noreply,
                schedule_sync(State#state{sync_paused = false, absorb_info = undefined})};
        shutdown ->
            {noreply, State#state{absorb_info = undefined}};
        Reason when Retries > 0 ->
            lager:warning(
                "Absorb process exited with reason ~p, retrying ~p more times",
                [Reason, Retries]
            ),
...
handle_info(
    ...
    case Reason of
        normal ->
            ...
            {noreply,
                schedule_sync(State#state{sync_paused = false, resync_info = undefined})};
        shutdown ->
            {noreply, State#state{resync_info = undefined}};
        Reason when Retries > 0 ->
            lager:warning(
                "Resync process exited with reason ~p, retrying ~p more times",
                [Reason, Retries]
            ),
...
maybe_sync_blocks(#state{blockchain = Chain} = State) ->
    ...
    SkewedSyncCooldownTime =
        application:get_env(blockchain, skewed_sync_cooldown_time, 300),
...
snapshot_sync(
    Hash,
    Height,
    #state{blockchain = Chain, swarm_tid = SwarmTID, swarm = Swarm} = State
) ->
...
add_handlers(SwarmTID, Blockchain) ->
    GossipPid = libp2p_swarm:gossip_group(
        SwarmTID
    ),
    Ref =
        erlang:monitor(process, GossipPid),
    ...
    ok = libp2p_swarm:add_stream_handler(
        ...
        ?SNAPSHOT_PROTOCOL,
        {libp2p_framed_stream, server,
            [blockchain_snapshot_handler, ?SERVER, Blockchain]}
    ),
    {ok, Ref}.
...
remove_handlers(SwarmTID) ->
    ...
    ok = libp2p_group_gossip:remove_handler(
            ...
            {ok, Stream} ->
                ...
                receive
                    ...
                    {'DOWN', Ref1, process, Stream, _Reason} ->
                        ...
                        ok
                after timer:minutes(
                          application:get_env(blockchain, sync_timeout_mins, 10)
                      ) ->
                    libp2p_framed_stream:close(Stream),
                    ok
                end;
...
start_snapshot_sync(Hash, Height, Swarm, Chain, Peer) ->
    ...
    spawn_monitor(fun () ->
        case libp2p_swarm:dial_framed_stream(
                 Swarm,
                 Peer,
                 ?SNAPSHOT_PROTOCOL,
    ...
    ok = blockchain_txn_mgr:submit(
        ...
        (fun (Res) ->
            case Res of
                ...
                {error, Reason} ->
                    lager:error(
                        "failed to submit txn: ~s error: ~p",
