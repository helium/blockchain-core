...
-define(PLAUSIBLE_SIZE, 50 * 1024000). % 50 MB or more is a plausible size for a snapshot download
...
attempt_fetch_s3_snapshot(BaseUrl, Height) ->
    ...
    case have_plausible_snapshot(Filepath) of
        true ->
            lager:info("Already have plausible snapshot for height ~p", [Height]),
            {ok, Filepath};
        false -> do_s3_download(build_url(BaseUrl, Filename), BaseDir, Filename, Filepath)
    end.

do_s3_download(Url, BaseDir, Filename, Filepath) ->
    ScratchFile = filename:join([BaseDir, "snap-scratch", Filename]),
    ...
    case httpc:request(get, {Url, Headers}, HTTPOptions, Options) of
        {ok, saved_to_file} ->
            ...
            {ok, _BytesCopied} = file:copy(ScratchFile, Filepath),
            lager:info("snap copied to ~p, deleting ~p", [Filepath, ScratchFile]),
            ok = file:delete(ScratchFile),
            {ok, Filepath};
...
have_plausible_snapshot(File) ->
    case file:read_file_info(File, [raw, {time, posix}]) of
        {error, enoent} -> false; %% file does not exist
        {error, _} = Err -> throw(Err);
        {ok, FileInfo} ->
            Type = FileInfo#file_info.type,
            Sz = FileInfo#file_info.size,
            case Sz >= ?PLAUSIBLE_SIZE andalso Type == regular of
                true -> true;
                false ->
                    %% file exists, but it's not
                    %% plausible, so delete it
                    ok = file:delete(File),
                    false
            end
    end.
