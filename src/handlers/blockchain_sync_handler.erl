...
-spec dial(
    SwarmTID :: ets:tab(), Chain :: blockchain:blockchain(), Peer :: libp2p_crypto:pubkey_bin()
) ->
    {ok, pid()} | {error, any()}.
    ...
    DialFun =
        fun
            ...
            Dial([ProtocolVersion | Rest]) ->
                case
                    blockchain_sync_handler:dial(
                        SwarmTID, Chain, Peer, ProtocolVersion, Heights, GossipedHash
                    )
                of
                    {ok, Stream} ->
                        lager:debug(
                            "dialing Sync stream successful, stream pid: ~p, protocol version: ~p",
                            [Stream, ProtocolVersion]
                        ),
                        {ok, Stream};
                    {error, protocol_unsupported} ->
                        lager:debug(
                            "dialing Sync stream failed with protocol version: ~p, trying next supported protocol version",
                            [ProtocolVersion]
                        ),
                        Dial(Rest);
                    {error, Reason} ->
...
handle_data(
    client,
    Data0,
    #state{blockchain = Chain, path = Path, gossiped_hash = GossipedHash, swarm = SwarmTID} = State
) ->
    Data =
        case Path of
    ...
    case blockchain:save_plausible_blocks(lists:zip(BinBlocks, Blocks), Chain) of
        error ->
            ...
            {noreply, State,
                blockchain_sync_handler_pb:encode_msg(#blockchain_sync_req_pb{
                    msg = {response, true}
                })};
        HighestPlausible ->
...
handle_data(
    ...
    case blockchain_sync_handler_pb:decode_msg(Data, blockchain_sync_req_pb) of
        #blockchain_sync_req_pb{
            msg =
                {hash, #blockchain_sync_hash_pb{
                    hash = Hash,
                    heights = Requested
                }}
        } ->
            ...
            case Blocks of
                ...
                [_ | _] ->
                    ...
                    case Requested1 == [] andalso Requested /= [] of
                        ...
                        _ ->
                            lager:info("sending blocks ~p to sync peer", [
                                element(1, lists:unzip(Blocks))
                            ]),
        ...
        #blockchain_sync_req_pb{msg = {response, true}} when
            Sent < Limit, State#state.last_block_height /= undefined
        ->
...
build_blocks(R, Hash, Blockchain, BatchSize) when is_list(R) ->
    ...
    ExtraBlocks =
        case length(Blocks) < BatchSize of
            true ->
