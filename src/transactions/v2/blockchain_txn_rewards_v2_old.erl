...
%% @end
%%%-------------------------------------------------------------------
-module(blockchain_txn_rewards_v2_old).
...
-export_type([txn_rewards_v2/0, rewards_share_metadata/0, rewards_metadata/0]).
...
%% @doc Return the computed reward shares per gateway
calculate_rewards_share_metadata(Start, End, Ledger, Chain) ->
    ...
    try
        ...
        C:Error:Stack ->
            lager:error("Caught ~p; couldn't calculate rewards share metadata because: ~p~n~p", [C, Error, Stack]),
            Error
    end.

-spec shares_to_hnt_rewards(SharesMap :: rewards_share_map(),
                            Ledger :: blockchain_ledger_v1:ledger(),
                            Vars :: reward_vars() ) ->
                            {ok, Results :: rewards_metadata()} | {error, term()}.
shares_to_hnt_rewards(SharesMap, Ledger, Vars) ->
    try
        Results = finalize_reward_calculations(SharesMap, Ledger, Vars),
        ...
        C:Error:Stack ->
            lager:error("Caught ~p; couldn't calculate hnt rewards because: ~p~n~p", [C, Error, Stack]),
            Error
    end.
