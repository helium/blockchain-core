...
-type dc_rewards_share_map() :: #{
    libp2p_crypto:pubkey_bin() => non_neg_integer(),
    seen => [blockchain_state_channel_v1:id()]
}.
-type rewards_share_metadata() :: #{
    poc_challenger => rewards_share_map(),
...
-type reward_types() ::
    poc_challengers | poc_challengees | poc_witnesses | data_credits | consensus.
-type gateway_key() :: {gateway, reward_types(), libp2p_crypto:pubkey_bin()}.
-type rewards_map() :: #{owner_key() | gateway_key() => number()}.
-type rewards_metadata() :: #{
    poc_challenger => rewards_map(),
...
-spec fee_payer(txn_rewards_v2(), blockchain_ledger_v1:ledger()) ->
    libp2p_crypto:pubkey_bin() | undefined.
fee_payer(_Txn, _Ledger) ->
    undefined.

-spec is_valid(txn_rewards_v2(), blockchain:blockchain()) ->
...
-spec absorb(txn_rewards_v2(), blockchain:blockchain()) ->
    ok | {error, atom()} | {error, {atom(), any()}}.
absorb(Txn, Chain) ->
...
aux_absorb(Txn, AuxLedger, Chain) ->
    ...
    case calculate_rewards_(Start, End, AuxLedger, Chain, true) of
        ...
        {ok, AuxRewards, AuxMD} ->
            ...
            case
                calculate_rewards_(
                    Start, End, blockchain_ledger_v1:mode(active, AuxLedger), Chain, true
                )
            of
                {error, _} = E ->
                    E;
...
calculate_rewards_metadata(Start, End, Chain) ->
    ...
    AccInit = #{
        dc_rewards => PreviousGraceBlockDCRewards,
        poc_challenger => #{},
        poc_challengee => #{},
        poc_witness => #{}
    },
    ...
    try
        ...
        Results0 = fold_blocks_for_rewards(
            Start,
            End,
            Chain,
            Vars,
...
print(#blockchain_txn_rewards_v2_pb{
    start_epoch = Start,
    end_epoch = End
}) ->
    io_lib:format(
...
calculate_reward_for_txn(
    ...
    throw({error, already_existing_rewards_v2});
calculate_reward_for_txn(
    ...
    throw({error, already_existing_rewards_v1});
calculate_reward_for_txn(
    blockchain_txn_poc_receipts_v1,
    Txn,
    _End,
    #{poc_challenger := Challenger} = Acc,
    Chain,
    Ledger,
    Vars
) ->
...
-spec calculate_poc_challengee_rewards(
    Txn :: blockchain_txn:txn(),
    Acc :: rewards_share_metadata(),
    Chain :: blockchain:blockchain(),
    Ledger :: blockchain_ledger_v1:ledger(),
...
calculate_poc_challengee_rewards(
    Txn,
    #{poc_challengee := ChallengeeMap} = Acc,
    Chain,
    Ledger,
    #{var_map := VarMap} = Vars
) ->
    Path = blockchain_txn_poc_receipts_v1:path(Txn),
    NewCM = poc_challengees_rewards_(
        Vars, Path, Path, Txn, Chain, Ledger, true, VarMap, ChallengeeMap
    ),
    Acc#{poc_challengee => NewCM}.

-spec calculate_poc_witness_rewards(
    Txn :: blockchain_txn:txn(),
    Acc :: rewards_share_metadata(),
    Chain :: blockchain:blockchain(),
    Ledger :: blockchain_ledger_v1:ledger(),
...
finalize_reward_calculations(
    #{
        ...
        poc_challengee := ChallengeeShares
    } = AccIn,
    Ledger,
    Vars
) ->
...
-spec prepare_rewards_v2_txns(
    Results :: rewards_metadata(),
    Ledger :: blockchain_ledger_v1:ledger()
) -> rewards().
...
-spec calculate_epoch_reward(pos_integer(), pos_integer(), blockchain_ledger_v1:ledger()) ->
    float().
...
-spec calculate_epoch_reward(
    pos_integer(), pos_integer(), pos_integer(), blockchain_ledger_v1:ledger()
) -> float().
calculate_epoch_reward(Version, Start, End, Ledger) ->
    {ok, ElectionInterval} = blockchain:config(?election_interval, Ledger),
...
calculate_epoch_reward(
    Version, Start, End, BlockTime0, _ElectionInterval, MonthlyReward, Ledger
) when Version >= 6 ->
...
poc_challenger_reward(Txn, ChallengerRewards, #{poc_version := Version}) ->
    ...
    case
        blockchain_txn_poc_receipts_v1:check_path_continuation(
...
-spec normalize_challenger_rewards(
    ChallengerRewards :: rewards_share_map(),
    Vars :: reward_vars()
) -> rewards_map().
...
-spec normalize_challengee_rewards(
    ChallengeeRewards :: rewards_share_map(),
    Vars :: reward_vars()
) -> rewards_map().
...
-spec poc_challengees_rewards_(
    ...
    Acc0 :: rewards_share_map()
) -> rewards_share_map().
poc_challengees_rewards_(_Vars, [], _StaticPath, _Txn, _Chain, _Ledger, _, _, Acc) ->
    Acc;
poc_challengees_rewards_(
    #{poc_version := Version} = Vars,
    [Elem | Path],
    StaticPath,
    Txn,
    Chain,
    ...
    case blockchain_poc_path_element_v1:receipt(Elem) of
        undefined ->
            Acc1 =
                ...
                of
                    true when is_integer(Version), Version > 4, IsFirst == true ->
                        ...
                        case
                            poc_challengee_reward_unit(
                                WitnessRedundancy, DecayRate, HIP15TxRewardUnitCap, Witnesses
                            )
                        of
                            {error, _} ->
                                %% Old behavior
            ...
            poc_challengees_rewards_(
                Vars, Path, StaticPath, Txn, Chain, Ledger, false, VarMap, Acc1
            );
        Receipt ->
...
poc_challengees_rewards_(
    Vars, [Elem | Path], StaticPath, Txn, Chain, Ledger, _IsFirst, VarMap, Acc0
) ->
    case blockchain_poc_path_element_v1:receipt(Elem) of
        undefined ->
            poc_challengees_rewards_(
                Vars, Path, StaticPath, Txn, Chain, Ledger, false, VarMap, Acc0
            );
        _Receipt ->
            Challengee = blockchain_poc_path_element_v1:challengee(Elem),
            I = maps:get(Challengee, Acc0, 0),
...
-spec poc_challengee_reward_unit(
    WitnessRedundancy :: undefined | pos_integer(),
...
poc_challengee_reward_unit(WitnessRedundancy, DecayRate, HIP15TxRewardUnitCap, Witnesses) ->
    case {WitnessRedundancy, DecayRate} of
        {undefined, _} ->
            {error, witness_redundancy_undefined};
        {_, undefined} ->
            {error, poc_reward_decay_rate_undefined};
        {N, R} ->
            W = length(Witnesses),
...
-spec normalize_reward_unit(HIP15TxRewardUnitCap :: undefined | float(), Unit :: float()) ->
    float().
normalize_reward_unit(undefined, Unit) when Unit > 1.0 -> 1.0;
normalize_reward_unit(undefined, Unit) ->
    Unit;
normalize_reward_unit(HIP15TxRewardUnitCap, Unit) when Unit >= HIP15TxRewardUnitCap ->
    HIP15TxRewardUnitCap;
normalize_reward_unit(_TxRewardUnitCap, Unit) ->
    Unit.
    ...
    try
        ...
        lists:foldl(
            fun(Elem, Acc1) ->
                ...
                case
                    blockchain_txn_poc_receipts_v1:valid_witnesses(
                        Elem,
                        WitnessChannel,
                        Ledger
                    )
                    ...
                    ValidWitnesses ->
                        ...
                        case DensityTgtRes of
                            undefined ->
                                ...
                                lists:foldl(
                                    fun(WitnessRecord, Acc2) ->
                                        ...
                                        maps:put(
                                            Witness,
                                            {C + 1, I + (ToAdd * witness_decay(C, Vars))},
                                            Acc2
                                        )
                                    end,
                                    Acc1,
                            ...
                            D ->
                                ...
                                lists:foldl(
                                    fun(WitnessRecord, Acc2) ->
                                        Challengee = blockchain_poc_path_element_v1:challengee(
                                            Elem
                                        ),
                                        %% This must always be {ok, ...}
                                        %% Challengee must have a location
                                        {ok, ChallengeeLoc} =
                                            blockchain_ledger_v1:find_gateway_location(
                                                Challengee, Ledger
                                            ),
                                        Witness =
                                        ...
                                        maps:put(
                                            Witness,
                                            {C + 1, I + (Value * witness_decay(C, Vars))},
                                            Acc2
                                        )
                                    end,
                                    Acc1,
...
-spec normalize_witness_rewards(
    WitnessRewards :: rewards_share_map(),
    Vars :: reward_vars()
) -> rewards_map().
normalize_witness_rewards(
    WitnessRewards,
    #{
        epoch_reward := EpochReward,
        poc_witnesses_percent := PocWitnessesPercent
...
-spec collect_dc_rewards_from_previous_epoch_grace(
    ...
    Vars :: reward_vars(),
    Ledger :: blockchain_ledger_v1:ledger()
) -> {ok, dc_rewards_share_map()} | {error, any()}.
collect_dc_rewards_from_previous_epoch_grace(
    Start,
    End,
    Chain,
    #{
...
-spec scan_grace_block(
    ...
    Chain :: blockchain:blockchain(),
    Ledger :: blockchain_ledger_v1:ledger(),
    Acc :: dc_rewards_share_map()
) -> {ok, dc_rewards_share_map()} | {error, term()}.
scan_grace_block(Current, Start, _End, _Vars, _Chain, _Ledger, Acc) when
    Current == Start + 1
->
    {ok, Acc};
        ...
        true ->
            ...
            case lists:member(SCID, maps:get(seen, AccIn, [])) of
                false ->
                    ...
                    case
                        blockchain_ledger_v1:find_state_channel(
                    ...
                    of
                        {ok, SC} ->
                            ...
                            case blockchain_ledger_state_channel_v2:is_v2(SC) of
                                true ->
                                    ...
                                    Summaries =
                                        case RewardVersion > 3 of
                                            %% reward version 4 normalizes payouts
                                            true ->
                                                blockchain_state_channel_v1:summaries(
                                                    blockchain_state_channel_v1:normalize(FinalSC)
                                                );
                                            false ->
                                                blockchain_state_channel_v1:summaries(FinalSC)
                                        end,
                                    ...
                                    Bonus =
                                        case blockchain_ledger_state_channel_v2:close_state(SC) of
                                            ...
                                            dispute when RewardVersion < 4 ->
                                                ...
                                                %% participants
                                                OverCommit =
                                                    blockchain_ledger_state_channel_v2:amount(SC) -
                                                        blockchain_ledger_state_channel_v2:original(
                                                            SC
                                                        ),
                                    ...
                                    lists:foldl(
                                        fun(Summary, A) ->
                                            Key = blockchain_state_channel_summary_v1:client_pubkeybin(
                                                Summary
                                            ),
                                            DCs =
                        ...
                        {error, not_found} ->
                            ExpireAt = blockchain_txn_state_channel_close_v1:state_channel_expire_at(
                                Txn
                            ),
                            lager:warning("missing scid ~p", [SCID]),
                            lager:warning("expire ~p + grace ~p > end ~p?", [
                                ExpireAt, GraceBlocks, End
                            ]),
                            AccIn
                    end;
                true ->
...
-spec normalize_dc_rewards(
    DCRewards0 :: dc_rewards_share_map(),
    Vars :: reward_vars()
) -> {non_neg_integer(), rewards_map()}.
normalize_dc_rewards(
    DCRewards0,
    #{
        epoch_reward := EpochReward,
        dc_percent := DCPercent
    ...
    DCReward =
        case OraclePrice == 0 orelse RewardVersion < 3 of
            true ->
    ...
    case Version of
        V when is_integer(V), V >= 9 ->
            try
                ...
                WitnessChannel = lists:nth(ElemPos, Channels),
                ValidWitnesses = blockchain_txn_poc_receipts_v1:valid_witnesses(
                    Elem, WitnessChannel, Ledger
                ),
                ...
                throw:{error, {unknown_region, Region}}:_ST ->
                    ...
                    [];
                What:Why:ST ->
                    lager:error("failed to calculate poc_challengees_rewards, error ~p:~p:~p", [
                        What, Why, ST
                    ]),
                    []
            end;
...
witness_decay(Count, Vars) ->
    case maps:find(witness_reward_decay_rate, Vars) of
        ...
        {ok, DecayRate} ->
            Exclusion =
                case maps:find(witness_reward_decay_exclusion, Vars) of
                    {ok, undefined} -> 0;
...
v1_to_v2(RewardsV1) ->
    ...
    %% drop any 0 amount reward, as in v2
    lists:sort(
        maps:fold(
            fun
                (_O, 0, Acc) -> Acc;
                (O, A, Acc) -> [new_reward(O, A) | Acc]
            end,
            [],
            R
        )
...
poc_challengers_rewards_2_test() ->
    ...
    ChallengerShares = lists:foldl(
        fun(T, Acc) -> poc_challenger_reward(T, Acc, Vars) end, #{}, Txns
    ),
...
poc_witnesses_rewards_test() ->
    ...
    Elem = blockchain_poc_path_element_v1:new(<<"c">>, <<"Receipt not undefined">>, [
        Witness1, Witness2
    ]),
    Txns = [
    ...
    Rewards = #{
        {gateway, poc_witnesses, <<"a">>} => 25,
        {gateway, poc_witnesses, <<"b">>} => 25
    },

    WitnessShares = lists:foldl(
        fun(T, Acc) -> poc_witness_reward(T, Acc, Chain, Ledger, EpochVars) end,
        #{},
        Txns
    ),
...
dc_rewards_v3_test() ->
    ...
    LedgerVars = maps:merge(
        #{?poc_version => 5, ?sc_version => 2, ?sc_grace_blocks => 5}, common_poc_vars()
    ),
    ...
    SC = blockchain_state_channel_v1:summaries(
        [
            ...
            blockchain_state_channel_summary_v1:new(<<"b">>, 2, 2)
        ],
        SC0
    ),
    ...
    ok = blockchain_ledger_v1:close_state_channel(
        <<"owner">>, <<"owner">>, SC, <<"id">>, false, Ledger1
    ),

    {ok, _} = blockchain_ledger_v1:find_state_channel(<<"id">>, <<"owner">>, Ledger1),

    SCClose = blockchain_txn_state_channel_close_v1:new(SC, <<"owner">>),
    %% 3 DCs burned at HNT price of 1 dollar
    {ok, DCsInEpochAsHNT} = blockchain_ledger_v1:dc_to_hnt(3, 100000000),
...
dc_rewards_v3_spillover_test() ->
    ...
    ChallengersAward = trunc(
        maps:get(epoch_reward, Vars) * maps:get(poc_challengers_percent, Vars)
    ),
    %% entire 15% allocation
    ?assertEqual(#{{gateway, poc_challengers, <<"X">>} => ChallengersAward}, ChallengerRewards),
    ChallengeesAward = trunc(
        maps:get(epoch_reward, Vars) * maps:get(poc_challengees_percent, Vars)
    ),
    %% 4 of 8 shares of 20% allocation
    ?assertEqual(
        #{
            {gateway, poc_challengees, <<"a">>} => trunc(ChallengeesAward * 4 / 8),
            %% 3 shares
            {gateway, poc_challengees, <<"b">>} => trunc(ChallengeesAward * 3 / 8),
            %% 1 share
            {gateway, poc_challengees, <<"c">>} => trunc(ChallengeesAward * 1 / 8)
        },
        ChallengeeRewards
    ),
    ...
    %% entire 15% allocation
    ?assertEqual(
        #{{gateway, poc_witnesses, <<"c">>} => WitnessesAward},
        WitnessRewards
    ),
    ...
    ChallengerSpilloverAward = erlang:round(
        DCRemainder *
            (maps:get(poc_challengers_percent, Vars) /
                (maps:get(poc_challengees_percent, Vars) +
                    maps:get(poc_witnesses_percent, Vars) +
    ...
    %% entire 15% allocation
    ?assertEqual(
        #{{gateway, poc_challengers, <<"X">>} => ChallengersAward + ChallengerSpilloverAward},
        SpilloverChallengerRewards
    ),
    ChallengeeSpilloverAward = erlang:round(
        DCRemainder *
            (maps:get(poc_challengees_percent, Vars) /
                (maps:get(poc_challengees_percent, Vars) +
                    maps:get(poc_witnesses_percent, Vars) +
    ...
    ?assertEqual(
        #{
            {gateway, poc_challengees, <<"a">>} => trunc(
                (ChallengeesAward + ChallengeeSpilloverAward) * 4 / 8
            ),
            %% 3 shares
            {gateway, poc_challengees, <<"b">>} => trunc(
                (ChallengeesAward + ChallengeeSpilloverAward) * 3 / 8
            ),
            %% 1 share
            {gateway, poc_challengees, <<"c">>} => trunc(
                (ChallengeesAward + ChallengeeSpilloverAward) * 1 / 8
            )
        },
        SpilloverChallengeeRewards
    ),
    WitnessesSpilloverAward = erlang:round(
        DCRemainder *
            (maps:get(poc_witnesses_percent, Vars) /
                (maps:get(poc_challengees_percent, Vars) +
                    maps:get(poc_witnesses_percent, Vars) +
    ...
    %% entire 15% allocation
    ?assertEqual(
        #{{gateway, poc_witnesses, <<"c">>} => WitnessesAward + WitnessesSpilloverAward},
...
fixed_normalize_reward_unit_test() ->
    ...
    %% 0.1 -> 0.1
    ?assertEqual(0.1, maps:get(0.1, Correct)),
    %% 0.9 -> 0.9
    ?assertEqual(0.9, maps:get(0.9, Correct)),
    %% 2.5 -> 2.0
    ...
    %% 0.1 -> 0.1
    ?assertEqual(0.1, maps:get(0.1, Incorrect)),
    %% 0.9 -> 0.9
    ?assertEqual(0.9, maps:get(0.9, Incorrect)),
