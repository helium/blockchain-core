...
-spec fee_payer(txn_assert_location(), blockchain_ledger_v1:ledger()) ->
...
-spec calculate_fee(
    txn_assert_location(), blockchain_ledger_v1:ledger(), pos_integer(), pos_integer(), boolean()
) -> non_neg_integer().
...
calculate_fee(Txn, Ledger, DCPayloadSize, TxnFeeMultiplier, true) ->
    case Txn#blockchain_txn_assert_location_v2_pb.payer of
        Payer when Payer == undefined; Payer == <<>> ->
            ...
            ?calculate_fee(
                Txn#blockchain_txn_assert_location_v2_pb{
                    fee = 0,
                    staking_fee = 0,
...
sign_payer(Txn, SigFun) ->
    BaseTxn = Txn#blockchain_txn_assert_location_v2_pb{
        owner_signature = <<>>,
        payer_signature = <<>>
    },
    EncodedTxn = blockchain_txn_assert_location_v2_pb:encode_msg(BaseTxn),
...
is_valid_payer(#blockchain_txn_assert_location_v2_pb{payer = undefined}) ->
    %% no payer
    true;
is_valid_payer(#blockchain_txn_assert_location_v2_pb{
    payer = <<>>,
    payer_signature = <<>>
}) ->
                    ...
                    case blockchain:config(?max_antenna_gain, Ledger) of
                        {ok, MaxGain} ->
                            case is_valid_gain(Txn, MinGain, MaxGain) of
                                false ->
                                    {error,
                                        {invalid_assert_loc_txn_v2,
                                            {invalid_antenna_gain, Gain, MinGain, MaxGain}}};
                                true ->
                                    Res = blockchain_txn:validate_fields([
                                    ...
                                    case Res of
                                        {error, _} = E -> E;
                                        ok -> do_is_valid_checks(Txn, Chain)
                                    end
                            end;
                        _ ->
...
do_is_valid_checks(Txn, Chain) ->
    Ledger = blockchain:ledger(Chain),
    case {?MODULE:is_valid_owner(Txn), ?MODULE:is_valid_payer(Txn)} of
        {false, _} ->
        ...
        {true, true} ->
            ...
            of
                ...
                {_, false} ->
                    {error,
                        {wrong_staking_fee, {assert_location_v2, ExpectedStakingFee, StakingFee}}};
                {true, true} ->
                    do_remaining_checks(Txn, TxnFee + StakingFee, Ledger)
            end
    end.

-spec is_new_location(Txn :: txn_assert_location(), Ledger :: blockchain_ledger_v1:ledger()) ->
    boolean().
...
do_remaining_checks(Txn, TotalFee, Ledger) ->
    ...
    case
        blockchain_ledger_v1:check_dc_or_hnt_balance(ActualPayer, TotalFee, Ledger, AreFeesEnabled)
    of
        {error, _} = Error ->
            Error;
        ok ->
                    ...
                    case Owner == GwOwner of
                        ...
                        true ->
                            ...
                            case ?MODULE:is_valid_location(Txn, MinAssertH3Res) of
                                false ->
                                    {error,
                                        {insufficient_assert_res,
                                            {assert_location_v2, Location, Gateway}}};
                                true ->
                                    ...
                                    case Nonce =:= LedgerNonce + 1 of
                                        false ->
                                            {error,
                                                {bad_nonce,
                                                    {assert_location_v2, Nonce, LedgerNonce}}};
                                        true ->
                                            ok
                                    end
                            end
...
%% @end
%%--------------------------------------------------------------------
-spec absorb(txn_assert_location(), blockchain:blockchain()) ->
...
absorb(Txn, Chain) ->
    ...
    case
        blockchain_ledger_v1:debit_fee(
            ActualPayer, Fee + StakingFee, Ledger, AreFeesEnabled, TxnHash, Chain
        )
    of
        {error, _Reason} = Error ->
            Error;
...
-spec maybe_alter_hex(
    OldGw :: blockchain_ledger_gateway_v2:gateway(),
    Gateway :: libp2p_crypto:pubkey_bin(),
    Location :: h3:index(),
    Ledger :: blockchain_ledger_v1:ledger()
) -> ok.
...
print(undefined) ->
    <<"type=assert_location, undefined">>;
print(#blockchain_txn_assert_location_v2_pb{
    ...
    io_lib:format(
        "type=assert_location, gateway=~p, owner=~p, payer=~p, location=~p, owner_signature=~p, payer_signature=~p, nonce=~p, gain=~p, elevation=~p, staking_fee=~p, fee=~p",
        [
...
-spec staking_fee_for_gw_mode(blockchain_ledger_gateway_v2:mode(), blockchain_ledger_v1:ledger()) ->
    non_neg_integer().
...
sign_test() ->
    ...
    ?assert(
        libp2p_crypto:verify(
            blockchain_txn_assert_location_v2_pb:encode_msg(BaseTxn1), Sig1, PubKey
        )
    ).
...
sign_payer_test() ->
    ...
    BaseTx1 = Tx2#blockchain_txn_assert_location_v2_pb{
        owner_signature = <<>>, payer_signature = <<>>
    },
    ?assert(
        libp2p_crypto:verify(blockchain_txn_assert_location_v2_pb:encode_msg(BaseTx1), Sig2, PubKey)
    ).
