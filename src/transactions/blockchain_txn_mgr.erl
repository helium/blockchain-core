...
-type txn_key() :: term().
-type cached_txn_type() :: {
    TxnKey :: txn_key(), Txn :: blockchain_txn:txn(), TxnData :: #txn_data{}
}.
-type dialers() :: [dialer()].
...
handle_call(
    ...
    IsNewElection = false,
    NewCGMembers = [],
    Result = process_cached_txns(
        Chain, CurBlockHeight, SubmitF, HasBeenSynced, IsNewElection, NewCGMembers
    ),
    {reply, Result, State};
            ...
            case blockchain_txn:hash(Txn) == Hash of
                true ->
                    throw(
                        {reply,
                            {ok, #{
                                received_at => TxnData#txn_data.recv_block_height,
                                accepted_by => TxnData#txn_data.acceptions,
                                rejected_by => TxnData#txn_data.rejections,
                                dialers => TxnData#txn_data.dialers
                            }},
                            State}
                    );
                false ->
...
handle_info({blockchain_event, {new_chain, NC}}, State) ->
    ...
    {noreply, NewState};
handle_info(
    {blockchain_event, {add_block, _BlockHash, _Sync, _Ledger} = Event},
    State0 = #state{chain = undefined}
) ->
    lager:info(
        "received add block event whilst no chain and sync ~p.  Initializing chain and then handling block",
        [_Sync]
    ),
...
terminate(_Reason, _State) ->
    ...
    [
        blockchain_txn_mgr_sup:stop_dialers(TxnData#txn_data.dialers)
     || {_TxnKey, _Txn, TxnData} <- cached_txns()
    ],
    ok.
...
initialize_with_chain(State, Chain) ->
    ...
    {ok, N} = blockchain:config(?num_consensus_members, Ledger),
    State#state{
        chain = Chain, cur_block_height = Height, submit_f = submit_f(N), reject_f = reject_f(N)
    }.

-spec handle_add_block_event(
    {atom(), blockchain_block:hash(), boolean(), blockchain_ledger_v1:ledger()}, #state{}
) -> {noreply, #state{}}.
handle_add_block_event(
    {add_block, BlockHash, Sync, _Ledger},
    State = #state{
        chain = Chain,
        cur_block_height = CurBlockHeight
    }
    ...
    case blockchain:get_block(BlockHash, Chain) of
        {ok, Block} ->
            ...
            HasBeenSynced =
                (Sync == false orelse BlockAge < ?RECENT_BLOCK_AGE) orelse
                    State#state.has_been_synced,
            ...
            ok = process_cached_txns(
                Chain, BlockHeight, SubmitF, HasBeenSynced == false, IsNewElection, NewCGMembers
            ),
            ...
            lager:debug("received block height: ~p,  updated state block height: ~p", [
                BlockHeight, NewCurBlockHeight
            ]),
            State1 = State#state{
                cur_block_height = NewCurBlockHeight, has_been_synced = HasBeenSynced
            },
...
process_deferred_rejections(
    ...
    % Sanity check
    {[], Deferred1} = lists:partition(IsPast, Deferred1),
    lager:debug(
...
-spec check_block_for_new_election(blockchain_block:block()) ->
    {boolean(), [libp2p_crypto:pubkey_bin()]}.
...
-spec maybe_update_block_height(undefined | integer(), integer(), boolean()) ->
    undefined | integer().
...
signatory_rand_members(Chain, SubmitF, Acceptions, Rejections, Dialers) ->
    ...
    Signatories = [
        Signer
     || {Signer, _} <- blockchain_block:signatures(PrevBlock),
    ...
    case Signatories of
        [] ->
            ...
            Members =
                ((Members0 -- [blockchain_swarm:pubkey_bin()] -- Acceptions) -- Rejections) --
                    MembersBeingDialed,
        ...
        _ ->
            %% we have signatories
            RandomSignatories =
                ((blockchain_utils:shuffle(Signatories) -- Acceptions) -- Rejections) --
                    MembersBeingDialed,
...
process_cached_txns(
    _Chain, _CurBlockHeight, _SubmitF, true = _Sync, _IsNewElection, _NewGroupMember
) ->
    ok;
process_cached_txns(Chain, CurBlockHeight, SubmitF, _Sync, IsNewElection, NewGroupMembers) ->
    ...
    ok = lists:foreach(
        fun({TxnKey, Txn, #txn_data{recv_block_height = RecvBlockHeight} = _TxnData} = CachedTxn) ->
            case
                {lists:keyfind(Txn, 1, InvalidTransactions), lists:member(Txn, ValidTransactions)}
            of
                {false, false} ->
                    ...
                    case (CurBlockHeight - RecvBlockHeight) > TxnMaxBlockSpan of
                        true ->
                            lager:debug("txn has exceeded max block space, rejecting: ~p", [
                                blockchain_txn:hash(Txn)
                            ]),
                            process_invalid_txn(CachedTxn, {error, {invalid, undecided_timeout}});
                        _ ->
                            lager:debug("txn has undecided validations, leaving in cache: ~p", [
                                blockchain_txn:hash(Txn)
                            ]),
                            ok
                    end;
...
-spec process_invalid_txn(
    cached_txn_type(), {error, {invalid, atom()}} | {error, {invalid, {any}}}
) -> ok.
process_invalid_txn({TxnKey, Txn, TxnData}, CallbackResponse) ->
    ...
    lager:info(
        "txn declared invalid with reason ~p, removing from cache and invoking callback: ~p", [
            CallbackResponse, blockchain_txn:hash(Txn)
        ]
    ),
...
process_valid_txn(
    ...
    lager:debug(
        "txn with key ~p is valid and there is a new election: ~p.  Checking if it needs to be resubmitted",
        [TxnKey, blockchain_txn:hash(Txn)]
    ),
    #txn_data{
    ...
    {RemainingDialers, StaleDialers} = lists:partition(
        fun({_DialerPid, DialerMember}) ->
    ...
    check_for_deps_and_resubmit(
        ...
        SubmitF,
        TxnData#txn_data{
            recv_block_height = RecvBlockHeight0,
...
process_valid_txn(
    ...
    lager:debug(
        "txn with key ~p is valid and there is NO new election: ~p.  Checking if it needs to be resubmitted",
        [TxnKey, blockchain_txn:hash(Txn)]
    ),
    #txn_data{
        acceptions = Acceptions,
    ...
    case length(Dialers) < (SubmitF - length(Acceptions)) of
        ...
        false ->
            ...
            lager:debug(
                "txn is valid but no need to resubmit to new or additional members: ~p Accepted: ~p Rejected ~p Dialers ~p F ~p",
                [blockchain_txn:hash(Txn), length(Acceptions), length(Rejections), Dialers, SubmitF]
            ),
            ok
    end.

-spec check_for_deps_and_resubmit(
    txn_key(),
    blockchain_txn:txn(),
    [blockchain_txn:txn()],
    blockchain:blockchain(),
...
check_for_deps_and_resubmit(
    ...
    case
        lists:filter(
            fun({DepTxnKey, _DepTxn, _DepTxnData}) ->
                cached_txn(DepTxnKey) /= {error, txn_not_found}
            end,
            blockchain_txn:depends_on(Txn, CachedTxns)
        )
    of
        [] ->
            ...
            NewDialers = submit_txn_to_cg(
                Chain, TxnKey, Txn, SubmitF, Acceptions, Rejections, Dialers
            ),
            lager:info("Resubmitting txn: ~p to ~b new dialers", [
                blockchain_txn:hash(Txn), length(NewDialers)
            ]),
        ...
        Dependencies ->
            ...
            {ok,
                {_, _,
                    #txn_data{acceptions = Dep1TxnAcceptions, dialers = Dep1TxnDialers} =
                        _Dep1TxnData1}} = cached_txn(Dep1TxnKey),
            A0 =
            ...
            ElegibleMembers = sets:to_list(
                lists:foldl(
                    fun({Dep2TxnKey, _Dep2Txn, _Dep2TxnData}, Acc) ->
                        {ok,
                            {_, _,
                                #txn_data{acceptions = Dep2TxnAcceptions, dialers = Dep2TxnDialers} =
                                    _Dep2TxnData1}} = cached_txn(Dep2TxnKey),
                        A1 =
                            case Dep2TxnAcceptions of
                                [] ->
                                    [
                                        Dep2TxnDialedMember
                                     || {_, Dep2TxnDialedMember} <- Dep2TxnDialers
                                    ];
                                Dep2TxnAccs ->
                                    Dep2TxnAccs
                            end,
            ...
            lager:debug("txn ~p has eligible members: ~p", [
                blockchain_txn:hash(Txn), ElegibleMembers
            ]),
            ...
            NewDialers = dial_members(
                lists:sublist(ElegibleMembers1, MaxNewDiallersCount), Chain, TxnKey, Txn
            ),
            lager:debug("txn ~p depends on ~p other txns, can dial ~p members and dialed ~p", [
                blockchain_txn:hash(Txn),
                length(Dependencies),
                length(ElegibleMembers1),
                length(NewDialers)
            ]),
            cache_txn(TxnKey, Txn, TxnData#txn_data{dialers = Dialers ++ NewDialers})
    end.

-spec purge_old_cg_members(
    [libp2p_crypto:pubkey_bin()],
    [libp2p_crypto:pubkey_bin()],
    [libp2p_crypto:pubkey_bin()]
            ...
            case lists:keymember(Dialer, 1, Dialers) of
                ...
                true ->
                    ...
                    cache_txn(TxnKey, Txn, TxnData#txn_data{
                        acceptions = lists:usort([Member | Acceptions]),
        ...
        {ok, {TxnKey, Txn, #txn_data{rejections = Rejections, dialers = Dialers} = TxnData}} ->
            case lists:keymember(Dialer, 1, Dialers) of
                ...
                true ->
                    ...
                    NewTxnData = TxnData#txn_data{
                        rejections = lists:usort([Member | Rejections]),
                        dialers = lists:keydelete(Dialer, 1, Dialers)
                    },
...
reject_actions(
    {TxnKey, _Txn, #txn_data{callback = Callback, dialers = Dialers, rejections = _Rejections}},
    RejectF,
    _CurBlockHeight
) when
    length(_Rejections) > RejectF
->
...
-spec dial_members(
    [libp2p_crypto:pubkey_bin()], blockchain:blockchain(), txn_key(), blockchain_txn:txn()
) -> dialers().
dial_members(Members, Chain, TxnKey, Txn) ->
    dial_members(Members, Chain, TxnKey, Txn, []).
