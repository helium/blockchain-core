...
hash(Txn) ->
    BaseTxn = Txn#blockchain_txn_add_gateway_v1_pb{
...
-spec fee_payer(txn_add_gateway(), blockchain_ledger_v1:ledger()) ->
...
-spec calculate_fee(
    txn_add_gateway(), blockchain_ledger_v1:ledger(), pos_integer(), pos_integer(), boolean()
) -> non_neg_integer().
...
calculate_fee(Txn, Ledger, DCPayloadSize, TxnFeeMultiplier, true) ->
    case Txn#blockchain_txn_add_gateway_v1_pb.payer of
        Payer when Payer == undefined; Payer == <<>> ->
            ...
            ?calculate_fee(
                Txn#blockchain_txn_add_gateway_v1_pb{
                    fee = 0,
                    staking_fee = 0,
...
is_valid_staking_key(#blockchain_txn_add_gateway_v1_pb{payer = Payer} = _Txn, Ledger) ->
    case blockchain_ledger_v1:staking_keys(Ledger) of
        not_found ->
            %% chain var not active, so default to true
            true;
        Keys ->
...
%% @end
%%--------------------------------------------------------------------
-spec absorb(txn_add_gateway(), blockchain:blockchain()) ->
    ok | {error, atom()} | {error, {atom(), any()}}.
absorb(Txn, Chain) ->
    Ledger = blockchain:ledger(Chain),
    AreFeesEnabled = blockchain_ledger_v1:txn_fees_active(Ledger),
...
-spec gateway_mode(blockchain_ledger_v1:ledger(), libp2p_crypto:pubkey_bin()) ->
...
print(#blockchain_txn_add_gateway_v1_pb{
    ...
    io_lib:format(
        "type=add_gateway, owner=~p, gateway=~p, payer=~p, staking_fee=~p, fee=~p",
...
sign_request_test() ->
    ...
    BaseTx1 = Tx1#blockchain_txn_add_gateway_v1_pb{
        gateway_signature = <<>>, owner_signature = <<>>, payer_signature = <<>>
    },
    ?assert(
        libp2p_crypto:verify(blockchain_txn_add_gateway_v1_pb:encode_msg(BaseTx1), Sig1, PubKey)
    ).
...
sign_test() ->
    ...
    ?assert(
        libp2p_crypto:verify(blockchain_txn_add_gateway_v1_pb:encode_msg(BaseTx1), Sig2, PubKey)
    ).
...
sign_payer_test() ->
    ...
    BaseTx1 = Tx3#blockchain_txn_add_gateway_v1_pb{
        gateway_signature = <<>>, owner_signature = <<>>, payer_signature = <<>>
    },
    ?assert(
        libp2p_crypto:verify(blockchain_txn_add_gateway_v1_pb:encode_msg(BaseTx1), Sig2, PubKey)
