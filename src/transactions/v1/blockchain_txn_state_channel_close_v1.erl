...
-spec new(blockchain_state_channel_v1:state_channel(), libp2p_crypto:pubkey_bin()) ->
    txn_state_channel_close().
new(SC, Closer) ->
...
-spec conflicts_with(txn_state_channel_close()) ->
...
-spec fee_payer(txn_state_channel_close(), blockchain_ledger_v1:ledger()) ->
...
sign(Txn, SigFun) ->
    EncodedTxn = blockchain_txn_state_channel_close_v1_pb:encode_msg(
        Txn#blockchain_txn_state_channel_close_v1_pb{signature = <<>>}
    ),
...
calculate_fee(_Txn, _Ledger, _DCPayloadSize, _TxnFeeMultiplier, true) ->
    %% for now we are defaulting close fees to 0
    0.

-spec is_valid(txn_state_channel_close(), blockchain:blockchain()) ->
    ...
    case
        SCVersion == 0 orelse
            (LedgerHeight >= ExpiresAt andalso
                LedgerHeight =< ExpiresAt + SCGrace)
    of
        false ->
                    ...
                    case
                        {
                            ...
                            blockchain_state_channel_v1:validate(SC)
                        }
                    of
                        {false, _} ->
                            ...
                            case blockchain_ledger_v1:find_state_channel(ID, Owner, Ledger) of
                                ...
                                {ok, LedgerSC} ->
                                    case Owner == Closer of
                                        ...
                                        true ->
                                            case
                                                blockchain_ledger_state_channel_v2:is_v2(LedgerSC)
                                            of
                                                false ->
                                                    ok;
                                                true ->
                                                    LSC = blockchain_ledger_state_channel_v2:state_channel(
                                                        LedgerSC
                                                    ),
                                                    CloseState = blockchain_ledger_state_channel_v2:close_state(
                                                        LedgerSC
                                                    ),
                                                    lager:info("close state was ~p", [CloseState]),
                                                    %% check this new SC is newer than the current one, if any
                                                    case
                                                        LSC == undefined orelse
                                                            (CloseState /= dispute andalso
                                                                blockchain_state_channel_v1:compare_causality(
                                                                    LSC, SC
                                                                ) == caused)
                                                    of
                                                        true ->
                                        ...
                                        false ->
                                            case
                                                blockchain_state_channel_v1:get_summary(Closer, SC)
                                            of
                                                {error, _Reason} = E ->
                                                    E;
                                                {ok, _Summary} ->
                                                    ...
                                                    of
                                                        ok ->
                                                            ...
                                                            AreFeesEnabled = blockchain_ledger_v1:txn_fees_active(
                                                                Ledger
                                                            ),
...
check_close_updates(LedgerSC, Txn, Ledger) ->
    ...
    case blockchain_ledger_state_channel_v2:is_v2(LedgerSC) of
        ...
        true ->
            ...
            case
                LSC /= undefined andalso
                    ((blockchain_state_channel_v1:compare_causality(LSC, SC) == caused andalso
                        ConflictingSC == undefined) orelse
                        (CloseState /= dispute andalso
            ...
            of
                ...
                false ->
                    ...
                    {InternalConflict, ValidConflict, MergedSC} =
                        case ?MODULE:conflicts_with(Txn) of
                            undefined ->
                            ...
                            ConflictingSC ->
                                ...
                                case
                                    blockchain_state_channel_v1:id(SC) ==
                                        blockchain_state_channel_v1:id(ConflictingSC) andalso
                                        (not is_causally_correct(SC, ConflictingSC, Ledger))
                                of
                                    false ->
                                        {false, {error, sc_mismatch}, SC};
                                    true ->
                                        Merged = blockchain_state_channel_v1:merge(
                                            SC, ConflictingSC, MaxActorsAllowed
                                        ),
                                        {true, blockchain_state_channel_v1:validate(ConflictingSC),
                                            Merged}
                                end
                        end,
                        ...
                        ok ->
                            case LSC of
                                ...
                                _ ->
                                    ...
                                    %%
                                    case
                                        (InternalConflict == true orelse ExternalConflict) andalso
                                            blockchain_ledger_state_channel_v2:close_state(LedgerSC) /=
                                                dispute
                                    of
                                        true ->
                                        ...
                                        false ->
                                            ...
                                            case
                                                blockchain_state_channel_v1:compare_causality(
                                                    LSC,
                                                    blockchain_state_channel_v1:merge(
                                                        LSC, MergedSC, MaxActorsAllowed
                                                    )
                                                )
                                            of
                                                equal ->
...
-spec is_causally_correct(
    OlderSC :: blockchain_state_channel_v1:state_channel(),
...
-spec absorb(txn_state_channel_close(), blockchain:blockchain()) ->
    ok | {error, atom()} | {error, {atom(), any()}}.
absorb(Txn, Chain) ->
    Ledger = blockchain:ledger(Chain),
    ...
    case blockchain_ledger_v1:debit_fee(Closer, TxnFee, Ledger, AreFeesEnabled, TxnHash, Chain) of
        ...
        ok ->
            MaxActorsAllowed = blockchain_state_channel_v1:max_actors_allowed(Ledger),
            {MergedSC, HadConflict} =
                case ?MODULE:conflicts_with(Txn) of
                    undefined ->
                        {SC, false};
                    ConflictingSC ->
                        {
                            blockchain_state_channel_v1:merge(SC, ConflictingSC, MaxActorsAllowed),
                            true
                        }
                end,
            lager:info("Closing with conflict ~p", [HadConflict]),
            blockchain_ledger_v1:close_state_channel(
                Owner, Closer, MergedSC, ID, HadConflict, Ledger
            )
    end.

-spec print(txn_state_channel_close()) -> iodata().
print(undefined) ->
    <<"type=state_channel_close, undefined">>;
...
to_json(Txn, _Opts) ->
    #{
        ...
        conflicts_with =>
            case conflicts_with(Txn) of
                undefined -> undefined;
                SC -> blockchain_state_channel_v1:to_json(SC, [])
            end
    }.
...
sign_test() ->
    ...
    Tx1 = sign(Tx0, SigFun),
    Sig1 = signature(Tx1),
    EncodedTx1 = blockchain_txn_state_channel_close_v1_pb:encode_msg(
        Tx1#blockchain_txn_state_channel_close_v1_pb{signature = <<>>}
    ),
