                                    ...
                                    {false, _Distance} ->
                                         try h3:grid_distance(SourceParentIndex, DestinationParentIndex) of
                                             Dist when Dist >= ExclusionCells ->
                                                 RSSI = blockchain_poc_witness_v1:signal(Witness),
                                                 SNR = blockchain_poc_witness_v1:snr(Witness),
                                                 WitFreq = blockchain_poc_witness_v1:frequency(Witness),
                                                 MinRcvSig = min_rcv_sig(undefined, Ledger, SourceLoc, DstPubkeyBin, DestinationLoc, WitFreq),
                                                     ...
                                                     false ->
                                                         ...
                                                         Desc = lists:flatten(
                                                                  io_lib:format(
                                                                    ...
                                                                    [SrcName, WitName, RejectHt, RSSI, MinRcvSig, SNR, ReceiptFreq, WitFreq])),
                                                         [{false, list_to_binary(Desc), Witness} | Acc];
                                                     true ->
                                                         case check_valid_frequency(SourceLoc, WitFreq, Ledger) of
                                                             true ->
                                                                case blockchain:config(?data_aggregation_version, Ledger) of
                                                                    {ok, DataAggVsn} when DataAggVsn > 1 ->
                                                                        case check_rssi_snr(Ledger, RSSI, SNR) of
                                                                            ...
                                                                            {false, LowerBound} ->
                                                                                ...
                                                                                Desc = lists:flatten(
                                                                                         io_lib:format(
                                                                                           ...
                                                                                           [SrcName, WitName, RejectHt, RSSI, LowerBound, SNR, ReceiptFreq, WitFreq])),
                                                                                [{false, list_to_binary(Desc), Witness} | Acc]
                                                                        end;
                                                                    _ ->
                                                                        %% SNR+Freq+Channels not collected, nothing else we can check
