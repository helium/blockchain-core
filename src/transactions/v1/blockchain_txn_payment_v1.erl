...
-spec fee_payer(txn_payment(), blockchain_ledger_v1:ledger()) ->
...
-spec calculate_fee(
    txn_payment(), blockchain_ledger_v1:ledger(), pos_integer(), pos_integer(), boolean()
) -> non_neg_integer().
...
calculate_fee(Txn, Ledger, DCPayloadSize, TxnFeeMultiplier, true) ->
    ?calculate_fee(
        Txn#blockchain_txn_payment_v1_pb{fee = 0, signature = <<0:512>>},
        Ledger,
        DCPayloadSize,
        TxnFeeMultiplier
    ).

-spec is_valid(txn_payment(), blockchain:blockchain()) ->
                                                ...
                                                false ->
                                                    ...
                                                    case AmountCheck of
                                                        ...
                                                        true ->
                                                            ...
                                                            ExpectedTxnFee = ?MODULE:calculate_fee(
                                                                Txn, Chain
                                                            ),
                                                            case
...
-spec absorb(txn_payment(), blockchain:blockchain()) ->
    ok | {error, atom()} | {error, {atom(), any()}}.
absorb(Txn, Chain) ->
    Ledger = blockchain:ledger(Chain),
    Amount = ?MODULE:amount(Txn),
...
print(#blockchain_txn_payment_v1_pb{
    payer = Payer,
    payee = Recipient,
    amount = Amount,
    fee = Fee,
    ...
    io_lib:format(
        "type=payment, payer=~p, payee=~p, amount=~p, fee=~p, nonce=~p, signature=~s",
...
sign_test() ->
    ...
    Tx1 = sign(Tx0, SigFun),
    Sig1 = signature(Tx1),
    EncodedTx1 = blockchain_txn_payment_v1_pb:encode_msg(Tx1#blockchain_txn_payment_v1_pb{
        signature = <<>>
    }),
...
is_valid_with_extended_validation_test() ->
    {timeout, 30000, fun() ->
        ...
        Tx1 = sign(
            new(Payer, libp2p_crypto:b58_to_bin("1BR9RgYoP5psbcw9aKh1cDskLaGMBmkb8"), 666, 1),
            SigFun
        ),
