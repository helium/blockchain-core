...
-spec fee_payer(txn_create_htlc(), blockchain_ledger_v1:ledger()) ->
...
-spec calculate_fee(
    txn_create_htlc(), blockchain_ledger_v1:ledger(), pos_integer(), pos_integer(), boolean()
) -> non_neg_integer().
...
calculate_fee(Txn, Ledger, DCPayloadSize, TxnFeeMultiplier, true) ->
    ?calculate_fee(
        Txn#blockchain_txn_create_htlc_v1_pb{fee = 0, signature = <<0:512>>},
        Ledger,
        DCPayloadSize,
        TxnFeeMultiplier
    ).
...
%% @end
%%--------------------------------------------------------------------
-spec is_valid(txn_create_htlc(), blockchain:blockchain()) ->
                            ...
                            case blockchain_ledger_v1:find_entry(Payer, Ledger) of
                                ...
                                {ok, Entry} ->
                                    ...
                                    case TxnNonce =:= LedgerNonce + 1 of
                                        false ->
                                            {error,
                                                {bad_nonce, {create_htlc, TxnNonce, LedgerNonce}}};
                                        true ->
                                            Amount = ?MODULE:amount(Txn),
                                            TxnFee = ?MODULE:fee(Txn),
                                            AmountCheck =
                                                case
                                                    blockchain:config(?allow_zero_amount, Ledger)
                                                of
                                                    {ok, false} ->
                                            ...
                                            case AmountCheck of
                                                false ->
                                                    lager:error(
                                                        "amount < 0 for CreateHTLCTxn: ~p", [Txn]
                                                    ),
                                                ...
                                                true ->
                                                    ...
                                                    case
                                                        (ExpectedTxnFee =< TxnFee orelse
                                                            not AreFeesEnabled)
                                                    of
                                                        false ->
                                                            {error,
                                                                {wrong_txn_fee,
                                                                    {ExpectedTxnFee, TxnFee}}};
                                                        true ->
...
%% @end
%%--------------------------------------------------------------------
-spec absorb(txn_create_htlc(), blockchain:blockchain()) ->
    ok | {error, atom()} | {error, {atom(), any()}}.
absorb(Txn, Chain) ->
    Ledger = blockchain:ledger(Chain),
    Amount = ?MODULE:amount(Txn),
        ...
        {ok, _Entry} ->
            ...
            case
                blockchain_ledger_v1:debit_fee(
                    Payer, TxnFee, Ledger, AreFeesEnabled, TxnHash, Chain
                )
            of
                {error, _Reason} = Error ->
                    Error;
...
print(#blockchain_txn_create_htlc_v1_pb{
    ...
    io_lib:format(
        "type=create_htlc payer=~p payee=~p, address=~p, hashlock=~p, timelock=~p, amount=~p, fee=~p, signature=~p",
...
hashlock_test() ->
    ...
    ?assertEqual(
        <<"c3ab8ff13720e8ad9047dd39466b3c8974e592c2fa383d4a3960714caef0c4f2">>, hashlock(Tx)
    ).
...
sign_test() ->
    ...
    Tx1 = sign(Tx0, SigFun),
    Sig1 = signature(Tx1),
    EncodedTx1 = blockchain_txn_create_htlc_v1_pb:encode_msg(Tx1#blockchain_txn_create_htlc_v1_pb{
        signature = <<>>
    }),
...
is_valid_with_extended_validation_test() ->
    {timeout, 40, fun() ->
        ...
        Tx = sign(
            new(Payer, <<"payee">>, <<"address">>, crypto:strong_rand_bytes(32), 0, 666, 1), SigFun
        ),
        ...
        Tx1 = sign(
            new(
                Payer,
                libp2p_crypto:b58_to_bin("1BR9RgYoP5psbcw9aKh1cDskLaGMBmkb8"),
                <<"address">>,
        ...
        Tx2 = sign(
            new(Payer, ValidPayee, ValidPayee, crypto:strong_rand_bytes(32), 0, 666, 1), SigFun
        ),
        %% This check can be improved but whatever (it fails on fee)
        ?assertNotEqual({error, {invalid_address, payee}}, is_valid(Tx2, Chain)),

        Tx3 = sign(
            new(Payer, ValidPayee, <<"address">>, crypto:strong_rand_bytes(32), 0, 666, 1), SigFun
        ),
