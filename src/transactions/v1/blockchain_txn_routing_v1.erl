...
-export([
    ...
    calculate_staking_fee/2, calculate_staking_fee/5,
    nonce/1,
    signature/1,
    sign/2,
    is_valid/2,
...
-type action() ::
    {update_routers, RouterAddresses :: [binary()]}
...
-spec update_router_addresses(
    non_neg_integer(), libp2p_crypto:pubkey_bin(), [binary()], non_neg_integer()
) -> txn_routing().
...
-spec new_xor(non_neg_integer(), libp2p_crypto:pubkey_bin(), binary(), non_neg_integer()) ->
    txn_routing().
...
-spec update_xor(
    non_neg_integer(), libp2p_crypto:pubkey_bin(), non_neg_integer(), binary(), non_neg_integer()
) -> txn_routing().
...
-spec request_subnet(
    non_neg_integer(), libp2p_crypto:pubkey_bin(), pos_integer(), non_neg_integer()
) -> txn_routing().
request_subnet(OUI, Owner, SubnetSize, Nonce) ->
    #blockchain_txn_routing_v1_pb{
        ...
        fee = ?LEGACY_TXN_FEE,
        %% routing did not implement staking fee until txn fees, so has to default to zero
        staking_fee = 0,
        nonce = Nonce,
        signature = <<>>
    }.
...
-spec fee_payer(txn_routing(), blockchain_ledger_v1:ledger()) ->
...
-spec calculate_fee(
    txn_routing(), blockchain_ledger_v1:ledger(), pos_integer(), pos_integer(), boolean()
) -> non_neg_integer().
...
calculate_fee(Txn, Ledger, DCPayloadSize, TxnFeeMultiplier, true) ->
    ?calculate_fee(
        Txn#blockchain_txn_routing_v1_pb{fee = 0, staking_fee = 0, signature = <<0:512>>},
        Ledger,
        DCPayloadSize,
        TxnFeeMultiplier
    ).
...
calculate_staking_fee(
    #blockchain_txn_routing_v1_pb{update = {request_subnet, SubnetSize}} = _Txn,
    _Ledger,
    Fee,
    _ExtraData,
    true
                                    ...
                                    case blockchain:config(?max_xor_filter_num, Ledger) of
                                        {ok, XORFilterNum} ->
                                            case blockchain:config(?max_subnet_size, Ledger) of
                                                {ok, MaxSubnetSize} ->
                                                    case
                                                        blockchain:config(?min_subnet_size, Ledger)
                                                    of
                                                        {ok, MinSubnetSize} ->
                                                            case
                                                                blockchain:config(
                                                                    ?max_subnet_num, Ledger
                                                                )
                                                            of
                                                                {ok, MaxSubnetNum} ->
                                                                    case
...
absorb(Txn, Chain) ->
    ...
    case Action of
        ...
        _ ->
            ...
            case
                blockchain_ledger_v1:debit_fee(
                    Owner, TxnFee + StakingFee, Ledger, AreFeesEnabled, TxnHash, Chain
                )
            of
                {error, _} = Error ->
                    Error;
...
print(#blockchain_txn_routing_v1_pb{
    oui = OUI,
    owner = Owner,
    update = Update,
    fee = Fee,
    ...
    io_lib:format(
        "type=routing oui=~p owner=~p update=~p fee=~p nonce=~p signature=~p",
...
-spec validate_owner(Txn :: txn_routing(), Routing :: blockchain_ledger_routing_v1:routing()) ->
    boolean().
        ...
        {L, L} when L =< 3 ->
            ok ==
                blockchain_txn:validate_fields([
                    {{router_address, P}, {address, libp2p}}
                 || P <- Addresses
                ]);
        _ ->
            false
...
do_is_valid_checks(
    ...
    case ?MODULE:action(Txn) of
        ...
        {request_subnet, SubnetSize} when
            SubnetSize < MinSubnetSize orelse SubnetSize > MaxSubnetSize
        ->
                            ...
                            case blockchain_ledger_v1:allocate_subnet(SubnetSize, Ledger) of
                                {ok, _} ->
                                    ...
                                    of
                                        ...
                                        {true, true} ->
                                            blockchain_ledger_v1:check_dc_or_hnt_balance(
                                                Owner, TxnFee + StakingFee, Ledger, AreFeesEnabled
                                            )
                                    end;
                                Error ->
                                    Error
...
-spec subnets_left(
    Routing :: blockchain_ledger_routing_v1:routing(), MaxSubnetNum :: pos_integer()
) -> boolean().
...
sign_test() ->
    ...
    Tx1 = sign(Tx0, SigFun),
    Sig1 = signature(Tx1),
    EncodedTx1 = blockchain_txn_routing_v1_pb:encode_msg(Tx1#blockchain_txn_routing_v1_pb{
        signature = <<>>
    }),
...
ecode_decode_test() ->
    ...
    ?assertEqual(
        Tx,
        blockchain_txn_routing_v1_pb:decode_msg(
            blockchain_txn_routing_v1_pb:encode_msg(Tx), blockchain_txn_routing_v1_pb
        )
    ).
