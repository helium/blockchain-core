...
-spec new(
    ...
    Buyer :: libp2p_crypto:pubkey_bin(),
    BuyerNonce :: non_neg_integer(),
    AmountToSeller :: non_neg_integer()
) -> txn_transfer_hotspot().
new(Gateway, Seller, Buyer, BuyerNonce, AmountToSeller) when
    is_integer(AmountToSeller) andalso
        AmountToSeller >= 0
->
...
hash(Txn) ->
    BaseTxn = Txn#blockchain_txn_transfer_hotspot_v1_pb{
        seller_signature = <<>>,
        buyer_signature = <<>>
    },
...
-spec fee_payer(txn_transfer_hotspot(), blockchain_ledger_v1:ledger()) ->
...
calculate_fee(Txn, Ledger, DCPayloadSize, TxnFeeMultiplier, true) ->
    ?calculate_fee(
        Txn#blockchain_txn_transfer_hotspot_v1_pb{
            fee = 0,
...
is_valid(
    ...
    Conditions = [
        {fun() -> Seller /= Buyer end, {error, seller_is_buyer}},
        {fun() -> ?MODULE:is_valid_seller(Txn) end, {error, bad_seller_signature}},
        {fun() -> ?MODULE:is_valid_buyer(Txn) end, {error, bad_buyer_signature}},
        {fun() -> is_integer(Bones) andalso Bones >= 0 end, {error, invalid_hnt_to_seller}},
        {fun() -> gateway_not_stale(Txn, Ledger) end, {error, gateway_too_stale}},
        {fun() -> seller_owns_gateway(Txn, Ledger) end, {error, gateway_not_owned_by_seller}},
        {fun() -> buyer_nonce_correct(Txn, Ledger) end, {error, wrong_buyer_nonce}},
        {fun() -> txn_fee_valid(Txn, Chain, AreFeesEnabled) end, {error, wrong_txn_fee}},
        {fun() -> buyer_has_enough_hnt(Txn, Ledger) end, {error, buyer_insufficient_hnt_balance}}
    ],
...
print(undefined) ->
    <<"type=transfer_hotspot, undefined">>;
print(#blockchain_txn_transfer_hotspot_v1_pb{
    ...
    buyer_nonce = Nonce,
    fee = Fee,
    amount_to_seller = HNT
}) ->
    io_lib:format(
        "type=transfer_hotspot, gateway=~p, seller=~p, buyer=~p, seller_signature=~p, buyer_signature=~p, buyer_nonce=~p, fee=~p (dc), amount_to_seller=~p",
...
buyer_nonce_correct(
    ...
    case blockchain_ledger_v1:find_entry(Buyer, Ledger) of
        {error, _} -> false;
        {ok, Entry} -> Nonce =:= blockchain_ledger_entry_v1:nonce(Entry) + 1
    end.
...
-ifdef(TEST).
new_4_test() ->
    Tx = #blockchain_txn_transfer_hotspot_v1_pb{
        gateway = <<"gateway">>,
        seller = <<"seller">>,
        seller_signature = <<>>,
        buyer = <<"buyer">>,
