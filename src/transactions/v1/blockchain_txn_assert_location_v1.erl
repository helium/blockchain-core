...
hash(Txn) ->
    BaseTxn = Txn#blockchain_txn_assert_location_v1_pb{
...
calculate_fee(Txn, Ledger, DCPayloadSize, TxnFeeMultiplier, true) ->
    case Txn#blockchain_txn_assert_location_v1_pb.payer of
        Payer when Payer == undefined; Payer == <<>> ->
            ...
            ?calculate_fee(
                Txn#blockchain_txn_assert_location_v1_pb{
                    fee = 0,
                    staking_fee = 0,
...
is_valid_payer(#blockchain_txn_assert_location_v1_pb{payer = undefined}) ->
    %% no payer
    true;
is_valid_payer(#blockchain_txn_assert_location_v1_pb{
    payer = <<>>,
    payer_signature = <<>>
}) ->
...
%% @end
%%--------------------------------------------------------------------
-spec is_valid(txn_assert_location(), blockchain:blockchain()) ->
    ok | {error, atom()} | {error, {atom(), any()}}.
is_valid(Txn, Chain) ->
    Ledger = blockchain:ledger(Chain),
    case
        {?MODULE:is_valid_owner(Txn), ?MODULE:is_valid_gateway(Txn), ?MODULE:is_valid_payer(Txn)}
    of
        {false, _, _} ->
                ...
                {true, true} ->
                    case
                        blockchain_ledger_v1:check_dc_or_hnt_balance(
                            ActualPayer, TxnFee + StakingFee, Ledger, AreFeesEnabled
                        )
                    of
                        {error, _} = Error ->
                            Error;
                            ...
                            case blockchain_ledger_v1:find_gateway_info(Gateway, Ledger) of
                                ...
                                {ok, GwInfo} ->
                                    ...
                                    case Owner == GwOwner of
                                        ...
                                        true ->
                                            {ok, MinAssertH3Res} = blockchain:config(
                                                ?min_assert_h3_res, Ledger
                                            ),
                                                ...
                                                false ->
                                                    {error,
                                                        {insufficient_assert_res,
                                                            {assert_location, Location, Gateway}}};
                                                true ->
                                                    LedgerNonce = blockchain_ledger_gateway_v2:nonce(
                                                        GwInfo
                                                    ),
...
absorb(Txn, Chain) ->
    ...
    case
        blockchain_ledger_v1:debit_fee(
            ActualPayer, Fee + StakingFee, Ledger, AreFeesEnabled, Hash, Chain
        )
    of
        {error, _Reason} = Error ->
            Error;
...
print(#blockchain_txn_assert_location_v1_pb{
    ...
    io_lib:format(
        "type=assert_location, gateway=~p, owner=~p, payer=~p, location=~p, gateway_signature=~p, owner_signature=~p, payer_signature=~p, nonce=~p, staking_fee=~p, fee=~p",
        [
...
sign_request_test() ->
    ...
    BaseTxn1 = Tx1#blockchain_txn_assert_location_v1_pb{
        gateway_signature = <<>>, owner_signature = <<>>
    },
    ?assert(
        libp2p_crypto:verify(
            blockchain_txn_assert_location_v1_pb:encode_msg(BaseTxn1), Sig1, PubKey
        )
    ).
...
sign_test() ->
    ...
    ?assert(
        libp2p_crypto:verify(
            blockchain_txn_assert_location_v1_pb:encode_msg(BaseTxn1), Sig2, PubKey
        )
    ).
...
sign_payer_test() ->
    ...
    BaseTx1 = Tx3#blockchain_txn_assert_location_v1_pb{
        gateway_signature = <<>>, owner_signature = <<>>, payer_signature = <<>>
    },
    ?assert(
        libp2p_crypto:verify(blockchain_txn_assert_location_v1_pb:encode_msg(BaseTx1), Sig2, PubKey)
    ).
