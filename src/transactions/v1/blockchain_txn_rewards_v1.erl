...
%% @end
%%--------------------------------------------------------------------
-spec new(non_neg_integer(), non_neg_integer(), blockchain_txn_reward_v1:rewards()) ->
    txn_rewards().
...
-spec fee_payer(txn_rewards(), blockchain_ledger_v1:ledger()) ->
...
%% @end
%%--------------------------------------------------------------------
-spec is_valid(txn_rewards(), blockchain:blockchain()) ->
...
%% @end
%%--------------------------------------------------------------------
-spec absorb(txn_rewards(), blockchain:blockchain()) ->
    ok | {error, atom()} | {error, {atom(), any()}}.
absorb(Txn, Chain) ->
...
-spec calculate_rewards_(
    ...
    Ledger :: blockchain_ledger_v1:ledger(),
    Chain :: blockchain:blockchain()
) -> {error, any()} | {ok, blockchain_txn_reward_v1:rewards()}.
calculate_rewards_(Start, End, Ledger, Chain) ->
    ...
    case get_rewards_for_epoch(Start, End, Chain, Vars, Ledger, PreviousGraceBlockDCRewards) of
        ...
        {ok, POCChallengersRewards, POCChallengeesRewards, POCWitnessesRewards, DCRewards} ->
            ...
            ConsensusRewards = consensus_members_rewards(
                Ledger, maps:put(epoch_reward, ConsensusEpochReward, Vars)
            ),
            Result = lists:foldl(
                fun(Map, Acc0) ->
                    maps:fold(
                        fun
                            ({owner, Type, Owner}, Amount, Acc1) ->
                                Reward = blockchain_txn_reward_v1:new(
                                    Owner, undefined, Amount, Type
                                ),
                                [Reward | Acc1];
                ...
                [
                    ConsensusRewards,
                    SecuritiesRewards,
                    POCChallengersRewards,
                    POCChallengeesRewards,
                    POCWitnessesRewards,
                    DCRewards
                ]
            ),
...
print(undefined) ->
    <<"type=rewards undefined">>;
print(#blockchain_txn_rewards_v1_pb{
    start_epoch = Start,
    end_epoch = End,
    rewards = Rewards
}) ->
    ...
    io_lib:format(
        "type=rewards start_epoch=~p end_epoch=~p rewards=~p",
...
get_rewards_for_epoch(
    ...
    {ok, normalize_challenger_rewards(ChallengerRewards, Vars),
        normalize_challengee_rewards(ChallengeeRewards, Vars), NormalizedWitnessRewards,
        NewDCRewards};
get_rewards_for_epoch(
    Current,
    End,
                    ...
                    case blockchain_hex:var_map(Ledger) of
                        {error, _} ->
                            ...
                            get_rewards_for_epoch(
                                ...
                                Vars,
                                Ledger,
                                poc_challengers_rewards(
                                    Transactions,
                                    Vars,
                                    ChallengerRewards
                                ),
                                poc_challengees_rewards(
                                    Transactions,
                                    Vars,
                                    Chain,
                                    Ledger,
...
calculate_epoch_reward(Version, Start, End, BlockTime0, _ElectionInterval, MonthlyReward) when
    Version >= 2
->
...
consensus_members_rewards(Ledger, #{
    epoch_reward := EpochReward,
    consensus_percent := ConsensusPercent
}) ->
    GwOrVal =
...
-spec securities_rewards(
    blockchain_ledger_v1:ledger(),
    map()
) -> #{{owner, libp2p_crypto:pubkey_bin()} => non_neg_integer()}.
            ...
            case blockchain_txn:type(Txn) == blockchain_txn_poc_receipts_v1 of
                ...
                true ->
                    ...
                    poc_challengees_rewards_(
                        Vars, Path, Path, Txn, Chain, Ledger, true, VarMap, Acc0
                    )
            end
        end,
        ExistingRewards,
                            ...
                            true when is_integer(Version), Version > 4 ->
                                ...
                                case
                                    poc_challengee_reward_unit(
                                        WitnessRedundancy, DecayRate, Witnesses
                                    )
                                of
                                    {error, _} ->
                                        %% Old behavior
                    ...
                    case POCVersion of
                        V when is_integer(V), V >= 9 ->
                            try
                                ...
                                {ok, Channels} = blockchain_txn_poc_receipts_v1:get_channels(
                                    Txn, Chain
                                ),
                                Path = blockchain_txn_poc_receipts_v1:path(Txn),
                                                    ...
                                                    undefined ->
                                                        ...
                                                        lists:foldl(
                                                            fun(WitnessRecord, Acc2) ->
                                                                Witness = blockchain_poc_witness_v1:gateway(
                                                                    WitnessRecord
                                                                ),
                                                    ...
                                                    D ->
                                                        ...
                                                        lists:foldl(
                                                            fun(WitnessRecord, Acc2) ->
                                                                ...
                                                                {ok, ChallengeeGw} = blockchain_ledger_v1:find_gateway_info(
                                                                    Challengee, Ledger
                                                                ),
                                                                %% Challengee must have a location
                                                                ChallengeeLoc = blockchain_ledger_gateway_v2:location(
                                                                    ChallengeeGw
                                                                ),
                                                                Witness = blockchain_poc_witness_v1:gateway(
                                                                    WitnessRecord
                                                                ...
                                                                Value = blockchain_utils:normalize_float(
                                                                    ToAdd * RxScale
                                                                ),
                                                                I = maps:get(Witness, Acc2, 0),
                                ...
                                What:Why:ST ->
                                    lager:error(
                                        "failed to calculate poc_witnesses_rewards, error ~p:~p:~p",
                                        [What, Why, ST]
                                    ),
                                    Acc0
                            end;
                        V when is_integer(V), V > 4 ->
                            lists:foldl(
                                fun(Elem, Acc1) ->
                                    case
                                        blockchain_txn_poc_receipts_v1:good_quality_witnesses(
                                            Elem, Ledger
                                        )
                                    of
                                        [] ->
...
dc_rewards(
    Transactions,
    EndHeight,
    #{sc_grace_blocks := GraceBlocks, sc_version := 2} = Vars,
    Ledger,
    DCRewards
) ->
    lists:foldl(
        fun(Txn, Acc) ->
            ...
            case
                blockchain_txn:type(Txn) == blockchain_txn_state_channel_close_v1 andalso
                    blockchain_txn_state_channel_close_v1:state_channel_expire_at(Txn) + GraceBlocks <
                        EndHeight
            of
                true ->
                            ...
                            case blockchain_ledger_state_channel_v2:is_v2(SC) of
                                true ->
                                    ...
                                    lists:foldl(
                                        ...
                                        maps:update_with(
                                            seen, fun(Seen) -> [SCID | Seen] end, [SCID], Acc
                                        ),
                                        Summaries
                                    );
                                false ->
                ...
                false ->
                    ...
                    try Type:calculate_fee(Txn, Ledger) - Type:fee(Txn) of
                        Overage when Overage > 0 ->
                            maps:update_with(
                                overages, fun(Overages) -> Overages + Overage end, Overage, Acc
                            );
                        _ ->
                            Acc
                    catch
...
normalize_dc_rewards(
    ...
    DCRewards =
        case maps:take(overages, DCRewards1) of
            {OverageTotal, NewMap} ->
                ...
                lists:foldl(
                    fun(Member, Acc) ->
                        maps:update_with(
                            Member,
                            fun(Balance) -> Balance + OveragePerMember end,
                            OveragePerMember,
                            Acc
                        )
                    end,
...
share_of_dc_rewards(Key, Vars = #{dc_remainder := DCRemainder}) ->
    erlang:round(
        DCRemainder *
            (maps:get(Key, Vars) /
                (maps:get(poc_challengers_percent, Vars) + maps:get(poc_challengees_percent, Vars) +
...
poc_challengers_rewards_1_test() ->
    ...
    ?assertEqual(
        Rewards, normalize_challenger_rewards(poc_challengers_rewards(Txns, Vars, #{}), Vars)
    ).
...
old_poc_witnesses_rewards_test() ->
    ...
    Elem = blockchain_poc_path_element_v1:new(<<"Y">>, <<"Receipt not undefined">>, [
        Witness1, Witness2
    ]),
    Txns = [
...
dc_rewards_test() ->
    ...
    ?assertEqual(
        {0, Rewards}, normalize_dc_rewards(dc_rewards(Txns, 100, Vars, Ledger1, #{}), Vars)
    ),
    test_utils:cleanup_tmp_dir(BaseDir).
...
dc_rewards_v3_test() ->
    ...
    ?assertEqual(
        {26999, Rewards}, normalize_dc_rewards(dc_rewards(Txns, 100, Vars, Ledger1, #{}), Vars)
    ),
    test_utils:cleanup_tmp_dir(BaseDir).
...
dc_rewards_v3_spillover_test() ->
    ...
    {DCRemainder, DCRewards} = normalize_dc_rewards(
        dc_rewards(AllTxns, 100, Vars, Ledger, #{}), Vars
    ),
    ...
    ChallengerRewards = normalize_challenger_rewards(
        poc_challengers_rewards(AllTxns, Vars, #{}), Vars
    ),
    ...
    %% apply the DC remainder, if any to the other PoC categories pro rata
    SpilloverChallengerRewards = normalize_challenger_rewards(
        poc_challengers_rewards(AllTxns, Vars, #{}), NewVars
    ),
