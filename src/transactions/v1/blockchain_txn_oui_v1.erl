                ...
                true ->
                    case
                        blockchain_ledger_v1:debit_fee(
                            ActualPayer, TxnFee + StakingFee, Ledger, AreFeesEnabled, TxnHash, Chain
                        )
                    of
                        {error, _} = Error ->
                            Error;
...
-spec calculate_fee(
    txn_oui(), blockchain_ledger_v1:ledger(), pos_integer(), pos_integer(), boolean()
) -> non_neg_integer().
...
calculate_staking_fee(Txn, Chain) ->
    ...
    calculate_staking_fee(
        Txn, Ledger, OUIStakingFee, ExtraData, blockchain_ledger_v1:txn_fees_active(Ledger)
    ).
...
print(undefined) ->
    <<"type=oui, undefined">>;
print(#blockchain_txn_oui_v1_pb{
    owner = Owner,
    addresses = Addresses,
    payer = Payer,
    ...
    io_lib:format(
        "type=oui, owner=~p, addresses=~p, payer=~p, staking_fee=~p, fee=~p",
...
-spec validate_oui(OUI :: pos_integer(), Ledger :: blockchain_ledger_v1:ledger()) ->
...
-spec do_oui_validation_checks(txn_oui(), blockchain:blockchain()) ->
    ok | {error, atom()} | {error, {atom(), any()}}.
...
sign_test() ->
    ...
    EncodedTx1 = blockchain_txn_oui_v1_pb:encode_msg(Tx1#blockchain_txn_oui_v1_pb{
