...
-spec fee_payer(txn_unstake_validator(), blockchain_ledger_v1:ledger()) ->
...
calculate_fee(Txn, Ledger, DCPayloadSize, TxnFeeMultiplier, _) ->
    ?calculate_fee(
        Txn#blockchain_txn_unstake_validator_v1_pb{
            fee = 0,
            ...
            try
                ...
                case blockchain_ledger_v1:get_validator(Validator, Ledger) of
                    {ok, V} ->
                        ...
                        case
                            StakeReleaseHeight >= (ThisBlockHeight + Cooldown) andalso
                                StakeReleaseHeight < (ThisBlockHeight + Cooldown + CooldownMax)
                        of
                            true -> ok;
...
-spec absorb(txn_unstake_validator(), blockchain:blockchain()) ->
    ok | {error, atom()} | {error, {atom(), any()}}.
absorb(Txn, Chain) ->
    Ledger = blockchain:ledger(Chain),
    Owner = owner(Txn),
    ...
    case blockchain_ledger_v1:debit_fee(Owner, Fee, Ledger, true, Hash, Chain) of
        ...
        ok -> blockchain_ledger_v1:deactivate_validator(Validator, StakeReleaseHeight, Ledger)
    end.

-spec print(txn_unstake_validator()) -> iodata().
print(undefined) ->
    <<"type=unstake_validator, undefined">>;
print(#blockchain_txn_unstake_validator_v1_pb{
    ...
    io_lib:format(
        "type=unstake_validator, owner=~p, validator=~p, stake_amount=~p, stake_release_height=~p",
