...
hash(Txn) ->
    BaseTxn = Txn#blockchain_txn_update_gateway_oui_v1_pb{
        gateway_owner_signature = <<>>, oui_owner_signature = <<>>
    },
...
-spec fee_payer(txn_update_gateway_oui(), blockchain_ledger_v1:ledger()) ->
...
-spec gateway_owner_sign(txn_update_gateway_oui(), libp2p_crypto:sig_fun()) ->
    txn_update_gateway_oui().
...
is_valid_gateway_owner(
    GatewayOwner,
    #blockchain_txn_update_gateway_oui_v1_pb{gateway_owner_signature = Signature} = Txn
) ->
    BaseTxn = Txn#blockchain_txn_update_gateway_oui_v1_pb{
        gateway_owner_signature = <<>>, oui_owner_signature = <<>>
    },
...
is_valid_oui_owner(
    OUIOwner, #blockchain_txn_update_gateway_oui_v1_pb{oui_owner_signature = Signature} = Txn
) ->
    BaseTxn = Txn#blockchain_txn_update_gateway_oui_v1_pb{
        gateway_owner_signature = <<>>, oui_owner_signature = <<>>
    },
...
-spec calculate_fee(
    txn_update_gateway_oui(), blockchain_ledger_v1:ledger(), pos_integer(), pos_integer(), boolean()
) -> non_neg_integer().
...
calculate_fee(Txn, Ledger, DCPayloadSize, TxnFeeMultiplier, true) ->
    ?calculate_fee(
        Txn#blockchain_txn_update_gateway_oui_v1_pb{
            fee = 0, gateway_owner_signature = <<0:512>>, oui_owner_signature = <<0:512>>
        },
        Ledger,
        DCPayloadSize,
        TxnFeeMultiplier
...
-spec is_valid(txn_update_gateway_oui(), blockchain:blockchain()) ->
    ok | {error, atom()} | {error, {atom(), any()}}.
is_valid(Txn, Chain) ->
    Ledger = blockchain:ledger(Chain),
    case {validate_oui(Txn, Ledger), validate_gateway(Txn, Ledger)} of
        {{error, _} = Err, _} ->
            Err;
        {_, {error, _} = Err} ->
            Err;
                    ...
                    case ExpectedTxnFee =< TxnFee orelse not AreFeesEnabled of
                        ...
                        true ->
                            blockchain_ledger_v1:check_dc_or_hnt_balance(
                                GatewayOwner, TxnFee, Ledger, AreFeesEnabled
                            )
                    end
            end
    end.

-spec absorb(txn_update_gateway_oui(), blockchain:blockchain()) ->
    ok | {error, atom()} | {error, {atom(), any()}}.
absorb(Txn, Chain) ->
    Ledger = blockchain:ledger(Chain),
        ...
        {ok, GWInfo} ->
            ...
            case
                blockchain_ledger_v1:debit_fee(
                    GatewayOwner, Fee, Ledger, AreFeesEnabled, Hash, Chain
                )
            of
                {error, _} = Error ->
                    Error;
...
-spec validate_gateway(txn_update_gateway_oui(), blockchain_ledger_v1:ledger()) ->
...
print(undefined) ->
    <<"type=update_gateway_oui, undefined">>;
print(#blockchain_txn_update_gateway_oui_v1_pb{gateway = GW, oui = OUI, nonce = Nonce, fee = Fee}) ->
    io_lib:format("type=update_gateway_oui, gateway=~p, oui=~p, nonce=~p, fee=~p", [
        ?TO_ANIMAL_NAME(GW), OUI, Nonce, Fee
    ]).
