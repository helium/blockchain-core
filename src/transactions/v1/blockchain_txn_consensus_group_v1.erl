...
%% @end
%%--------------------------------------------------------------------
-spec new([libp2p_crypto:pubkey_bin()], binary(), pos_integer(), non_neg_integer()) ->
    txn_consensus_group().
...
-spec fee_payer(txn_consensus_group(), blockchain_ledger_v1:ledger()) ->
...
%% @end
%%--------------------------------------------------------------------
-spec is_valid(txn_consensus_group(), blockchain:blockchain()) ->
    ...
    try
        ...
        case blockchain_ledger_v1:current_height(Ledger) of
            ...
            {ok, 0} ->
                ok;
            {ok, CurrHeight} ->
                {ok, #block_info_v2{election_info = {_, LastElectionHeight}}} = blockchain:get_block_info(
                    CurrHeight, Chain
                ),
                ...
                case TxnHeight >= LastElectionHeight + ElectionInterval of
                    true ->
                        ...
                        {ok, RestartInterval} = blockchain:config(
                            ?election_restart_interval, Ledger
                        ),
                        IntervalRange =
                        ...
                        NextRestart =
                            LastElectionHeight + ElectionInterval + Delay +
                                ...
                                case
                                    lists:all(
                                        fun(M) ->
                                            ...
                                            blockchain_ledger_validator_v1:status(V) == staked
                                        end,
                                        Members
                                    )
                                of
                    ...
                    _ ->
                        throw(
                            {error,
                                {election_too_early,
                                    {TxnHeight, LastElectionHeight + ElectionInterval}}}
                        )
                end
        end
    catch
...
%% @end
%%--------------------------------------------------------------------
-spec absorb(txn_consensus_group(), blockchain:blockchain()) ->
        ...
        ok ->
            case blockchain_ledger_v1:config(?election_version, Ledger) of
                {ok, N} when N >= 5 andalso Gen == false ->
                    {ok, PenaltyLimit} = blockchain_ledger_v1:config(
                        ?penalty_history_limit, Ledger
                    ),
                    ...
                    lists:foreach(
                        fun(M) ->
                            {ok, V} = blockchain_ledger_v1:get_validator(M, Ledger),
                            V1 =
                                case maps:get(M, EpochPenalties, none) of
                                    none ->
                                        V;
                                    0.0 ->
                                        V;
                                    Penalty ->
                                        blockchain_ledger_validator_v1:add_penalty(
                                            V,
                                            CurrHeight,
                                            performance,
                                            Penalty,
...
print(#blockchain_txn_consensus_group_v1_pb{
    height = Height,
    delay = Delay,
    members = Members,
    proof = Proof
    ...
    io_lib:format(
        "type=group height=~p delay=~p members=~p proof_hash=~p",
        [
            Height,
            Delay,
...
verify_proof(Proof, Members, Hash, Delay, OldLedger) ->
    ...
    case HashMembers of
        ...
        _ ->
            lager:info(
                "groups didn't match: ~p ~p ~ntxn ~p ~nhash ~p",
                [
                    length(Members),
                    length(HashMembers),
                    lists:map(fun blockchain_utils:addr2name/1, Members),
...
new_test() ->
    Tx = #blockchain_txn_consensus_group_v1_pb{
        members = [<<"1">>],
        proof = <<"proof">>,
        height = 1,
        delay = 0
