...
-type hash() :: <<_:256>>.
-type txn() ::
    blockchain_txn_add_gateway_v1:txn_add_gateway()
...
-type before_commit_callback() :: fun(
    (blockchain:blockchain(), blockchain_block:hash()) -> ok | {error, any()}
).
-type txns() :: [txn()].
...
wrap_txn(#blockchain_txn_bundle_v1_pb{transactions = Txns} = Txn) ->
    #blockchain_txn_pb{
        txn =
            {bundle, Txn#blockchain_txn_bundle_v1_pb{
                transactions = lists:map(fun wrap_txn/1, Txns)
            }}
    };
            ...
            case catch Type:is_valid(Txn, Chain) of
                ok ->
                    case ?MODULE:absorb(Txn, Chain) of
                        ...
                        {error, {bad_nonce, {_NonceType, Nonce, LedgerNonce}}} when
                            Nonce > LedgerNonce + 1
                        ->
                        ...
                        {error, {InvalidReason, _Details}} = Error ->
                            lager:warning("invalid txn while absorbing ~p : ~p / ~s", [
                                Type, Error, print(Txn)
                            ]),
                            validate(
                                Tail, Valid, [{Txn, InvalidReason} | Invalid], PType, PBuf, Chain
                            );
...
separate_res([{T, Err} | Rest], Chain, V, I) ->
    case Err of
        ...
        Error ->
            ...
            lager:warning("BUG: unexpected txn validation error format ~p : ~p / ~s", [
                type(T), Error, print(T)
            ]),
...
%% @end
%%--------------------------------------------------------------------
-spec absorb_and_commit(
    blockchain_block:block(), blockchain:blockchain(), before_commit_callback()
) ->
    ok | {error, any()}.
...
-spec absorb_and_commit(
    blockchain_block:block(), blockchain:blockchain(), before_commit_callback(), boolean()
) ->
    ok | {error, any()}.
                ...
                {ok, Chain2} ->
                    ...
                    case BeforeCommit(Chain2, Hash) of
                        ok ->
                            ...
                            lager:info(
                                "validation took ~p absorb took ~p post took ~p ms for block height ~p",
...
-spec unvalidated_absorb_and_commit(
    blockchain_block:block(), blockchain:blockchain(), before_commit_callback(), boolean()
) ->
    ok | {error, any()}.
                ...
                {ok, Chain2} ->
                    ...
                    case BeforeCommit(Chain2, Hash) of
                        ok ->
                            ...
                            End3 = erlang:monotonic_time(millisecond),
                            lager:info(
                                "validation took ~p absorb took ~p post took ~p ms height ~p",
                                [End - Start, End2 - End, End3 - End2, Height]
                            ),
                            ok;
                        Any ->
...
absorb(Txn, Chain) ->
    ...
    try Type:absorb(Txn, Chain) of
        ...
        ok ->
            End = erlang:monotonic_time(millisecond),
            % in ms
            Slow = application:get_env(blockchain, slow_txn_log_threshold, 25),
...
validate_fields([{{Name, Field}, {member, List}} | Tail]) when
    is_list(List),
    is_binary(Field)
->
...
validate_fields([{{_Name, Field}, {is_integer, Min}} | Tail]) when
    is_integer(Field) andalso
        Field >= Min
->
    validate_fields(Tail);
validate_fields([{{Name, Field}, {is_integer, Min}} | _Tail]) when
    is_integer(Field) andalso
        Field < Min
->
...
absorb_txns([Txn | Txns], Rescue, Chain) ->
    ...
    case
        Rescue andalso
            erlang:function_exported(Type, rescue_absorb, 2)
    of
        true ->
        ...
        blockchain_txn_state_channel_close_v1 ->
            ...
            case blockchain_state_channel_v1:amount(SC) > 0 of
                true ->
                    ...
                    1 -
                        blockchain_state_channel_v1:nonce(
...
-spec depends_on(txn(), [blockchain_txn_mgr:cached_txn_type()]) ->
    [blockchain_txn_mgr:cached_txn_type()].
depends_on(ThisTxn, CachedTxns) ->
    case type(ThisTxn) of
        Type when
            Type == blockchain_txn_payment_v1;
        ...
        ->
            ...
            lists:filter(
                fun({_TxnKey, Txn, _TxnData}) ->
                    ThisNonce = nonce(Txn),
                    lists:member(type(Txn), Types) andalso actor(Txn) == Actor andalso
                        ThisNonce < Nonce
                end,
                CachedTxns
            );
        blockchain_txn_assert_location_v1 ->
            ...
            lists:filter(
                fun({_TxnKey, Txn, _TxnData}) ->
                    (type(Txn) == blockchain_txn_assert_location_v1 andalso actor(Txn) == Actor andalso
                        nonce(Txn) < Nonce) orelse
                        (type(Txn) == blockchain_txn_add_gateway_v1 andalso
                            blockchain_txn_add_gateway_v1:gateway(Txn) == Actor)
                end,
                CachedTxns
            );
        blockchain_txn_security_exchange_v1 ->
            ...
            lists:filter(
                fun({_TxnKey, Txn, _TxnData}) ->
                    ThisNonce = nonce(Txn),
                    type(Txn) == blockchain_txn_security_exchange_v1 andalso actor(Txn) == Actor andalso
                        ThisNonce < Nonce
                end,
                CachedTxns
            );
        blockchain_txn_vars_v1 ->
            ...
            lists:filter(
                fun({_TxnKey, Txn, _TxnData}) ->
                    type(Txn) == blockchain_txn_vars_v1 andalso
                        blockchain_txn_vars_v1:nonce(Txn) < Nonce
                end,
                CachedTxns
            );
        blockchain_txn_state_channel_open_v1 ->
            ...
            lists:filter(
                fun({_TxnKey, Txn, _TxnData}) ->
                    (type(Txn) == blockchain_txn_oui_v1 andalso
                        lists:member(Actor, blockchain_txn_oui_v1:addresses(Txn))) orelse
                        (type(Txn) == blockchain_txn_state_channel_open_v1 andalso
                            actor(Txn) == Actor andalso nonce(Txn) < Nonce) orelse
                        (type(Txn) == blockchain_txn_routing_v1 andalso
                            blockchain_txn_routing_v1:oui(Txn) == OUI) orelse
                        (type(Txn) == blockchain_txn_token_burn_v1 andalso
                            blockchain_txn_token_burn_v1:payee(Txn) == Actor)
                end,
                CachedTxns
            );
        blockchain_txn_state_channel_close_v1 ->
        ...
        blockchain_txn_routing_v1 ->
            ...
            lists:filter(
                fun({_TxnKey, Txn, _TxnData}) ->
                    (type(Txn) == blockchain_txn_oui_v1 andalso
                        blockchain_txn_oui_v1:owner(Txn) == Actor) orelse
                        (type(Txn) == blockchain_txn_routing_v1 andalso actor(Txn) == Actor andalso
                            nonce(Txn) < Nonce)
                end,
                CachedTxns
            );
        blockchain_txn_assert_location_v2 ->
            ...
            lists:filter(
                fun({_TxnKey, Txn, _TxnData}) ->
                    (type(Txn) == blockchain_txn_assert_location_v2 andalso actor(Txn) == Actor andalso
                        nonce(Txn) < Nonce) orelse
                        (type(Txn) == blockchain_txn_add_gateway_v1 andalso
                            blockchain_txn_add_gateway_v1:gateway(Txn) == Actor)
                end,
...
-define(ADDRESS_KEY4,
    <<0, 201, 24, 252, 94, 154, 8, 151, 21, 177, 201, 93, 234, 97, 223, 234, 109, 216, 141, 189,
        126, 227, 92, 243, 87, 8, 134, 107, 91, 11, 221, 179, 190>>
).
                ...
                1 ->
                    ...
                    {Nonce,
                        blockchain_txn_payment_v1:sign(
                            blockchain_txn_payment_v1:new(Payer, Recipient, 1, Nonce), SigFun
                        ),
                        []};
                2 ->
                    %% need to create a payload same as blockchain_txn_mgr:cached_txn() to pass to depends_on
                    {Nonce,
                        blockchain_txn_payment_v2:sign(
                            blockchain_txn_payment_v2:new(
                                Payer, [blockchain_payment_v2:new(Recipient, 1)], Nonce
                            ),
                            SigFun
                        ),
                        []}
...
sc_depends_on_test() ->
    ...
    %% oui for payer
    O0 = blockchain_txn_oui_v1:sign(
        blockchain_txn_oui_v1:new(1, Payer, [Payer], Filter, 8), SigFun
    ),
    %% emulate blockchain_txn_mgr:cached_txn()
    O0TxnData = {1, O0, []},
    %% oui for payer1
    O1 = blockchain_txn_oui_v1:sign(
        blockchain_txn_oui_v1:new(2, Payer1, [Payer1], Filter1, 8), SigFun1
    ),
    %% emulate blockchain_txn_mgr:cached_txn()
    O1TxnData = {2, O1, []},

    %% routing for payer
    RT1 = blockchain_txn_routing_v1:sign(
        blockchain_txn_routing_v1:update_router_addresses(1, Payer, gen_pubkeys(3), 1), SigFun
    ),
    %% emulate blockchain_txn_mgr:cached_txn()
    RT1TxnData = {3, RT1, []},
    %% routing for payer1
    RT2 = blockchain_txn_routing_v1:sign(
        blockchain_txn_routing_v1:update_router_addresses(2, Payer1, gen_pubkeys(3), 1), SigFun
    ),
    %% emulate blockchain_txn_mgr:cached_txn()
    RT2TxnData = {4, RT2, []},

    %% sc opens for payer
    SC1 = blockchain_txn_state_channel_open_v1:sign(
        blockchain_txn_state_channel_open_v1:new(crypto:strong_rand_bytes(24), Payer, 10, 1, 1, 0),
        SigFun
    ),
    %% emulate blockchain_txn_mgr:cached_txn()
    SC1TxnData = {5, SC1, []},
    SC2 = blockchain_txn_state_channel_open_v1:sign(
        blockchain_txn_state_channel_open_v1:new(crypto:strong_rand_bytes(24), Payer, 20, 1, 2, 0),
        SigFun
    ),
    %% emulate blockchain_txn_mgr:cached_txn()
    SC2TxnData = {6, SC2, []},
    SC3 = blockchain_txn_state_channel_open_v1:sign(
        blockchain_txn_state_channel_open_v1:new(crypto:strong_rand_bytes(24), Payer, 30, 1, 3, 0),
        SigFun
    ),
    %% emulate blockchain_txn_mgr:cached_txn()
    SC3TxnData = {7, SC3, []},
    ...
    %% sc open for payer1
    SC4 = blockchain_txn_state_channel_open_v1:sign(
        blockchain_txn_state_channel_open_v1:new(crypto:strong_rand_bytes(24), Payer1, 30, 2, 1, 0),
        SigFun
    ),

    ?assertEqual(
        lists:sort([O0TxnData, RT1TxnData, SC1TxnData, SC2TxnData]),
        lists:sort(
            depends_on(
                SC3, blockchain_utils:shuffle([O0TxnData, RT1TxnData, SC1TxnData, SC2TxnData])
            )
        )
    ),
    ?assertEqual(
        ...
        lists:sort(
            depends_on(
                SC3,
                blockchain_utils:shuffle([O0TxnData, O1TxnData, RT1TxnData, SC1TxnData, SC2TxnData])
            )
        )
    ),
    ?assertEqual(
        lists:sort([O0TxnData, RT1TxnData, SC1TxnData]),
        lists:sort(
            depends_on(
                SC2,
                blockchain_utils:shuffle([
                    O0TxnData, O1TxnData, RT1TxnData, SC1TxnData, SC2TxnData, SC3TxnData
                ])
            )
        )
    ),
    ?assertEqual(
        lists:sort([O1TxnData, RT2TxnData]),
        lists:sort(
            depends_on(
                SC4,
                blockchain_utils:shuffle([
                    O0TxnData, O1TxnData, RT1TxnData, RT2TxnData, SC1TxnData, SC2TxnData, SC3TxnData
                ])
            )
        )
    ),
...
txn_fees_payment_v1_test() ->
    ...
    Txn00Fee = blockchain_txn_payment_v1:calculate_fee(
        Txn00, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
    ...
    Txn02Fee = blockchain_txn_payment_v1:calculate_fee(
        Txn02, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
...
txn_fees_payment_v2_test() ->
    ...
    Txn00Fee = blockchain_txn_payment_v2:calculate_fee(
        Txn00, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
    ?assertEqual(35000, Txn00Fee),
    %% set the fee value of the txn, sign it and confirm the fee remains the same and unaffected by signature of fee values
    ...
    Txn02Fee = blockchain_txn_payment_v2:calculate_fee(
        Txn02, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
    ?assertEqual(35000, Txn02Fee),
    ok.
...
txn_fees_add_gateway_v1_test() ->
    ...
    Txn00Fee = blockchain_txn_add_gateway_v1:calculate_fee(
        Txn00, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
    Txn00StakingFee = blockchain_txn_add_gateway_v1:calculate_staking_fee(
        Txn00, ignore_ledger, ?ADD_GW_STAKING_FEE, [], true
    ),
    Txn00LegacyStakingFee = blockchain_txn_add_gateway_v1:calculate_staking_fee(
        Txn00, ignore_ledger, ?ADD_GW_STAKING_FEE, [], false
    ),
    ...
    Txn05Fee = blockchain_txn_add_gateway_v1:calculate_fee(
        Txn05, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
    Txn05StakingFee = blockchain_txn_add_gateway_v1:calculate_staking_fee(
        Txn05, ignore_ledger, ?ADD_GW_STAKING_FEE, [], true
    ),
    Txn05LegacyStakingFee = blockchain_txn_add_gateway_v1:calculate_staking_fee(
        Txn05, ignore_ledger, ?ADD_GW_STAKING_FEE, [], false
    ),
...
txn_fees_assert_location_v1_test() ->
    ...
    %% create new txn, and confirm expected txn and staking fee
    Txn00 = blockchain_txn_assert_location_v1:new(
        GWPubkeyBin, OwnerPubkeyBin, Payer, ?TEST_LOCATION, 1
    ),
    Txn00Fee = blockchain_txn_assert_location_v1:calculate_fee(
        Txn00, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
    Txn00StakingFee = blockchain_txn_assert_location_v1:calculate_staking_fee(
        Txn00, ignore_ledger, ?ASSERT_LOC_STAKING_FEE, [], true
    ),
    Txn00LegacyStakingFee = blockchain_txn_assert_location_v1:calculate_staking_fee(
        Txn00, ignore_ledger, ?ASSERT_LOC_STAKING_FEE, [], false
    ),
    ...
    Txn05Fee = blockchain_txn_assert_location_v1:calculate_fee(
        Txn05, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
    Txn05StakingFee = blockchain_txn_assert_location_v1:calculate_staking_fee(
        Txn05, ignore_ledger, ?ASSERT_LOC_STAKING_FEE, [], true
    ),
    Txn05LegacyStakingFee = blockchain_txn_assert_location_v1:calculate_staking_fee(
        Txn05, ignore_ledger, ?ASSERT_LOC_STAKING_FEE, [], false
    ),
...
txn_fees_create_htlc_v1_test() ->
    ...
    Txn00 = blockchain_txn_create_htlc_v1:new(
        Payer, RandomPubKey1, RandomPubKey2, Hashlock, 0, 10000, 1
    ),
    Txn00Fee = blockchain_txn_create_htlc_v1:calculate_fee(
        Txn00, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
    ...
    Txn02Fee = blockchain_txn_create_htlc_v1:calculate_fee(
        Txn02, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
...
txn_fees_redeem_htlc_v1_test() ->
    ...
    Txn00Fee = blockchain_txn_redeem_htlc_v1:calculate_fee(
        Txn00, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
    ?assertEqual(35000, Txn00Fee),
    %% set the fee value of the txn, sign it and confirm the fee remains the same and unaffected by signature of fee values
    ...
    Txn02Fee = blockchain_txn_redeem_htlc_v1:calculate_fee(
        Txn02, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
    ?assertEqual(35000, Txn02Fee),
    ok.
...
txn_fees_oui_test() ->
    ...
    Txn00Fee = blockchain_txn_oui_v1:calculate_fee(
        Txn00, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
    Txn00StakingFee = blockchain_txn_oui_v1:calculate_staking_fee(
        Txn00, ignore_ledger, ?OUI_STAKING_FEE, [{per_address, ?OUI_PER_ADDRESS_STAKING_FEE}], true
    ),
    Txn00LegacyStakingFee = blockchain_txn_oui_v1:calculate_staking_fee(
        Txn00, ignore_ledger, ?OUI_STAKING_FEE, [{per_address, ?OUI_PER_ADDRESS_STAKING_FEE}], false
    ),
    ?assertEqual(70000, Txn00Fee),
    ...
    Txn04Fee = blockchain_txn_oui_v1:calculate_fee(
        Txn04, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
    Txn04StakingFee = blockchain_txn_oui_v1:calculate_staking_fee(
        Txn04, ignore_ledger, ?OUI_STAKING_FEE, [{per_address, ?OUI_PER_ADDRESS_STAKING_FEE}], true
    ),
    Txn04LegacyStakingFee = blockchain_txn_oui_v1:calculate_staking_fee(
        Txn04, ignore_ledger, ?OUI_STAKING_FEE, [{per_address, ?OUI_PER_ADDRESS_STAKING_FEE}], false
    ),
...
txn_fees_routing_update_router_test() ->
    ...
    %% create new txn, and confirm expected fee size
    Txn00 = blockchain_txn_routing_v1:update_router_addresses(
        0, Owner, [?ADDRESS_KEY1, ?ADDRESS_KEY2], 1
    ),
    Txn00Fee = blockchain_txn_routing_v1:calculate_fee(
        Txn00, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
    Txn00StakingFee = blockchain_txn_routing_v1:calculate_staking_fee(
        Txn00, ignore_ledger, ?ROUTER_UPDATE_ROUTER_STAKING_FEE, [], true
    ),
    Txn00LegacyStakingFee = blockchain_txn_routing_v1:calculate_staking_fee(
        Txn00, ignore_ledger, ?ROUTER_UPDATE_ROUTER_STAKING_FEE, [], false
    ),
    ...
    Txn03Fee = blockchain_txn_routing_v1:calculate_fee(
        Txn03, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
    Txn03StakingFee = blockchain_txn_routing_v1:calculate_staking_fee(
        Txn00, ignore_ledger, ?ROUTER_UPDATE_ROUTER_STAKING_FEE, [], true
    ),
    Txn03LegacyStakingFee = blockchain_txn_routing_v1:calculate_staking_fee(
        Txn00, ignore_ledger, ?ROUTER_UPDATE_ROUTER_STAKING_FEE, [], false
    ),
...
txn_fees_routing_new_xor_test() ->
    ...
    Txn00StakingFee = blockchain_txn_routing_v1:calculate_staking_fee(
        Txn00, ignore_ledger, ?ROUTER_NEW_XOR_STAKING_FEE, [], true
    ),
    Txn00LegacyStakingFee = blockchain_txn_routing_v1:calculate_staking_fee(
        Txn00, ignore_ledger, ?ROUTER_NEW_XOR_STAKING_FEE, [], false
    ),
    ?assertEqual(40000, Txn00Fee),
    ?assertEqual(0, Txn00StakingFee),
    ?assertEqual(0, Txn00LegacyStakingFee),
...
txn_fees_routing_update_xor_test() ->
    ...
    Txn00StakingFee = blockchain_txn_routing_v1:calculate_staking_fee(
        Txn00, ignore_ledger, ?ROUTER_UPDATE_XOR_STAKING_FEE, [], true
    ),
    Txn00LegacyStakingFee = blockchain_txn_routing_v1:calculate_staking_fee(
        Txn00, ignore_ledger, ?ROUTER_UPDATE_XOR_STAKING_FEE, [], false
    ),
    ?assertEqual(40000, Txn00Fee),
    ?assertEqual(0, Txn00StakingFee),
    ?assertEqual(0, Txn00LegacyStakingFee),
...
txn_fees_routing_request_subnet_test() ->
    ...
    Txn00StakingFee = blockchain_txn_routing_v1:calculate_staking_fee(
        Txn00, ignore_ledger, ?ROUTER_REQUEST_SUBNET_STAKING_FEE, [], true
    ),
    Txn00LegacyStakingFee = blockchain_txn_routing_v1:calculate_staking_fee(
        Txn00, ignore_ledger, ?ROUTER_REQUEST_SUBNET_STAKING_FEE, [], false
    ),
...
txn_fees_security_exchange_v1_test() ->
    ...
    Txn00Fee = blockchain_txn_security_exchange_v1:calculate_fee(
        Txn00, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
    ?assertEqual(35000, Txn00Fee),
    %% set the fee value of the txn, sign it and confirm the fee remains the same and unaffected by signature of fee values
    ...
    Txn02Fee = blockchain_txn_security_exchange_v1:calculate_fee(
        Txn02, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
    ?assertEqual(35000, Txn02Fee),
    ok.
...
txn_fees_state_channel_open_v1_test() ->
    ...
    SCAmount = 10,
    Txn00 = blockchain_txn_state_channel_open_v1:new(
        <<"state_channel_test_id">>, Owner, 10, 1, 1, SCAmount
    ),
    Txn00Fee = blockchain_txn_state_channel_open_v1:calculate_fee(
        Txn00, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
    ...
    Txn02Fee = blockchain_txn_state_channel_open_v1:calculate_fee(
        Txn02, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
...
txn_fees_state_channel_close_v1_test() ->
    ...
    Txn00Fee = blockchain_txn_state_channel_close_v1:calculate_fee(
        Txn00, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
    ...
    Txn02Fee = blockchain_txn_state_channel_close_v1:calculate_fee(
        Txn02, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
...
txn_fees_token_burn_v1_test() ->
    ...
    Txn00Fee = blockchain_txn_token_burn_v1:calculate_fee(
        Txn00, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
    ?assertEqual(35000, Txn00Fee),
    %% set the fee value of the txn, sign it and confirm the fee remains the same and unaffected by signature of fee values
    ...
    Txn02Fee = blockchain_txn_token_burn_v1:calculate_fee(
        Txn02, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
    ?assertEqual(35000, Txn02Fee),
    ok.
...
txn_fees_update_gateway_oui_v1_test() ->
    ...
    Txn00Fee = blockchain_txn_update_gateway_oui_v1:calculate_fee(
        Txn00, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
    ?assertEqual(40000, Txn00Fee),
    %% set the fee value of the txn, sign it and confirm the fee remains the same and unaffected by signature of fee values
    ...
    Txn02Fee = blockchain_txn_update_gateway_oui_v1:calculate_fee(
        Txn02, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
...
txn_fees_assert_location_v2_test() ->
    ...
    %% create new txn, and confirm expected txn and staking fee
    Txn00 = blockchain_txn_assert_location_v2:new(
        GWPubkeyBin, OwnerPubkeyBin, Payer, ?TEST_LOCATION, 1
    ),
    Txn00Fee = blockchain_txn_assert_location_v2:calculate_fee(
        Txn00, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
    Txn00StakingFee = blockchain_txn_assert_location_v2:calculate_staking_fee(
        Txn00, ignore_ledger, ?ASSERT_LOC_STAKING_FEE, [], true
    ),
    Txn00LegacyStakingFee = blockchain_txn_assert_location_v2:calculate_staking_fee(
        Txn00, ignore_ledger, ?ASSERT_LOC_STAKING_FEE, [], false
    ),
    ?assertEqual(55000, Txn00Fee),
    ...
    Txn04Fee = blockchain_txn_assert_location_v2:calculate_fee(
        Txn04, ignore_ledger, ?DC_PAYLOAD_SIZE, ?TXN_MULTIPLIER, true
    ),
    Txn04StakingFee = blockchain_txn_assert_location_v2:calculate_staking_fee(
        Txn04, ignore_ledger, ?ASSERT_LOC_STAKING_FEE, [], true
    ),
    Txn04LegacyStakingFee = blockchain_txn_assert_location_v2:calculate_staking_fee(
        Txn04, ignore_ledger, ?ASSERT_LOC_STAKING_FEE, [], false
    ),
