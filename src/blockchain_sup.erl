        ...
        [
            ...
            {libp2p_peerbook, [
                ...
                {notify_time,
                    application:get_env(
                        blockchain,
                        peerbook_update_interval,
                        timer:minutes(5)
                    )},
                {allow_rfc1918,
                    application:get_env(blockchain, peerbook_allow_rfc1918, false)}
            ]},
            {libp2p_group_gossip, [
                ...
                {inbound_connections,
                    application:get_env(blockchain, max_inbound_connections, 6)},
                {peer_cache_timeout,
                    application:get_env(blockchain, peer_cache_timeout, 10 * 1000)}
            ]}
        ] ++ GroupMgrArgs,
    BWorkerOpts = [
