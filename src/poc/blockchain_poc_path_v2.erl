...
build_(
    ...
    case
        next_hop(TargetPubkeyBin, GatewayScoreMap, HeadBlockTime, Vars, NewRandVal, Indices, Ledger)
    of
        {error, no_witness} ->
            lists:reverse(Path);
        {ok, WitnessPubkeyBin} ->
...
build_(
    _TargetPubkeyBin, _GatewayScoreMap, _HeadBlockTime, _Vars, _RandState, _Indices, Path, _Ledger
) ->
    lists:reverse(Path).
...
-spec next_hop(
    ...
    Indices :: [h3:h3_index()],
    Ledger :: blockchain_ledger_v1:ledger()
) -> {error, no_witness} | {ok, libp2p_crypto:pubkey_bin()}.
next_hop(GatewayBin, GatewayScoreMap, HeadBlockTime, Vars, RandVal, Indices, Ledger) ->
    ...
    case blockchain_ledger_gateway_v2:witnesses(GatewayBin, Gateway, Ledger) of
        ...
        Witnesses ->
            ...
            %% Filter witnesses
            FilteredWitnesses = filter_witnesses(
                GatewayLoc, Indices, Witnesses, GatewayScoreMap, Vars, Ledger
            ),
...
-spec select_witness(
    WitnessProbs :: [{libp2p_crypto:pubkey_bin(), float()}],
    Rnd :: float(),
    Vars :: map()
) -> {error, no_witness} | {ok, libp2p_crypto:pubkey_bin()}.
                ...
                true ->
                    ...
                    case
                        blockchain_ledger_gateway_v2:is_valid_capability(
                            Mode, ?GW_CAPABILITY_POC_WITNESS, Ledger
                        )
                    of
                        false ->
                            false;
