        ...
        true ->
            ...
            Lengths =
                [
                    {S, G}
                 || {S, G} <- [
                        {Score * ?MODULE:length(Graph, Target, Addr, PathLimit), G}
                     || {Score, Addr} = G <- blockchain_utils:shuffle_from_hash(Hash, GraphList)
                    ],
                    S > 0
                ],
            %% sort the highest scoring paths first
            [{_, {_, Start}}, {_, {_, End}} | _] = lists:sort(
                fun({S1, _}, {S2, _}) -> S1 > S2 end,
                Lengths
            ),
...
-spec shortest(Graph :: graph(), Start :: any(), End :: any(), Limit :: pos_integer() | 'infinity') ->
    {number(), list()}.
...
-spec length(Graph :: graph(), Start :: any(), End :: any(), Limit :: pos_integer() | 'infinity') ->
    integer().
...
build_graph_int([Address0 | Addresses], Gateways, Height, Ledger, Graph0) ->
    ...
    case Gateways of
        #{Address0 := {Gw0, Score0}} ->
                ...
                fun({_W, Address1}, Acc) ->
                    ...
                    case maps:is_key(Address1, Acc) of
                        ...
                        false ->
                            ...
                            Neighbors1 = filter_neighbors(
                                Address1, Score1, Neighbors1_0, Gateways, Height, Ledger
                            ),
            ...
            FilteredAddresses = lists:filter(
                fun(A) -> not maps:is_key(A, Graph1) end,
                Addresses
            ),
...
path(Graph, [{Cost, [Node | _] = Path} | Routes] = _OldRoutes, End, Seen, Limit) ->
    NewRoutes = lists:filter(
        fun({_, P}) -> length(P) =< Limit end,
        [
            {Cost + NewCost, [NewNode | Path]}
         || {NewCost, NewNode} <- maps:get(Node, Graph, [{0, []}]),
            not maps:get(NewNode, Seen, false)
        ]
    ),
...
cheapest_to_front([]) -> [];
cheapest_to_front([H | T]) -> cheapest_to_front(H, T, []).
...
neighbors(Gw, Gateways, Ledger) ->
    ...
    lists:foldl(
        fun({A, G0}, Acc) ->
            ...
            case
                {
                    blockchain_ledger_gateway_v2:location(G),
...
-spec create_probs(
    ...
    Height :: non_neg_integer(),
    Ledger :: blockchain_ledger_v1:ledger()
) -> [{float(), libp2p_crypto:pubkey_bin()}].
create_probs(Gateways, _Height, _Ledger) ->
    GwScores = lists:foldl(
        fun({A, {_G, Score}}, Acc) ->
...
entropy(Entropy) ->
    <<A:85/integer-unsigned-little, B:85/integer-unsigned-little, C:86/integer-unsigned-little,
        _/binary>> = crypto:hash(sha256, Entropy),
        ...
        fun(PubkeyBin, {Gateway, Score}, Acc0) ->
            ...
            case
                ...
                CheckSync andalso
                    ...
                    not blockchain_ledger_gateway_v2:is_valid_capability(
                        Gateway, ?GW_CAPABILITY_POC_CHALLENGEE, Ledger
                    )
            of
                true ->
                    Acc0;
                                    ...
                                    lists:foldl(
                                        fun({_, Neighbor}, Acc3) ->
                                            case Neighbor == Challenger of
                                                ...
                                                false ->
                                                    maps:put(
                                                        Neighbor, maps:get(Neighbor, Gateways), Acc3
                                                    )
                                            end
                                        end,
                                        Acc2,
