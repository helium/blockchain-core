...
%%
%% * This process is started first in the blockchain supervision tree
...
%% @end
%%%-------------------------------------------------------------------
-module(blockchain_poc_mgr_db_owner).
...
-spec write( POC :: blockchain_poc_mgr:local_poc(),
             Skewed :: skewed:skewed()) -> ok.
write(POC, Skewed) ->
    gen_server:cast(?MODULE, {write, POC, Skewed}).

-spec gc( [ blockchain_poc_mgr:local_poc_onion_key_hash() ] ) -> ok.
gc(IDs) ->
...
handle_cast({write, POC, Skewed}, #state{pending=P}=State) ->
    POCID = blockchain_poc_mgr:local_poc_onion_key_hash(POC),
