%%%-------------------------------------------------------------------
%% @doc
%% == Blockchain PoC Packet V2 ==
%% @end
%%%-------------------------------------------------------------------
-module(blockchain_poc_packet_v2).
...
decrypt(<<IV0:16/integer-unsigned-little, OnionCompactKey:33/binary, Tag:4/binary, CipherText/binary>>, ECDHFun) ->
    try libp2p_crypto:bin_to_pubkey(OnionCompactKey) of
        PubKey ->
            SecretKey = ECDHFun(PubKey),  %% note secret key will be used as block key
            IV = <<0:80/integer, IV0:16/integer-unsigned-little>>,
            case ?DECRYPT(SecretKey, IV, <<IV/binary, OnionCompactKey/binary>>,
                          CipherText, Tag) of
...
build(#{secret := OnionPrivKey, public := OnionPubKey}, IV, PubKeysAndData) ->
    ...
    DataMatrix = lists:foldl(fun({Row, Col=1}, Acc) ->
                                ...
                                ({Row, Col}, Acc) ->
                                     ...
                                     setelement(((Row-1)*N)+Col, Acc, encrypt_cell(Row+1, Col-1, N, Acc, OnionCompactKey, ECDHKeys, IVs))
    ...
    PaddingMatrix = lists:foldl(fun({Row, Col}, Acc) when Col == N ->
                                        ...
                                        case Row > N of
                                            false ->
                                                ...
                                                <<_:ExtraTagBytes/binary, Cell/binary>> = encrypt_cell(Row, Col, N, TempMatrix, OnionCompactKey, ECDHKeys, IVs),
    ...
    EncryptedMatrix = case N  of
                         ...
                         _ ->
                              lists:foldl(fun(R, Acc) ->
                                                  ...
                                                  Row = encrypt_row(R, N, Acc, OnionCompactKey, ECDHKeys, IVs),
    ...
    [FirstRow|_] = PacketRows = lists:map(fun(RowNumber) ->
                                                  case RowNumber > N of
                                                      true ->
                                                          ...
                                                          list_to_binary(Bins);
                                                      false ->
                                                          encrypt_row(RowNumber, N, EncryptedMatrix, OnionCompactKey, ECDHKeys, IVs)
                                                  end
                                          end, lists:seq(1, N+1)),
...
encrypt_cell(Row, Column, N, Matrix, OnionCompactKey, ECDHKeys, IVs) ->
