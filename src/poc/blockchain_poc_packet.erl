...
-spec decrypt(
    Packet :: binary(),
    ECDHFun :: libp2p_crypto:ecdh_fun(),
    BlockHash :: binary(),
    Ledger :: blockchain_ledger_v1:ledger()
) -> error | {Payload :: binary(), NextLayer :: binary()}.
decrypt(
    <<IV0:16/integer-unsigned-little, OnionCompactKey:33/binary, Tag:4/binary, CipherText/binary>>,
    ECDHFun,
    BlockHash,
    Ledger
) ->
        ...
        PubKey ->
            ...
            case
                ?DECRYPT(
                    BlockKey,
                    IV,
                    <<IV/binary, OnionCompactKey/binary>>,
                    CipherText,
                    Tag
                )
            of
                <<DataSize:8/integer, Data:DataSize/binary, Rest/binary>> ->
                    ...
                    <<Padding:PaddingSize/binary, Xor:16/integer-unsigned-little, _/binary>> = crypto:hash(
                        sha512, Data
                    ),
                    NextIV = <<((IV0 bxor Xor) band 16#ffff):16/integer-unsigned-little>>,
                    {
                        <<Data/binary>>,
                        <<NextIV/binary, OnionCompactKey/binary, Rest/binary, Padding/binary>>
                    };
                _ ->
                    error
            end
...
-spec build(
    OnionKey :: libp2p_crypto:key_map(),
    IV :: non_neg_integer(),
    KeysAndData :: [{libp2p_crypto:pubkey(), binary()}, ...],
    BlockHash :: binary(),
    Ledger :: blockchain_ledger_v1:ledger()
) -> {OuterLayer :: binary(), Layers :: [binary()]}.
        ...
        fun
            ...
            ({Row, Col}, Acc) ->
                ...
                setelement(
                    ...
                    Acc,
                    encrypt_cell(
                        Row + 1, Col - 1, N, Acc, OnionCompactKey, ECDHKeys, IVs, BlockHash, Ledger
                    )
                )
        end,
        EntryMatrix,
        lists:reverse(
            lists:sort([{X, Y} || X <- lists:seq(1, N + 1), Y <- lists:seq(1, N), X + Y =< N + 1])
        )
    ),

    %% fill in the padding cells
    PaddingMatrix = lists:foldl(
        fun
            ({Row, Col}, Acc) when Col == N ->
                ...
                case Row > N of
                    false ->
                        ...
                        TempMatrix = setelement(
                            ((Row - 1) * N) + Col,
                            Acc,
                            <<0:(ExtraTagBytes * 8)/integer, Hash/binary>>
                        ),
                        <<_:ExtraTagBytes/binary, Cell/binary>> = encrypt_cell(
                            Row,
                            Col,
                            N,
                            TempMatrix,
                ...
                case Row > N of
                    false ->
                        ...
                        TempMatrix = setelement(
                            ((Row - 1) * N) + Col,
                            Acc,
                            <<0:(ExtraTagBytes * 8)/integer, CellData/binary>>
                        ),
                        <<_:ExtraTagBytes/binary, Cell/binary>> = encrypt_cell(
                            Row,
                            Col,
            ...
            _ ->
                lists:foldl(
                    fun(R, Acc) ->
                        ...
                        Row = encrypt_row(
                            R, N, Acc, OnionCompactKey, ECDHKeys, IVs, BlockHash, Ledger
                        ),
                        TAcc = setelement(
                            ((R - 2) * N) + 2,
                            Acc,
                            binary:part(Row, 0, byte_size(Row) - PaddingSize)
                        ),
                        lists:foldl(
                            fun(E, Acc2) ->
                    ...
                    setelement(N, PaddingMatrix, <<>>),
                    lists:reverse(lists:seq(2, N))
                )
        end,

    [FirstRow | _] =
        PacketRows = lists:map(
            fun(RowNumber) ->
                case RowNumber > N of
                    true ->
                        %% the last row is all padding
                        Bins = lists:sublist(
                            tuple_to_list(EncryptedMatrix), ((RowNumber - 1) * N) + 1, N
                        ),
                        list_to_binary(Bins);
                    false ->
                        encrypt_row(
...
compute_ivs(InitialIV, KeysAndData) ->
    lists:foldl(
        fun({_, Data}, [H | _] = Acc) ->
            ...
            <<_:PaddingSize/binary, Xor:16/integer-unsigned-little, _/binary>> = crypto:hash(
                sha512, Data
            ),
            [(H bxor Xor) band 16#ffff | Acc]
        end,
        [InitialIV],
        lists:reverse(KeysAndData)
    ).
...
encrypt_decrypt_single_layer(Ledger) ->
    ...
    {<<"abc">>, Remainder1} = decrypt(
        OuterPacket, libp2p_crypto:mk_ecdh_fun(PrivKey1), BlockHash, Ledger
    ),
    ?assert(
        lists:all(fun(E) -> E == error end, [
            decrypt(OuterPacket, libp2p_crypto:mk_ecdh_fun(PK), BlockHash, Ledger)
         || PK <- PrivKeys -- [PrivKey1]
        ])
    ),
    ...
    {IVs, Layers} = lists:unzip([
        {ThisIV, Layer}
     || <<ThisIV:16/integer-unsigned-little, ThisKey:33/binary, Layer/binary>> <-
            [OuterPacket, Remainder1],
        ThisKey == OnionCompactKey
    ]),
...
encrypt_decrypt_double_layer(Ledger) ->
    ...
    %ExpectedIV = IV+1,
    <<_IV:16/integer-unsigned-little, OnionCompactKey:33/binary, _Rest/binary>> = Remainder1,
    {<<"def">>, Remainder2} = decrypt(
        Remainder1, libp2p_crypto:mk_ecdh_fun(PrivKey2), BlockHash, Ledger
    ),
    %% check all packets are the same length
    ?assertEqual(1, length(lists:usort([byte_size(B) || B <- [OuterPacket, Remainder1]]))),
    %% check all the packets at each decryption layer are as expected, and have the right IV
    {IVs, Layers} = lists:unzip([
        {ThisIV, Layer}
     || <<ThisIV:16/integer-unsigned-little, ThisKey:33/binary, Layer/binary>> <-
            [OuterPacket, Remainder1, Remainder2],
        ThisKey == OnionCompactKey
    ]),
    ?assertEqual(Layers, Rows),
    ?assertEqual(IVs, compute_ivs(IV, KeysAndData)),
...
encrypt_decrypt(Ledger) ->
    ...
    LayerData = [
        <<"abc">>, <<"def">>, <<"ghi">>, <<"jhk">>, <<"lmn">>, <<"opq">>, <<"rst">>, <<"uvw">>
    ],
    ...
    ?assert(
        lists:all(fun(E) -> E == error end, [
            decrypt(Remainder1, libp2p_crypto:mk_ecdh_fun(PK), BlockHash, Ledger)
         || PK <- PrivKeys -- [PrivKey2]
        ])
    ),
    {<<"ghi">>, Remainder3} = decrypt(
        Remainder2, libp2p_crypto:mk_ecdh_fun(PrivKey3), BlockHash, Ledger
    ),
    ?assert(
        lists:all(fun(E) -> E == error end, [
            decrypt(Remainder2, libp2p_crypto:mk_ecdh_fun(PK), BlockHash, Ledger)
         || PK <- PrivKeys -- [PrivKey3]
        ])
    ),
    {<<"jhk">>, Remainder4} = decrypt(
        Remainder3, libp2p_crypto:mk_ecdh_fun(PrivKey4), BlockHash, Ledger
    ),
    ?assert(
        lists:all(fun(E) -> E == error end, [
            decrypt(Remainder3, libp2p_crypto:mk_ecdh_fun(PK), BlockHash, Ledger)
         || PK <- PrivKeys -- [PrivKey4]
        ])
    ),
    {<<"lmn">>, Remainder5} = decrypt(
        Remainder4, libp2p_crypto:mk_ecdh_fun(PrivKey5), BlockHash, Ledger
    ),
    ?assert(
        lists:all(fun(E) -> E == error end, [
            decrypt(Remainder4, libp2p_crypto:mk_ecdh_fun(PK), BlockHash, Ledger)
         || PK <- PrivKeys -- [PrivKey5]
        ])
    ),
    {<<"opq">>, Remainder6} = decrypt(
        Remainder5, libp2p_crypto:mk_ecdh_fun(PrivKey6), BlockHash, Ledger
    ),
    ?assert(
        lists:all(fun(E) -> E == error end, [
            decrypt(Remainder5, libp2p_crypto:mk_ecdh_fun(PK), BlockHash, Ledger)
         || PK <- PrivKeys -- [PrivKey6]
        ])
    ),
    {<<"rst">>, Remainder7} = decrypt(
        Remainder6, libp2p_crypto:mk_ecdh_fun(PrivKey7), BlockHash, Ledger
    ),
    ?assert(
        lists:all(fun(E) -> E == error end, [
            decrypt(Remainder6, libp2p_crypto:mk_ecdh_fun(PK), BlockHash, Ledger)
         || PK <- PrivKeys -- [PrivKey7]
        ])
    ),
    {<<"uvw">>, Remainder8} = decrypt(
        Remainder7, libp2p_crypto:mk_ecdh_fun(PrivKey8), BlockHash, Ledger
    ),
    ?assert(
        lists:all(fun(E) -> E == error end, [
            decrypt(Remainder7, libp2p_crypto:mk_ecdh_fun(PK), BlockHash, Ledger)
         || PK <- PrivKeys -- [PrivKey8]
        ])
    ),
    %% check all packets are the same length
    ?assertEqual(
