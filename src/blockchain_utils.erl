...
-spec calculate_dc_amount(
    Ledger :: blockchain_ledger_v1:ledger(),
    PayloadSize :: non_neg_integer(),
    DCPayloadSize :: pos_integer()
) -> pos_integer().
calculate_dc_amount(_Ledger, PayloadSize, DCPayloadSize) ->
    do_calculate_dc_amount(PayloadSize, DCPayloadSize).

-spec do_calculate_dc_amount(PayloadSize :: non_neg_integer(), DCPayloadSize :: pos_integer()) ->
    pos_integer().
...
rand_state(Hash) ->
    <<A:85/integer-unsigned-little, B:85/integer-unsigned-little, C:86/integer-unsigned-little,
        _/binary>> = crypto:hash(sha256, Hash),
...
free_space_path_loss(Loc1, Loc2) ->
    ...
    ?TRANSMIT_POWER -
        (32.44 + 20 * math:log10(?FREQUENCY) + 20 * math:log10(Distance) - ?MAX_ANTENNA_GAIN -
            ?MAX_ANTENNA_GAIN).

-spec free_space_path_loss(
    Loc1 :: h3:index(),
    Loc2 :: h3:index(),
...
free_space_path_loss(Loc1, Loc2, undefined) ->
    ...
    10 *
        math:log10(
            math:pow(
                (4 * math:pi() * (?FREQUENCY * ?MHzToHzMultiplier) * (Distance * 1000)) /
                    (299792458),
                2
            )
        );
...
prop_to_bin(Keys, Size) ->
    <<
        <<
            (byte_size(Key)):Size/unsigned-integer,
            Key/binary,
            (byte_size(Value)):Size/unsigned-integer,
            Value/binary
        >>
     || {Key, Value} <- Keys
    >>.
...
count_votes(Artifact, MultiKeys, Proofs) ->
    ...
    {_UnusedKeys, Count} = lists:foldl(
        fun(Proof, {Keys, Count}) ->
...
%% @end
%%--------------------------------------------------------------------
-spec fold_condition_checks([{Condition :: fun(() -> boolean()), Error :: {error, any()}}]) ->
    ok | {error, any()}.
fold_condition_checks(Conditions) ->
    do_condition_check(Conditions, undefined, true).

do_condition_check(_Conditions, PrevErr, false) ->
    PrevErr;
do_condition_check([], _PrevErr, true) ->
    ok;
...
-spec get_var_(VarName :: atom(), Ledger :: blockchain_ledger_v1:ledger()) ->
    {ok, any()} | {error, any()}.
get_var_(VarName, Ledger) ->
...
count_votes_test() ->
    ...
    ?assertEqual(
        5,
        count_votes(Artifact, PKeys ++ [libp2p_crypto:pubkey_to_bin(PubKey5)], Sigs ++ [ExtraSig])
    ),
    ok.

fold_condition_checks_good_test() ->
    Conditions = [
        {fun() -> true end, {error, true_isnt_true}},
