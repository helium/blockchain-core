...
-define(GW_CAPABILITY_ROUTE_PACKETS, 16#01).
%% determines if a GW can issue POC Challenges
-define(GW_CAPABILITY_POC_CHALLENGER, 16#02).
%% determines if a GW can accept POC Challenges
-define(GW_CAPABILITY_POC_CHALLENGEE, 16#04).
%% determines if a GW can witness challenges
-define(GW_CAPABILITY_POC_WITNESS, 16#08).
%% determines if a GW can issue receipts
-define(GW_CAPABILITY_POC_RECEIPT, 16#10).
%% determines if a GW can participate in consensus group
-define(GW_CAPABILITY_CONSENSUS_GROUP, 16#20).
...
-define(GW_CAPABILITIES_LIGHT_GATEWAY_V1,
    0 bor ?GW_CAPABILITY_ROUTE_PACKETS bor
        ?GW_CAPABILITY_POC_CHALLENGER bor
        ?GW_CAPABILITY_POC_CHALLENGEE bor
        ?GW_CAPABILITY_POC_WITNESS bor
        ?GW_CAPABILITY_POC_RECEIPT
).
-define(GW_CAPABILITIES_FULL_GATEWAY_V1,
    0 bor ?GW_CAPABILITY_ROUTE_PACKETS bor
        ?GW_CAPABILITY_POC_CHALLENGER bor
        ?GW_CAPABILITY_POC_CHALLENGEE bor
        ?GW_CAPABILITY_POC_WITNESS bor
        ?GW_CAPABILITY_POC_RECEIPT bor
        ?GW_CAPABILITY_CONSENSUS_GROUP
).

%% helper macros
-define(GW_CAPABILITIES_SET(Capabilities),
    lists:foldl(fun(Capability, Acc) -> Acc bor Capability end, 0, Capabilities)
).
