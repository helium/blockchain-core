...
handle_info(
    {'RECV_DATA', StreamId, Bin, _StreamWindowError, _ConnectionWindowError},
    #{
        stream_id := StreamId,
        buffer := Buffer
    } = Stream
...
handle_info(
    ...
    HeadersMap = maps:from_list([
        grpc_lib:maybe_decode_header(H)
     || H <- Headers
    ]),
...
handle_info(
    {ClosedMessage, StreamId, _ErrorCode},
    #{stream_id := StreamId} = Stream
) when
    ClosedMessage == 'RESET_BY_PEER';
    ...
    Headers1 =
        case Compression of
            none ->
                [];
            _ ->
                [{<<"grpc-encoding">>, atom_to_binary(Compression, unicode)}]
        end,
    [
        {<<":method">>, <<"POST">>},
        {<<":scheme">>, Scheme},
