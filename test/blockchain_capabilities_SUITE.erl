...
init_per_testcase(TestCase, Config0) when
    TestCase == dataonly_gateway_simple_checks;
...
->
    ...
    #{public := DataOnlyStakingPub, secret := _DataOnlyStakingPrivKey} =
        DataOnlyStakingKey = libp2p_crypto:generate_keys(ecc_compact),
    DataOnlyStakingKeyPubBin = libp2p_crypto:pubkey_to_bin(DataOnlyStakingPub),
    %% the light mode staking keys
    #{public := LightStakingPub, secret := _LightStakingPrivKey} =
        LightStakingKey = libp2p_crypto:generate_keys(ecc_compact),
    LightStakingKeyPubBin = libp2p_crypto:pubkey_to_bin(LightStakingPub),
    %% the full mode staking keys
    #{public := FullStakingPub, secret := _FullStakingPrivKey} =
        FullStakingKey = libp2p_crypto:generate_keys(ecc_compact),
    FullStakingKeyPubBin = libp2p_crypto:pubkey_to_bin(FullStakingPub),

    %% create the staking key mappings chain var value
    Mappings = [
        {DataOnlyStakingKeyPubBin, <<"dataonly">>},
        {FullStakingKeyPubBin, <<"full">>},
        {LightStakingKeyPubBin, <<"light">>}
    ],
    ...
    ExtraConfig = [
        {dataonly_staking_key, DataOnlyStakingKey},
        {dataonly_staking_key_pub_bin, DataOnlyStakingKeyPubBin},
        {light_staking_key, LightStakingKey},
        {light_staking_key_pub_bin, LightStakingKeyPubBin},
        {full_staking_key, FullStakingKey},
        {full_staking_key_pub_bin, FullStakingKeyPubBin}
    ],
...
dataonly_gateway_simple_checks(Config) ->
    ...
    %%
    #{public := StakingPub, secret := StakingPrivKey} =
        _StakingKey = libp2p_crypto:generate_keys(ecc_compact),
    ...
    PoCReqTxn0 = blockchain_txn_poc_request_v1:new(
        Gateway, SecretHash0, OnionKeyHash0, blockchain_block:hash_block(AddGatewayBlock), 1
    ),
    SignedPoCReqTxn0 = blockchain_txn_poc_request_v1:sign(PoCReqTxn0, GatewaySigFun),
    ?assertEqual(
        {error, {gateway_not_allowed, dataonly}},
        blockchain_txn_poc_request_v1:is_valid(SignedPoCReqTxn0, Chain)
    ),
    ...
    ?assertEqual(
        {error, {challenger_not_allowed, dataonly}},
        blockchain_txn_poc_receipts_v1:is_valid(SignedPoCReceiptsTxn, Chain)
    ),
...
setup(Config) ->
    ...
    %%
    #{public := _DataOnlyStakingGatewayPub, secret := DataOnlyStakingGatewayPrivKey} = ?config(
        dataonly_staking_key, Config
    ),
    ...
    %% base txn
    DataOnlyPaymentTx0 = blockchain_txn_payment_v1:new(
        Payer, DataOnlyStakerGateway, 5000 * ?BONES_PER_HNT, 2
    ),
    %% get the fees for this txn
    DataOnlyPaymentTxFee = blockchain_txn_payment_v1:calculate_fee(DataOnlyPaymentTx0, Chain),
    ct:pal("payment txn fee ~p, staking fee ~p, total: ~p", [
        DataOnlyPaymentTxFee, 'NA', DataOnlyPaymentTxFee
    ]),
    %% set the fees on the base txn and then sign the various txns
    ...
    {ok, DataOnlyPaymentBlock} = test_utils:create_block(ConsensusMembers, [
        DataOnlySignedPaymentTx1
    ]),
    %% add the block
    ...
    %%
    #{public := _LightStakingGatewayPub, secret := LightStakingGatewayPrivKey} = ?config(
        light_staking_key, Config
    ),
    ...
    %% base txn
    LightPaymentTx0 = blockchain_txn_payment_v1:new(
        Payer, LightStakerGateway, 5000 * ?BONES_PER_HNT, 3
    ),
    %% get the fees for this txn
    LightPaymentTxFee = blockchain_txn_payment_v1:calculate_fee(LightPaymentTx0, Chain),
    ct:pal("payment txn fee ~p, staking fee ~p, total: ~p", [
        LightPaymentTxFee, 'NA', LightPaymentTxFee
    ]),
    %% set the fees on the base txn and then sign the various txns
    ...
    %%
    #{public := _FullStakingGatewayPub, secret := FullStakingGatewayPrivKey} = ?config(
        full_staking_key, Config
    ),
    ...
    %% base txn
    FullPaymentTx0 = blockchain_txn_payment_v1:new(
        Payer, FullStakerGateway, 5000 * ?BONES_PER_HNT, 4
    ),
    %% get the fees for this txn
    FullPaymentTxFee = blockchain_txn_payment_v1:calculate_fee(FullPaymentTx0, Chain),
    ct:pal("payment txn fee ~p, staking fee ~p, total: ~p", [
        FullPaymentTxFee, 'NA', FullPaymentTxFee
    ]),
    %% set the fees on the base txn and then sign the various txns
    ...
    %%
    #{public := DataOnlyGatewayPubKey, secret := DataOnlyGatewayPrivKey} = libp2p_crypto:generate_keys(
        ecc_compact
    ),
    ...
    %% add gateway base txn
    AddDataOnlyGatewayTx0 = blockchain_txn_add_gateway_v1:new(
        Owner, DataOnlyGateway, DataOnlyStakerGateway
    ),
    %% get the fees for this txn
    AddDataOnlyGatewayTxFee = blockchain_txn_add_gateway_v1:calculate_fee(
        AddDataOnlyGatewayTx0, Chain
    ),
    AddDataOnlyGatewayStFee = blockchain_txn_add_gateway_v1:calculate_staking_fee(
        AddDataOnlyGatewayTx0, Chain
    ),

    ct:pal("Add gateway txn fee ~p, staking fee ~p, total: ~p", [
        AddDataOnlyGatewayTxFee,
        AddDataOnlyGatewayStFee,
        AddDataOnlyGatewayTxFee + AddDataOnlyGatewayStFee
    ]),
    %% set the fees on the base txn and then sign the various txns
    AddDataOnlyGatewayTx1 = blockchain_txn_add_gateway_v1:fee(
        AddDataOnlyGatewayTx0, AddDataOnlyGatewayTxFee
    ),
    AddDataOnlyGatewayTx2 = blockchain_txn_add_gateway_v1:staking_fee(
        AddDataOnlyGatewayTx1, AddDataOnlyGatewayStFee
    ),
    SignedOwnerAddDataOnlyGatewayTx2 = blockchain_txn_add_gateway_v1:sign(
        AddDataOnlyGatewayTx2, OwnerSigFun
    ),
    SignedGatewayAddDataOnlyGatewayTx2 = blockchain_txn_add_gateway_v1:sign_request(
        SignedOwnerAddDataOnlyGatewayTx2, DataOnlyGatewaySigFun
    ),
    SignedPayerAddDataOnlyGatewayTx2 = blockchain_txn_add_gateway_v1:sign_payer(
        SignedGatewayAddDataOnlyGatewayTx2, DataOnlyStakerSigFun
    ),
    ?assertEqual(
        ok, blockchain_txn_add_gateway_v1:is_valid(SignedPayerAddDataOnlyGatewayTx2, Chain)
    ),

    {ok, AddDataOnlyGatewayBlock} = test_utils:create_block(ConsensusMembers, [
        SignedPayerAddDataOnlyGatewayTx2
    ]),
    %% add the block
    ...
    %%
    #{public := LightGatewayPubKey, secret := LightGatewayPrivKey} = libp2p_crypto:generate_keys(
        ecc_compact
    ),
    ...
    AddLightGatewayStFee = blockchain_txn_add_gateway_v1:calculate_staking_fee(
        AddLightGatewayTx0, Chain
    ),

    ct:pal("Add gateway txn fee ~p, staking fee ~p, total: ~p", [
        AddLightGatewayTxFee, AddLightGatewayStFee, AddLightGatewayTxFee + AddLightGatewayStFee
    ]),
    %% set the fees on the base txn and then sign the various txns
    AddLightGatewayTx1 = blockchain_txn_add_gateway_v1:fee(
        AddLightGatewayTx0, AddLightGatewayTxFee
    ),
    AddLightGatewayTx2 = blockchain_txn_add_gateway_v1:staking_fee(
        AddLightGatewayTx1, AddLightGatewayStFee
    ),
    SignedOwnerAddLightGatewayTx2 = blockchain_txn_add_gateway_v1:sign(
        AddLightGatewayTx2, OwnerSigFun
    ),
    SignedGatewayAddLightGatewayTx2 = blockchain_txn_add_gateway_v1:sign_request(
        SignedOwnerAddLightGatewayTx2, LightGatewaySigFun
    ),
    SignedPayerAddLightGatewayTx2 = blockchain_txn_add_gateway_v1:sign_payer(
        SignedGatewayAddLightGatewayTx2, LightStakerSigFun
    ),
    ?assertEqual(ok, blockchain_txn_add_gateway_v1:is_valid(SignedPayerAddLightGatewayTx2, Chain)),

    {ok, AddLightGatewayBlock} = test_utils:create_block(ConsensusMembers, [
        SignedPayerAddLightGatewayTx2
    ]),
    %% add the block
    ...
    %%
    #{public := FullGatewayPubKey, secret := FullGatewayPrivKey} = libp2p_crypto:generate_keys(
        ecc_compact
    ),
    ...
    AddFullGatewayStFee = blockchain_txn_add_gateway_v1:calculate_staking_fee(
        AddFullGatewayTx0, Chain
    ),

    ct:pal("Add gateway txn fee ~p, staking fee ~p, total: ~p", [
        AddFullGatewayTxFee, AddFullGatewayStFee, AddFullGatewayTxFee + AddFullGatewayStFee
    ]),
    %% set the fees on the base txn and then sign the various txns
    AddFullGatewayTx1 = blockchain_txn_add_gateway_v1:fee(AddFullGatewayTx0, AddFullGatewayTxFee),
    AddFullGatewayTx2 = blockchain_txn_add_gateway_v1:staking_fee(
        AddFullGatewayTx1, AddFullGatewayStFee
    ),
    SignedOwnerAddFullGatewayTx2 = blockchain_txn_add_gateway_v1:sign(
        AddFullGatewayTx2, OwnerSigFun
    ),
    SignedGatewayAddFullGatewayTx2 = blockchain_txn_add_gateway_v1:sign_request(
        SignedOwnerAddFullGatewayTx2, FullGatewaySigFun
    ),
    SignedPayerAddFullGatewayTx2 = blockchain_txn_add_gateway_v1:sign_payer(
        SignedGatewayAddFullGatewayTx2, FullStakerSigFun
    ),
    ?assertEqual(ok, blockchain_txn_add_gateway_v1:is_valid(SignedPayerAddFullGatewayTx2, Chain)),

    {ok, AddFullGatewayBlock} = test_utils:create_block(ConsensusMembers, [
        SignedPayerAddFullGatewayTx2
    ]),
    %% add the block
    ...
    %%
    #{public := Full2GatewayPubKey, secret := Full2GatewayPrivKey} = libp2p_crypto:generate_keys(
        ecc_compact
    ),
    ...
    AddFull2GatewayStFee = blockchain_txn_add_gateway_v1:calculate_staking_fee(
        AddFull2GatewayTx0, Chain
    ),

    ct:pal("Add gateway txn fee ~p, staking fee ~p, total: ~p", [
        AddFull2GatewayTxFee, AddFull2GatewayStFee, AddFull2GatewayTxFee + AddFull2GatewayStFee
    ]),
    %% set the fees on the base txn and then sign the various txns
    AddFull2GatewayTx1 = blockchain_txn_add_gateway_v1:fee(
        AddFull2GatewayTx0, AddFull2GatewayTxFee
    ),
    AddFull2GatewayTx2 = blockchain_txn_add_gateway_v1:staking_fee(
        AddFull2GatewayTx1, AddFull2GatewayStFee
    ),
    SignedOwnerAddFull2GatewayTx2 = blockchain_txn_add_gateway_v1:sign(
        AddFull2GatewayTx2, OwnerSigFun
    ),
    SignedGatewayAddFull2GatewayTx2 = blockchain_txn_add_gateway_v1:sign_request(
        SignedOwnerAddFull2GatewayTx2, Full2GatewaySigFun
    ),
    SignedPayerAddFull2GatewayTx2 = blockchain_txn_add_gateway_v1:sign_payer(
        SignedGatewayAddFull2GatewayTx2, FullStakerSigFun
    ),
    ?assertEqual(ok, blockchain_txn_add_gateway_v1:is_valid(SignedPayerAddFull2GatewayTx2, Chain)),

    {ok, AddFull2GatewayBlock} = test_utils:create_block(ConsensusMembers, [
        SignedPayerAddFull2GatewayTx2
    ]),
    %% add the block
    ...
    AssertLocationDataOnlyGatewayTx = blockchain_txn_assert_location_v1:new(
        DataOnlyGateway, Owner, DataOnlyStakerGateway, ?TEST_LOCATION, 1
    ),
    AssertLocationDataOnlyGatewayTxFee = blockchain_txn_assert_location_v1:calculate_fee(
        AssertLocationDataOnlyGatewayTx, Chain
    ),
    AssertLocationDataOnlyGatewayStFee = blockchain_txn_assert_location_v1:calculate_staking_fee(
        AssertLocationDataOnlyGatewayTx, Chain
    ),

    ?assertEqual(5 * ?USD_TO_DC, AssertLocationDataOnlyGatewayStFee),

    AssertLocationDataOnlyGatewayTx1 = blockchain_txn_assert_location_v1:fee(
        AssertLocationDataOnlyGatewayTx, AssertLocationDataOnlyGatewayTxFee
    ),
    AssertLocationDataOnlyGatewayTx2 = blockchain_txn_assert_location_v1:staking_fee(
        AssertLocationDataOnlyGatewayTx1, AssertLocationDataOnlyGatewayStFee
    ),
    PartialAssertLocationDataOnlyGatewayTxn = blockchain_txn_assert_location_v1:sign_request(
        AssertLocationDataOnlyGatewayTx2, DataOnlyGatewaySigFun
    ),
    SignedAssertLocationDataOnlyGatewayTx = blockchain_txn_assert_location_v1:sign(
        PartialAssertLocationDataOnlyGatewayTxn, OwnerSigFun
    ),
    SignedPayerAssertLocationDataOnlyGatewayTx = blockchain_txn_assert_location_v1:sign_payer(
        SignedAssertLocationDataOnlyGatewayTx, DataOnlyStakerSigFun
    ),

    ?assertEqual(
        ok,
        blockchain_txn_assert_location_v1:is_valid(
            SignedPayerAssertLocationDataOnlyGatewayTx, Chain
        )
    ),

    {ok, AssertLocationDataOnlyGatewayBlock} = test_utils:create_block(ConsensusMembers, [
        SignedPayerAssertLocationDataOnlyGatewayTx
    ]),
    %% add the block
    ...
    %%
    AddLightGatewayTx0 = blockchain_txn_add_gateway_v1:new(Owner, LightGateway, LightStakerGateway),
    AssertLocationLightGatewayTx = blockchain_txn_assert_location_v1:new(
        LightGateway, Owner, LightStakerGateway, ?TEST_LOCATION, 1
    ),
    AssertLocationLightGatewayTxFee = blockchain_txn_assert_location_v1:calculate_fee(
        AssertLocationLightGatewayTx, Chain
    ),
    AssertLocationLightGatewayStFee = blockchain_txn_assert_location_v1:calculate_staking_fee(
        AssertLocationLightGatewayTx, Chain
    ),

    ?assertEqual(5 * ?USD_TO_DC, AssertLocationLightGatewayStFee),

    AssertLocationLightGatewayTx1 = blockchain_txn_assert_location_v1:fee(
        AssertLocationLightGatewayTx, AssertLocationLightGatewayTxFee
    ),
    AssertLocationLightGatewayTx2 = blockchain_txn_assert_location_v1:staking_fee(
        AssertLocationLightGatewayTx1, AssertLocationLightGatewayStFee
    ),
    PartialAssertLocationLightGatewayTxn = blockchain_txn_assert_location_v1:sign_request(
        AssertLocationLightGatewayTx2, LightGatewaySigFun
    ),
    SignedAssertLocationLightGatewayTx = blockchain_txn_assert_location_v1:sign(
        PartialAssertLocationLightGatewayTxn, OwnerSigFun
    ),
    SignedPayerAssertLocationLightGatewayTx = blockchain_txn_assert_location_v1:sign_payer(
        SignedAssertLocationLightGatewayTx, LightStakerSigFun
    ),

    ?assertEqual(
        ok,
        blockchain_txn_assert_location_v1:is_valid(SignedPayerAssertLocationLightGatewayTx, Chain)
    ),

    {ok, AssertLocationLightGatewayBlock} = test_utils:create_block(ConsensusMembers, [
        SignedPayerAssertLocationLightGatewayTx
    ]),
    %% add the block
    ...
    AssertLocationRequestTx = blockchain_txn_assert_location_v1:new(
        FullGateway, Owner, Payer, ?TEST_LOCATION, 1
    ),
    AssertLocationTxFee = blockchain_txn_assert_location_v1:calculate_fee(
        AssertLocationRequestTx, Chain
    ),
    ...
    AssertLocationRequestTx1 = blockchain_txn_assert_location_v1:fee(
        AssertLocationRequestTx, AssertLocationTxFee
    ),
    ...
    PartialAssertLocationTxn = blockchain_txn_assert_location_v1:sign_request(
        AssertLocationRequestTx2, FullGatewaySigFun
    ),
    SignedAssertLocationTx = blockchain_txn_assert_location_v1:sign(
        PartialAssertLocationTxn, OwnerSigFun
    ),
    SignedPayerAssertLocationTx = blockchain_txn_assert_location_v1:sign_payer(
        SignedAssertLocationTx, PayerSigFun
    ),

    ?assertEqual(
        ok, blockchain_txn_assert_location_v1:is_valid(SignedPayerAssertLocationTx, Chain)
    ),
    ...
    OnionKeyHash0 = crypto:hash(sha256, libp2p_crypto:pubkey_to_bin(OnionCompactKey0)),
    PoCReqTxn0 = blockchain_txn_poc_request_v1:new(
        FullGateway, SecretHash0, OnionKeyHash0, blockchain_block:hash_block(BurnBlock), 1
    ),
...
staking_keys() ->
    ...
    [
        <<1, 37, 193, 104, 249, 129, 155, 16, 116, 103, 223, 160, 89, 196, 199, 11, 94, 109, 49,
            204, 84, 242, 3, 141, 250, 172, 153, 4, 226, 99, 215, 122, 202>>,
        <<1, 90, 111, 210, 126, 196, 168, 67, 148, 63, 188, 231, 78, 255, 150, 151, 91, 237, 189,
            148, 99, 248, 41, 4, 103, 140, 225, 49, 117, 68, 212, 132, 113>>,
        <<1, 81, 215, 107, 13, 100, 54, 92, 182, 84, 235, 120, 236, 201, 115, 77, 249, 2, 33, 68,
            206, 129, 109, 248, 58, 188, 53, 45, 34, 109, 251, 217, 130>>,
        <<1, 251, 174, 74, 242, 43, 25, 156, 188, 167, 30, 41, 145, 14, 91, 0, 202, 115, 173, 26,
            162, 174, 205, 45, 244, 46, 171, 200, 191, 85, 222, 98, 120>>,
        <<1, 253, 88, 22, 88, 46, 94, 130, 1, 58, 115, 46, 153, 194, 91, 1, 57, 194, 165, 181, 225,
            251, 12, 13, 104, 171, 131, 151, 164, 83, 113, 147, 216>>,
        <<1, 6, 76, 109, 192, 213, 45, 64, 27, 225, 251, 102, 247, 132, 42, 154, 145, 70, 61, 127,
            106, 188, 70, 87, 23, 13, 91, 43, 28, 70, 197, 41, 91>>,
        <<1, 53, 200, 215, 84, 164, 84, 136, 102, 97, 157, 211, 75, 206, 229, 73, 177, 83, 153, 199,
            255, 43, 180, 114, 30, 253, 206, 245, 194, 79, 156, 218, 193>>,
        <<1, 229, 253, 194, 42, 80, 229, 8, 183, 20, 35, 52, 137, 60, 18, 191, 28, 127, 218, 234,
            118, 173, 23, 91, 129, 251, 16, 39, 223, 252, 71, 165, 120>>,
        <<1, 54, 171, 198, 219, 118, 150, 6, 150, 227, 80, 208, 92, 252, 28, 183, 217, 134, 4, 217,
            2, 166, 9, 57, 106, 38, 182, 158, 255, 19, 16, 239, 147>>,
        <<1, 51, 170, 177, 11, 57, 0, 18, 245, 73, 13, 235, 147, 51, 37, 187, 248, 125, 197, 173,
            25, 11, 36, 187, 66, 9, 240, 61, 104, 28, 102, 194, 66>>,
        <<1, 187, 46, 236, 46, 25, 214, 204, 51, 20, 191, 86, 116, 0, 174, 4, 247, 132, 145, 22, 83,
            66, 159, 78, 13, 54, 52, 251, 8, 143, 59, 191, 196>>
    ].
