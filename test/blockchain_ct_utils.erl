...
init_per_testcase(TestCase, Config) ->
    ...
    NodeNames =
        lists:map(fun (_M) -> list_to_atom(randname(5)) end, lists:seq(1, TotalNodes)),
    ...
    ConfigResult = pmap(
        fun (Node) ->
            ...
            ct_rpc:call(Node, lager, set_loglevel, [
            ...
            {ok, StartedApps} =
                ct_rpc:call(Node, application, ensure_all_started, [blockchain]),
    ...
    lists:foreach(
        fun (Node) ->
            ...
            wait_until(
                fun () ->
                    ConnectedAddrs =
                        ct_rpc:call(Node, libp2p_group_gossip, connected_addrs, [
                            GossipGroup,
                            all
                        ]),
            ...
            ConnectedAddrs =
                ct_rpc:call(Node, libp2p_group_gossip, connected_addrs, [GossipGroup, all]),
            ct:pal(
