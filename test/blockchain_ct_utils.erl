...
init_per_testcase(TestCase, Config) ->
                        ...
                        case length(GossipPeers) >= (length(Nodes) / 2) + 1 of
                            ...
                            false ->
                                ...
                                lists:foreach(
                                    fun(A) ->
                                        CRes = ct_rpc:call(
                                            Node, libp2p_swarm, connect, [Swarm, A], 500
                                        ),
    ...
    GRPCServerConfigFun = fun(PeerPort) ->
        [
            #{
                grpc_opts => #{
                    service_protos => [state_channel_pb],
    ...
    ok = lists:foreach(
        fun(Node) ->
            ...
            [H | _] =
                _SortedAddrs = ct_rpc:call(Node, libp2p_transport, sort_addrs, [TID, ListenAddrs]),
            [_, _, _IP, _, Libp2pPort] = _Full = re:split(H, "/"),
            ThisPort = list_to_integer(binary_to_list(Libp2pPort)),
            ct_rpc:call(Node, application, set_env, [
                grpcbox, servers, GRPCServerConfigFun(ThisPort + 1000)
            ]),
...
reverse_bin(<<>>, Acc) -> Acc;
reverse_bin(<<H:1/binary, Rest/binary>>, Acc) -> reverse_bin(Rest, <<H/binary, Acc/binary>>).
