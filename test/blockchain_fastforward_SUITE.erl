...
init_per_testcase(TestCase, Config) ->
    ...
    {ok, SimSwarm} =
        libp2p_swarm:start(fastforward_SUITE_sim, [{libp2p_nat, [{enabled, false}]}]),
...
basic(
    ...
    BlocksN = 100,
    %% create a chain
    {ok, _GenesisMembers, _GenesisBlock,
        ConsensusMembers,
        _} = test_utils:init_chain(
        Balance,
        {PrivKey, PubKey}
    ),
    Chain0 =
    ...
    {ok, Genesis} =
        blockchain:genesis_block(Chain0),
    ...
    ok = libp2p_swarm:add_stream_handler(
        ...
        {libp2p_framed_stream, server,
            [blockchain_fastforward_handler, ?MODULE, [?FASTFORWARD_PROTOCOL_V1, Chain]]}
    ),
    ok = libp2p_swarm:add_stream_handler(
        SimSwarm,
        ?FASTFORWARD_PROTOCOL_V2,
        {libp2p_framed_stream, server,
            [blockchain_fastforward_handler, ?MODULE, [?FASTFORWARD_PROTOCOL_V2, Chain]]}
    ),
    % This is just to connect the 2 swarms
    [ListenAddr | _] = libp2p_swarm:listen_addrs(
        blockchain_swarm:swarm()
    ),
    {ok, _} =
        libp2p_swarm:connect(
            SimSwarm,
            ListenAddr
        ),
    [ListenAddr2 | _] =
    ...
    ok = test_utils:wait_until(fun () ->
        erlang:length(libp2p_peerbook:values(libp2p_swarm:peerbook(blockchain_swarm:swarm()))) >
            1
    end),
    ...
    case blockchain_fastforward_handler:dial(
             blockchain_swarm:swarm(),
             Chain0,
             ListenAddr2,
             ProtocolVersion
