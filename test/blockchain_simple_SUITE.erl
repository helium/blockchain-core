            ...
            #{
                ...
                in_consensus =>
                    not lists:member(TestCase, [
                        bogus_coinbase_test, bogus_coinbase_with_good_payment_test
                    ]),
                have_init_dc =>
                    true,
                extra_vars =>
    ...
    [LastBlock | _Blocks] = lists:foldl(
        fun(_, Acc) ->
            ...
            _ = blockchain_gossip_handler:add_block(
                Block, Chain0, self(), blockchain_swarm:swarm()
            ),
            timer:sleep(100),
            [Block | Acc]
        end,
...
htlc_payer_redeem_test(Config) ->
    ...
    % as we are reusing the same nonce
    ?assertEqual(
        {error, {bad_nonce, {create_htlc, 1, 1}}},
        blockchain_txn_create_htlc_v1:is_valid(SignedCreateTx, Chain)
    ),
...
poc_request_test(Config) ->
    ...
    PoCReqTxn0 = blockchain_txn_poc_request_v1:new(
        Gateway, SecretHash0, OnionKeyHash0, blockchain_block:hash_block(Block2), 1
    ),
    ...
    PoCReqTxn1 = blockchain_txn_poc_request_v1:new(
        Gateway, SecretHash1, OnionKeyHash1, blockchain_block:hash_block(Block62), 1
    ),
...
bogus_coinbase_test(Config) ->
    ...
    {error, {invalid_txns, [{BogusCoinbaseTxn, _InvalidReason}]}} = test_utils:create_block(
        ConsensusMembers, [BogusCoinbaseTxn]
    ),
...
export_test(Config) ->
    ...
    AddGatewayTx1 = blockchain_txn_add_gateway_v1:fee(
        AddGatewayTx0, blockchain_txn_add_gateway_v1:calculate_fee(AddGatewayTx0, Chain)
    ),
    AddGatewayTx = blockchain_txn_add_gateway_v1:staking_fee(
        AddGatewayTx1, blockchain_txn_add_gateway_v1:calculate_staking_fee(AddGatewayTx1, Chain)
    ),
    SignedOwnerAddGatewayTx = blockchain_txn_add_gateway_v1:sign(AddGatewayTx, OwnerSigFun),
    SignedGatewayAddGatewayTx = blockchain_txn_add_gateway_v1:sign_request(
        SignedOwnerAddGatewayTx, GatewaySigFun
    ...
    PaymentTxn1 = test_utils:create_payment_transaction(
        Payer1, PayerPrivKey1, Amount, 2, blockchain_swarm:pubkey_bin()
    ),
    PaymentTxn2 = test_utils:create_payment_transaction(
        Payer2, PayerPrivKey2, Amount, 1, blockchain_swarm:pubkey_bin()
    ),
    PaymentTxn3 = test_utils:create_payment_transaction(
        Payer3, PayerPrivKey3, Amount, 1, blockchain_swarm:pubkey_bin()
    ),
    Txns0 = [
        SignedAssertLocationTx, PaymentTxn2, SignedGatewayAddGatewayTx, PaymentTxn1, PaymentTxn3
    ],
    ...
    ?assertEqual(
        [
            [
                {gateway_address, libp2p_crypto:pubkey_to_b58(GatewayPubKey)},
    ...
    FilteredExportedAccounts = lists:foldl(
        fun(Account, Acc) ->
            ...
            case
                libp2p_crypto:bin_to_b58(Payer1) == AccountAddress orelse
...
delayed_ledger_test(Config) ->
    ...
    {ok, Entry3} = blockchain_ledger_v1:find_entry(
        Payer, blockchain_ledger_v1:new_context(DelayedLedger)
    ),
    ?assertEqual(Balance - 50, blockchain_ledger_entry_v1:balance(Entry3)),

    {ok, Entry4} = blockchain_ledger_v1:find_entry(
        Payee, blockchain_ledger_v1:new_context(DelayedLedger)
    ),
    ?assertEqual(Balance + 50, blockchain_ledger_entry_v1:balance(Entry4)),
...
routing_test(Config) ->
    ...
    {ok, Block2} = test_utils:create_block(ConsensusMembers, [
        SignedOUITxn3, SignedOUITxn4, SignedOUITxn4a
    ]),
    _ = blockchain_gossip_handler:add_block(Block2, Chain, self(), blockchain_swarm:swarm()),
    ...
    {ok, Block3} = test_utils:create_block(ConsensusMembers, [
        SignedOUITxn4b, SignedOUITxn4c, SignedOUITxn4d
    ]),
    _ = blockchain_gossip_handler:add_block(Block3, Chain, self(), blockchain_swarm:swarm()),
    ...
    Routing4 = blockchain_ledger_routing_v1:new(
        OUI2, Payer, Addresses0, Filter, <<0, 0, 4, 127, 255, 254>>, 0
    ),
...
max_subnet_test(Config) ->
    ...
    RoutingTxn21 = blockchain_txn_routing_v1:sign(
        blockchain_txn_routing_v1:request_subnet(OUI1, Payer, 32, 21), SigFun
    ),

    {error, {invalid_txns, [{RoutingTxn21, _InvalidReason}]}} = test_utils:create_block(
        ConsensusMembers, RoutingTxns ++ [RoutingTxn21]
    ),
...
net_emissions_reward_test(Config) ->
    ...
    {ok, Rewards3} = blockchain_txn_rewards_v2:calculate_rewards(
        Start3, End3, blockchain:ledger(Ledger, Chain)
    ),
...
election_v4_test(Config) ->
    ...
    %% five should have taken both hits
    FiveTarget = normalize_float(
        element(1, lists:nth(5, ScoredOldGroup)) -
            normalize_float((BlockCt * BBAPenalty + BlockCt * SeenPenalty))
    ),
    ?assertEqual(FiveTarget, FiveScore),

    %% six should have taken only the BBA hit
    SixTarget = normalize_float(
        element(1, lists:nth(6, ScoredOldGroup)) -
            (BlockCt * BBAPenalty)
    ),
    ?assertEqual(SixTarget, SixScore),
...
election_v5_test(Config) ->
    ...
    %% five should have taken both hits
    FiveTarget = normalize_float(
        element(2, lists:nth(5, OldGroupVals)) +
            normalize_float((BlockCt * BBAPenalty) + (BlockCt * SeenPenalty))
    ),
    ...
    SixTarget = normalize_float(
        element(2, lists:nth(6, OldGroupVals)) +
            (BlockCt * BBAPenalty)
    ),
...
chain_vars_set_unset_test(Config) ->
    ...
    lists:foreach(
        fun(_) ->
            ...
            _ = blockchain_gossip_handler:add_block(
                Block1, Chain, self(), blockchain_swarm:swarm()
            ),
...
token_burn_test(Config) ->
    ...
    {error, {invalid_txns, [{SignedBurnTx0, _InvalidReason}]}} = test_utils:create_block(
        ConsensusMembers, [SignedBurnTx0]
    ),
...
payer_test(Config) ->
    ...
    meck:expect(blockchain_ledger_v1, current_oracle_price_list, fun(_) -> {ok, [OP]} end),
    meck:expect(blockchain_ledger_v1, hnt_to_dc, fun(HNT, _) -> {ok, HNT * OP} end),
    meck:expect(blockchain_ledger_v1, config, fun
        (sc_version, _) -> {ok, 2};
        (A, B) -> blockchain_ledger_v1_meck_original:config(A, B)
    end),
    ...
    {ok, Block23} = test_utils:create_block(ConsensusMembers, [
    ...
    {ok, DCEntry2} = blockchain_ledger_v1:find_dc_entry(Payer, Ledger),
    ?assertEqual(
        (10 * OP - ((DefaultTxnFee + DefaultStakingFee) * 3) + DCBalance0),
        blockchain_ledger_data_credits_entry_v1:balance(DCEntry2)
    ),

    Routing = blockchain_ledger_routing_v1:new(
        1, Owner, Addresses0, Filter, <<0, 0, 0, 127, 255, 254>>, 0
    ),
    ...
    ?assertEqual(
        {error,
            {missing_block, blockchain_block:hash_block(Block22), blockchain_block:height(Block22)}},
    ...
    ?assertEqual(
        blockchain:head_hash(NewerChain), {ok, blockchain_block:hash_block(lists:nth(18, Blocks))}
    ),
...
poc_sync_interval_test(Config) ->
    ...
    SignedAssertLocTx = fake_assert_location(
        Owner, OwnerSigFun, Gateway, GatewaySigFun, ?TEST_LOCATION
    ),
...
update_gateway_oui_test(Config) ->
    ...
    SignedGatewayAddGatewayTxn = blockchain_txn_add_gateway_v1:sign_request(
        SignedOwnerAddGatewayTxn, GatewaySigFun
    ),
    OUI1 = 1,
    ...
    SignedUpdateGatewayOUITxn = blockchain_txn_update_gateway_oui_v1:oui_owner_sign(
        blockchain_txn_update_gateway_oui_v1:gateway_owner_sign(UpdateGatewayOUITxn, OwnerSigFun),
        OwnerSigFun
    ),
...
failed_txn_error_handling(Config) ->
    ...
    meck:expect(blockchain_utils, pmap, fun(_, _) ->
        [{SignedPoCReqTxn0, {error, txn_fail_reason}}]
    end),
    {[], [{SignedPoCReqTxn0, txn_fail_reason}]} = blockchain_txn:validate(
        [SignedPoCReqTxn0], Chain
    ),

    %% meck out the pmap function to simulate it returning an error msg in the format {error, {Reason, Details}}
    meck:expect(blockchain_utils, pmap, fun(_, _) ->
        [{SignedPoCReqTxn0, {error, {bad_nonce, {nonce_type, cur_nonce, ledger_nonce}}}}]
    end),
    ...
    meck:expect(blockchain_utils, pmap, fun(_, _) ->
        [{SignedPoCReqTxn0, {'EXIT', {{badmatch, {error, crash_in_pmap}}, []}}}]
    end),
    ...
    meck:expect(blockchain_utils, pmap, fun(_, _) ->
        [{SignedPoCReqTxn0, {crash, something_unexpected}}]
    end),
...
fake_poc_request(Gateway, GatewaySigFun, BlockHash) ->
    ...
    OnionKeyHash0 = crypto:hash(sha256, libp2p_crypto:pubkey_to_bin(OnionCompactKey0)),
    PoCReqTxn0 = blockchain_txn_poc_request_v1:new(
        Gateway, SecretHash0, OnionKeyHash0, BlockHash, 1
    ),
