...
init_per_testcase(TestCase, Config) ->
    ...
    ExtraVars = case TestCase of
                    ...
                    offchain_receipts_test ->
                        #{
                            ?poc_challenger_type => oracle,
                            ?poc_oracle_key => libp2p_crypto:pubkey_to_bin(PubKey),
                            ?poc_reject_empty_receipts => false,
                            ?poc_receipt_witness_validation => true,
                            ?poc_activity_filter_enabled => true,
                            ?poc_v4_parent_res => 11,
...
offchain_receipts_test(Config) ->
    ...
    PubkeyBin = libp2p_crypto:pubkey_to_bin(?config(pubkey, Config)),
    SigFun = libp2p_crypto:mk_sig_fun(?config(privkey, Config)),
