...
init_per_testcase(TestCase, Config0) when TestCase == txn_fees_pay_with_hnt;
    ...
    {ExtraVars, ExtraConfig} =
        case TestCase of
            staking_key_add_gateway ->
                ...
                {maps:put(staking_keys, EncodedStakingKeys, ExtraVars0),
    ...
    [_, {Payer, {_, PayerPrivKey, _}}, {Owner, {_, OwnerPrivKey, _}} | _] =
        ConsensusMembers,
...
submit_bad_public_key(Config) ->
    ...
    ?assertMatch(
        ...
        test_utils:create_block(ConsensusMembers, [BadTxn])
    ),

    %% check that a bad signature is invalid
    RawTxn = blockchain_txn_price_oracle_v1:new(
        libp2p_crypto:pubkey_to_bin(maps:get(public, hd(OracleKeys))),
        50,
        50
    ),
    ...
    ?assertMatch(
        {error, {invalid_txns, _}},
        test_utils:create_block(ConsensusMembers, [BadlySignedTxn])
    ),
...
double_submit_prices(Config) ->
    ...
    ?assertMatch(
        {error, {invalid_txns, _}},
        test_utils:create_block(ConsensusMembers, Txns)
    ),
    ok.
...
txn_fees_pay_with_dc(Config) ->
    ...
    ct:pal(
        "Token burn txn fee ~p, staking fee ~p, total: ~p",
        [BurnTxFee, 'NA', BurnTxFee]
    ),

    %% get the payers HNT bal pre the burn
    {ok, PayerPreBurnEntry} =
        blockchain_ledger_v1:find_entry(Payer, blockchain:ledger(Chain)),
    ...
    BurnTx2 =
        blockchain_txn_token_burn_v1:fee(
            BurnTx0,
            BurnTxFee + 10
        ),
    SignedBurnTx2 = blockchain_txn_token_burn_v1:sign(
        BurnTx2,
        PayerSigFun
    ),
    ...
    ?assertMatch(
        {error, {invalid_txns, _}},
        test_utils:create_block(ConsensusMembers, [SignedBurnTx0])
    ),
    %% all the fees are set, so this should work
    {ok, BurnBlock} = test_utils:create_block(
        ConsensusMembers,
        [SignedBurnTx1]
    ),
    %% add the block
    ...
    {ok, PayerPostBurnEntry} =
        blockchain_ledger_v1:find_entry(
            Payer,
            blockchain:ledger(Chain)
        ),
    PayerPostBurnHNTBal =
        blockchain_ledger_entry_v1:balance(
            PayerPostBurnEntry
        ),
    ct:pal(
        "Payer pre burn hnt bal: ~p, post burn hnt bal: ~p",
    ...
    ?assertEqual(
        PayerPreBurnHNTBal -
            (Balance div 2 + BurnTxHNTFee),
        PayerPostBurnHNTBal
    ),

    %% get the payers DC balance post burn, all subsequent DC balances will be derived from this base
    {ok, PayerDCBalEntry0} =
        blockchain_ledger_v1:find_dc_entry(
            Payer,
            Ledger
        ),
    PayerDCBal0 =
        blockchain_ledger_data_credits_entry_v1:balance(
            PayerDCBalEntry0
        ),
    ct:pal(
        "opening dc balance ~p",
        [PayerDCBal0]
    ),
    ...
    {Filter, _} =
        xor16:to_bin(xor16:new([], fun xxhash:hash64/1)),
    %% base txn
    OUITx0 = blockchain_txn_oui_v1:new(
        OUI1,
        Payer,
        Addresses0,
        Filter,
    ...
    %% get the fees for this txn
    OUITxFee = blockchain_txn_oui_v1:calculate_fee(
        OUITx0,
        Chain
    ),
    OUIStFee =
        blockchain_txn_oui_v1:calculate_staking_fee(
            OUITx0,
            Chain
        ),
    ct:pal(
        "OUT txn fee ~p, staking fee ~p, total: ~p",
        [OUITxFee, OUIStFee, OUITxFee + OUIStFee]
    ),

    %% set the fees on the base txn and then sign the various txns
    OUITx1 = blockchain_txn_oui_v1:fee(OUITx0, OUITxFee),
    OUITx2 =
        blockchain_txn_oui_v1:staking_fee(
            OUITx1,
            OUIStFee
        ),
    SignedOUITx0 =
        blockchain_txn_oui_v1:sign(
            OUITx0,
            PayerSigFun
        ),
    SignedOUITx1 =
    ...
    SignedOUITx2 = blockchain_txn_oui_v1:sign(
        OUITx2,
        PayerSigFun
    ),
    ...
    SignedOUITx4 = blockchain_txn_oui_v1:sign(
        OUITx4,
        PayerSigFun
    ),
    ...
    ?assertMatch(
        {error, {wrong_txn_fee, _, 0}},
        blockchain_txn_oui_v1:is_valid(SignedOUITx0, Chain)
    ),
    ?assertMatch(
        {error,
            {wrong_staking_fee, _, 1}},
        blockchain_txn_oui_v1:is_valid(
            SignedOUITx1,
            Chain
        )
    ),
    ...
    ?assertMatch(
        {error, {invalid_txns, _}},
        test_utils:create_block(ConsensusMembers, [SignedOUITx0])
    ),
    ?assertMatch(
        {error,
            {invalid_txns, _}},
        test_utils:create_block(
            ConsensusMembers,
            [SignedOUITx1]
        )
    ),
    %% all the fees are set, so this should work
    {ok, OUIBlock} = test_utils:create_block(
        ConsensusMembers,
        [SignedOUITx2]
    ),
    %% add the block
    ...
    %%
    #{
        public := GatewayPubKey,
        secret := GatewayPrivKey
    } = libp2p_crypto:generate_keys(
        ecc_compact
    ),
    Gateway =
    ...
    GatewaySigFun = libp2p_crypto:mk_sig_fun(
        GatewayPrivKey
    ),

    %% base txn
    AddGatewayTx0 = blockchain_txn_add_gateway_v1:new(
        Owner,
        Gateway,
        Payer
    ),
    %% get the fees for this txn
    AddGatewayTxFee = blockchain_txn_add_gateway_v1:calculate_fee(
        AddGatewayTx0,
        Chain
    ),
    AddGatewayStFee =
        blockchain_txn_add_gateway_v1:calculate_staking_fee(
            AddGatewayTx0,
            Chain
        ),
    ct:pal(
        "Add gateway txn fee ~p, staking fee ~p, total: ~p",
        [AddGatewayTxFee, AddGatewayStFee, AddGatewayTxFee + AddGatewayStFee]
    ),

    %% set the fees on the base txn and then sign the various txns
    AddGatewayTx1 = blockchain_txn_add_gateway_v1:fee(
        AddGatewayTx0,
        AddGatewayTxFee
    ),
    AddGatewayTx2 = blockchain_txn_add_gateway_v1:staking_fee(
        AddGatewayTx1,
        AddGatewayStFee
    ),

    SignedOwnerAddGatewayTx0 =
        blockchain_txn_add_gateway_v1:sign(
            AddGatewayTx0,
            OwnerSigFun
        ),
    SignedGatewayAddGatewayTx0 =
        blockchain_txn_add_gateway_v1:sign_request(
            SignedOwnerAddGatewayTx0,
            GatewaySigFun
        ),
    ...
    SignedOwnerAddGatewayTx1 =
        blockchain_txn_add_gateway_v1:sign(
            AddGatewayTx1,
            OwnerSigFun
        ),
    SignedGatewayAddGatewayTx1 =
        blockchain_txn_add_gateway_v1:sign_request(
            SignedOwnerAddGatewayTx1,
            GatewaySigFun
        ),
    ...
    SignedOwnerAddGatewayTx2 =
        blockchain_txn_add_gateway_v1:sign(
            AddGatewayTx2,
            OwnerSigFun
        ),
    SignedGatewayAddGatewayTx2 =
        blockchain_txn_add_gateway_v1:sign_request(
            SignedOwnerAddGatewayTx2,
            GatewaySigFun
        ),
    ...
    %% create version of the txn with a fee higher than expected, it should be declared valid as we accept higher txn fees
    AddGatewayTx3 =
        blockchain_txn_add_gateway_v1:fee(
            AddGatewayTx2,
            AddGatewayTxFee + 10
        ),
    SignedOwnerAddGatewayTx3 =
        blockchain_txn_add_gateway_v1:sign(
            AddGatewayTx3,
            OwnerSigFun
        ),
    SignedGatewayAddGatewayTx3 =
        blockchain_txn_add_gateway_v1:sign_request(
            SignedOwnerAddGatewayTx3,
            GatewaySigFun
        ),
    SignedPayerAddGatewayTx3 =
        blockchain_txn_add_gateway_v1:sign_payer(
            SignedGatewayAddGatewayTx3,
            PayerSigFun
        ),
    %% and create version with higher staking fee, it should be declared invalid as these need to be exact
    AddGatewayTx4 =
        blockchain_txn_add_gateway_v1:staking_fee(
            AddGatewayTx2,
            AddGatewayStFee + 10
        ),
    SignedOwnerAddGatewayTx4 =
        blockchain_txn_add_gateway_v1:sign(
            AddGatewayTx4,
            OwnerSigFun
        ),
    SignedGatewayAddGatewayTx4 =
        blockchain_txn_add_gateway_v1:sign_request(
            SignedOwnerAddGatewayTx4,
            GatewaySigFun
        ),
    ...
    ?assertMatch(
        ...
        blockchain_txn_add_gateway_v1:is_valid(
            SignedPayerAddGatewayTx1,
            Chain
        )
    ),
    ?assertMatch(
    ...
    %% all the fees are set, so this should work
    {ok, AddGatewayBlock} = test_utils:create_block(ConsensusMembers, [
        SignedPayerAddGatewayTx2
    ]),
    %% add the block
    ...
    AssertLocationTxFee = blockchain_txn_assert_location_v1:calculate_fee(
        AssertLocationRequestTx0,
        Chain
    ),
    AssertLocationStFee =
        blockchain_txn_assert_location_v1:calculate_staking_fee(
            AssertLocationRequestTx0,
            Chain
        ),
    ct:pal(
        "Assert location txn fee ~p, staking fee ~p, total: ~p",
    ...
    AssertLocationRequestTx2 = blockchain_txn_assert_location_v1:staking_fee(
        AssertLocationRequestTx1,
        AssertLocationStFee
    ),

    PartialAssertLocationTxn0 =
        blockchain_txn_assert_location_v1:sign_request(
            AssertLocationRequestTx0,
            GatewaySigFun
        ),
    SignedAssertLocationTx0 =
        blockchain_txn_assert_location_v1:sign(
            PartialAssertLocationTxn0,
            OwnerSigFun
        ),
    ...
    PartialAssertLocationTxn1 =
        blockchain_txn_assert_location_v1:sign_request(
            AssertLocationRequestTx1,
            GatewaySigFun
        ),
    SignedAssertLocationTx1 =
        blockchain_txn_assert_location_v1:sign(
            PartialAssertLocationTxn1,
            OwnerSigFun
        ),
    ...
    PartialAssertLocationTxn2 =
        blockchain_txn_assert_location_v1:sign_request(
            AssertLocationRequestTx2,
            GatewaySigFun
        ),
    ...
    %% create version of the txn with a fee higher than expected, it should be declared valid as we accept higher txn fees
    AssertLocationRequestTx3 =
        blockchain_txn_assert_location_v1:fee(
            AssertLocationRequestTx2,
            AssertLocationTxFee + 10
        ),
    PartialAssertLocationTxn3 =
        blockchain_txn_assert_location_v1:sign_request(
            AssertLocationRequestTx3,
            GatewaySigFun
        ),
    ...
    SignedPayerAssertLocationTx3 =
        blockchain_txn_assert_location_v1:sign_payer(
            SignedAssertLocationTx3,
            PayerSigFun
        ),
    %% and create version with higher staking fee, it should be declared invalid as these need to be exact
    AssertLocationRequestTx4 =
        blockchain_txn_assert_location_v1:staking_fee(
            AssertLocationRequestTx1,
            AssertLocationStFee + 10
        ),
    PartialAssertLocationTxn4 =
        blockchain_txn_assert_location_v1:sign_request(
            AssertLocationRequestTx4,
            GatewaySigFun
        ),
    SignedAssertLocationTx4 =
        blockchain_txn_assert_location_v1:sign(
            PartialAssertLocationTxn4,
            OwnerSigFun
        ),
    ...
    Payee = libp2p_crypto:pubkey_to_bin(
        PayeePubKey
    ),

    %% base txn
    CreateHTLCTx0 =
        blockchain_txn_create_htlc_v1:new(
            Payer,
            Payee,
            HTLCAddress,
            Hashlock,
    ...
    %% get the fees for this txn
    CreateHTLCTxFee = blockchain_txn_create_htlc_v1:calculate_fee(
        CreateHTLCTx0,
        Chain
    ),
    ct:pal(
        "create htlc txn fee ~p, staking fee ~p, total: ~p",
        [CreateHTLCTxFee, 'NA', CreateHTLCTxFee]
    ),

    %% set the fees on the base txn and then sign the various txns
    CreateHTLCTx1 = blockchain_txn_create_htlc_v1:fee(
        CreateHTLCTx0,
        CreateHTLCTxFee
    ),
    SignedCreateHTLCTx0 =
        blockchain_txn_create_htlc_v1:sign(
            CreateHTLCTx0,
            PayerSigFun
        ),
    SignedCreateHTLCTx1 = blockchain_txn_create_htlc_v1:sign(
        CreateHTLCTx1,
        PayerSigFun
    ),
    ...
    %% create version of the txn with a fee higher than expected, it should be declared valid as we accept higher txn fees
    CreateHTLCTx2 =
        blockchain_txn_create_htlc_v1:fee(
            CreateHTLCTx0,
            CreateHTLCTxFee + 10
        ),
    SignedCreateHTLCTx2 = blockchain_txn_create_htlc_v1:sign(
        CreateHTLCTx2,
        PayerSigFun
    ),
    ...
    ?assertMatch(
        {error, {invalid_txns, _}},
        test_utils:create_block(ConsensusMembers, [SignedCreateHTLCTx0])
    ),
    %% all the fees are set, so this should work
    {ok, CreateHTLCBlock} = test_utils:create_block(ConsensusMembers, [
        SignedCreateHTLCTx1
    ]),
    %% add the block
    ...
    {ok, _NewHTLC0} = blockchain_ledger_v1:find_htlc(
        HTLCAddress,
        blockchain:ledger(Chain)
    ),
    ...
    %% redeem htlc txn
    %%
    PayeeSigFun = libp2p_crypto:mk_sig_fun(
        PayeePrivKey
    ),
    ...
    PayPayeeTx1 = blockchain_txn_payment_v1:fee(
        PayPayeeTx0,
        blockchain_txn_payment_v1:calculate_fee(
            PayPayeeTx0,
            Chain
        )
    ),
    ...
    %% base txn
    RedeemTx0 =
        blockchain_txn_redeem_htlc_v1:new(
            Payee,
            HTLCAddress,
            <<"sharkfed">>
        ),
    ...
    %% get the fees for this txn
    RedeemTxFee = blockchain_txn_redeem_htlc_v1:calculate_fee(
        RedeemTx0,
        Chain
    ),
    ct:pal(
        "redeem htlc txn fee ~p, staking fee ~p, total: ~p",
        [RedeemTxFee, 'NA', RedeemTxFee]
    ),

    %% set the fees on the base txn and then sign the various txns
    RedeemTx1 = blockchain_txn_redeem_htlc_v1:fee(RedeemTx0, RedeemTxFee),
    SignedRedeemTx0 =
        blockchain_txn_redeem_htlc_v1:sign(
            RedeemTx0,
            PayeeSigFun
        ),
    SignedRedeemTx1 = blockchain_txn_redeem_htlc_v1:sign(
        RedeemTx1,
        PayeeSigFun
    ),
    ...
    %% create version of the txn with a fee higher than expected, it should be declared valid as we accept higher txn fees
    RedeemTx2 =
        blockchain_txn_redeem_htlc_v1:fee(
            RedeemTx0,
            RedeemTxFee + 10
        ),
    SignedRedeemTx2 = blockchain_txn_redeem_htlc_v1:sign(
        RedeemTx2,
        PayeeSigFun
    ),
    ...
    ?assertMatch(
        {error, {invalid_txns, _}},
        test_utils:create_block(ConsensusMembers, [SignedRedeemTx0])
    ),
    %% all the fees are set, so this should work
    {ok, RedeemBlock} = test_utils:create_block(
        ConsensusMembers,
        [SignedRedeemTx1]
    ),
    %% add the block
    ...
    %% get the fees for this txn
    PaymentTxFee = blockchain_txn_payment_v1:calculate_fee(
        PaymentTx0,
        Chain
    ),
    ct:pal(
        "payment txn fee ~p, staking fee ~p, total: ~p",
        [PaymentTxFee, 'NA', PaymentTxFee]
    ),
    ...
    SignedPaymentTx1 = blockchain_txn_payment_v1:sign(
        PaymentTx1,
        PayerSigFun
    ),
    ...
    %% create version of the txn with a fee higher than expected, it should be declared valid as we accept higher txn fees
    PaymentTx2 =
        blockchain_txn_payment_v1:fee(
            PaymentTx0,
            PaymentTxFee + 10
        ),
    SignedPaymentTx2 = blockchain_txn_payment_v1:sign(
        PaymentTx2,
        PayerSigFun
    ),
    ...
    ?assertMatch(
        {error, {invalid_txns, _}},
        test_utils:create_block(ConsensusMembers, [SignedPaymentTx0])
    ),
    %% all the fees are set, so this should work
    {ok, PaymentBlock} = test_utils:create_block(
        ConsensusMembers,
        [SignedPaymentTx1]
    ),
    %% add the block
    ...
    RouterAddresses1 =
        [RouterPubKeyBin],
    RouterSigFun = libp2p_crypto:mk_sig_fun(
        RouterPrivKey
    ),

    %% base txn
    RoutingTx0 =
        blockchain_txn_routing_v1:update_router_addresses(
            OUI1,
            Payer,
            RouterAddresses1,
            1
    ...
    ct:pal(
        "update_router_addresses txn fee ~p, staking fee ~p, total: ~p",
    ...
    SignedRoutingTx0 =
        blockchain_txn_routing_v1:sign(
            RoutingTx0,
            PayerSigFun
        ),
    SignedRoutingTx1 = blockchain_txn_routing_v1:sign(
        RoutingTx1,
        PayerSigFun
    ),
    ...
    %% create version of the txn with a fee higher than expected, it should be declared valid as we accept higher txn fees
    RoutingTx2 =
        blockchain_txn_routing_v1:fee(
            RoutingTx0,
            RoutingTxFee + 10
        ),
    SignedRoutingTx2 = blockchain_txn_routing_v1:sign(
        RoutingTx2,
        PayerSigFun
    ),
    ...
    ?assertMatch(
        {error, {invalid_txns, _}},
        test_utils:create_block(ConsensusMembers, [SignedRoutingTx0])
    ),
    %% all the fees are set, so this should work
    {ok, RoutingBlock} = test_utils:create_block(
        ConsensusMembers,
        [SignedRoutingTx1]
    ),
    %% add the block
    ...
    %% base txn
    RoutingSubnetTx0 =
        blockchain_txn_routing_v1:request_subnet(
            OUI1,
            Payer,
            8,
            2
    ...
    %% get the fees for this txn
    RoutingSubnetTxFee = blockchain_txn_routing_v1:calculate_fee(
        RoutingSubnetTx0,
        Chain
    ),
    RoutingSubnetStFee =
        blockchain_txn_routing_v1:calculate_staking_fee(
            RoutingSubnetTx0,
            Chain
        ),
    ct:pal(
        "request_subnet txn fee ~p, staking fee ~p, total: ~p",
        [RoutingSubnetTxFee, RoutingSubnetStFee, RoutingSubnetTxFee + RoutingSubnetStFee]
    ),

    %% set the fees on the base txn and then sign the various txns
    RoutingSubnetTx1 = blockchain_txn_routing_v1:fee(
        RoutingSubnetTx0,
        RoutingSubnetTxFee
    ),
    RoutingSubnetTx2 = blockchain_txn_routing_v1:staking_fee(
        RoutingSubnetTx1,
        RoutingSubnetStFee
    ),

    SignedRoutingSubnetTx0 =
        blockchain_txn_routing_v1:sign(
            RoutingSubnetTx0,
            PayerSigFun
        ),
    SignedRoutingSubnetTx1 =
        blockchain_txn_routing_v1:sign(
            RoutingSubnetTx1,
            PayerSigFun
        ),
    SignedRoutingSubnetTx2 = blockchain_txn_routing_v1:sign(
        RoutingSubnetTx2,
        PayerSigFun
    ),
    ...
    RoutingSubnetTx3 =
        blockchain_txn_routing_v1:fee(
            RoutingSubnetTx2,
            RoutingSubnetStFee + 10
        ),
    SignedRoutingSubnetTx3 =
        blockchain_txn_routing_v1:sign(
            RoutingSubnetTx3,
            PayerSigFun
        ),
    %% and create version with higher staking fee, it should be declared invalid as these need to be exact
    RoutingSubnetTx4 =
        blockchain_txn_routing_v1:staking_fee(
            RoutingSubnetTx1,
            RoutingSubnetStFee + 10
        ),
    SignedRoutingSubnetTx4 = blockchain_txn_routing_v1:sign(
        RoutingSubnetTx4,
        PayerSigFun
    ),
    ...
    ?assertMatch(
        ...
        blockchain_txn_routing_v1:is_valid(
            SignedRoutingSubnetTx1,
            Chain
        )
    ),
    ...
    %% all the fees are set, so this should work
    {ok, RoutingSubnetBlock} = test_utils:create_block(ConsensusMembers, [
        SignedRoutingSubnetTx2
    ]),
    %% add the block
    ...
    RoutingSubnetTxDCBal =
        blockchain_ledger_data_credits_entry_v1:balance(
            RoutingSubnetTxDCEntry
        ),
    ...
    %% base txn
    SecExchTx0 =
        blockchain_txn_security_exchange_v1:new(
            Payer,
            Payee,
            2500,
            1
    ...
    %% get the fees for this txn
    SecExchTxFee = blockchain_txn_security_exchange_v1:calculate_fee(
        SecExchTx0,
        Chain
    ),
    ct:pal(
        "security exch txn fee ~p, staking fee ~p, total: ~p",
        [SecExchTxFee, 'NA', SecExchTxFee]
    ),

    %% set the fees on the base txn and then sign the various txns
    SecExchTx1 =
        blockchain_txn_security_exchange_v1:fee(SecExchTx0, SecExchTxFee),

    SignedSecExchTx0 =
        blockchain_txn_security_exchange_v1:sign(
            SecExchTx0,
            PayerSigFun
        ),
    SignedSecExchTx1 = blockchain_txn_security_exchange_v1:sign(
        SecExchTx1,
        PayerSigFun
    ),
    ...
    %% create version of the txn with a fee higher than expected, it should be declared valid as we accept higher txn fees
    SecExchTx2 =
        blockchain_txn_security_exchange_v1:fee(
            SecExchTx0,
            SecExchTxFee + 10
        ),
    SignedSecExchTx2 = blockchain_txn_security_exchange_v1:sign(
        SecExchTx2,
        PayerSigFun
    ),
    ...
    ?assertMatch(
        {error, {invalid_txns, _}},
        test_utils:create_block(ConsensusMembers, [SignedSecExchTx0])
    ),
    %% all the fees are set, so this should work
    {ok, SecExchBlock} = test_utils:create_block(
        ConsensusMembers,
        [SignedSecExchTx1]
    ),
    %% add the block
    ...
    %%
    V2Payment = blockchain_payment_v2:new(
        Recipient,
        2500
    ),
    %% base txn
    ...
    %% get the fees for this txn
    PaymentV2TxFee = blockchain_txn_payment_v2:calculate_fee(
        PaymentV2Tx0,
        Chain
    ),
    ct:pal(
        "payment v2 txn fee ~p, staking fee ~p, total: ~p",
        [PaymentV2TxFee, 'NA', PaymentV2TxFee]
    ),

    %% set the fees on the base txn and then sign the various txns
    PaymentV2Tx1 = blockchain_txn_payment_v2:fee(
        PaymentV2Tx0,
        PaymentV2TxFee
    ),
    SignedPaymentV2Tx0 =
    ...
    SignedPaymentV2Tx1 = blockchain_txn_payment_v2:sign(
        PaymentV2Tx1,
        PayerSigFun
    ),
    ...
    %% create version of the txn with a fee higher than expected, it should be declared valid as we accept higher txn fees
    PaymentV2Tx2 =
        blockchain_txn_payment_v2:fee(
            PaymentV2Tx0,
            PaymentV2TxFee + 10
        ),
    SignedPaymentV2Tx2 = blockchain_txn_payment_v2:sign(
        PaymentV2Tx2,
        PayerSigFun
    ),
    ...
    ?assertMatch(
        {error, {invalid_txns, _}},
        test_utils:create_block(ConsensusMembers, [SignedPaymentV2Tx0])
    ),
    %% all the fees are set, so this should work
    {ok, PaymentV2Block} = test_utils:create_block(
        ConsensusMembers,
        [SignedPaymentV2Tx1]
    ),
    %% add the block
    ...
    %%
    ID = crypto:strong_rand_bytes(
        24
    ),
    ExpireWithin = 11,
    %% base txn
    SCTx0 =
        blockchain_txn_state_channel_open_v1:new(
            ID,
            RouterPubKeyBin,
            ExpireWithin,
            OUI1,
    ...
    %% get the fees for this txn
    SCTxFee = blockchain_txn_state_channel_open_v1:calculate_fee(
        SCTx0,
        Chain
    ),
    ct:pal(
        "state channels open txn fee ~p, staking fee ~p, total: ~p",
        [SCTxFee, 'NA', SCTxFee]
    ),
    ...
    SignedSCTx0 =
        blockchain_txn_state_channel_open_v1:sign(
            SCTx0,
            RouterSigFun
        ),
    SignedSCTx1 = blockchain_txn_state_channel_open_v1:sign(
        SCTx1,
        RouterSigFun
    ),
    ...
    %% create version of the txn with a fee higher than expected, it should be declared valid as we accept higher txn fees
    SCTx2 =
        blockchain_txn_state_channel_open_v1:fee(
            SCTx0,
            SCTxFee + 10
        ),
    SignedSCTx2 = blockchain_txn_state_channel_open_v1:sign(
        SCTx2,
        RouterSigFun
    ),
    ...
    ?assertMatch(
        {error, {wrong_txn_fee, _, 0}},
        blockchain_txn_state_channel_open_v1:is_valid(
            SignedSCTx0,
            Chain
        )
    ),
    ...
    ?assertMatch(
        {error, {invalid_txns, _}},
        test_utils:create_block(ConsensusMembers, [SignedSCTx0])
    ),
    %% all the fees are set, so this should work
    {ok, SCBlock} = test_utils:create_block(
        ConsensusMembers,
        [SignedSCTx1]
    ),
    %% add the block
...
txn_fees_pay_with_hnt(
    ...
    PayerOpenHNTBal = ?config(
        payer_opening_hnt_bal,
        Config
    ),
    ...
    %% set the fees on the base txn and then sign the various txns
    OUITx1 = blockchain_txn_oui_v1:fee(OUITx0, OUITxFee),
    OUITx2 = blockchain_txn_oui_v1:staking_fee(
        OUITx1,
        OUIStFee
    ),
    ...
    %% confirm DC balances are debited with correct fee
    {ok, NewEntry1} = blockchain_ledger_v1:find_entry(
        Payer,
        blockchain:ledger(Chain)
    ),
    PayerNewHNTBal =
    ...
    %% get the fee in HNT
    {ok, HNTFee} = blockchain_ledger_v1:dc_to_hnt(
        (OUITxFee + OUIStFee),
        Ledger
    ),
    ?assertEqual(
...
staking_key_add_gateway(Config) ->
    ...
    #{public := GatewayPubKey, secret := GatewayPrivKey} =
    ...
    #{public := OwnerPubKey, secret := OwnerPrivKey} =
        libp2p_crypto:generate_keys(ecc_compact),
    Owner =
        libp2p_crypto:pubkey_to_bin(
            OwnerPubKey
        ),
    OwnerSigFun = libp2p_crypto:mk_sig_fun(
        OwnerPrivKey
    ),
    ...
    %% base txn
    AddGatewayTx00 =
        blockchain_txn_add_gateway_v1:new(
            Owner,
            Gateway,
            Owner
        ),
    ...
    %% set the fees on the base txn and then sign the various txns
    AddGatewayTx01 = blockchain_txn_add_gateway_v1:fee(
        AddGatewayTx00,
        AddGatewayTxFee
    ),
    AddGatewayTx02 = blockchain_txn_add_gateway_v1:staking_fee(
        AddGatewayTx01,
        AddGatewayStFee
    ),

    SignedOwnerAddGatewayTx02 =
        blockchain_txn_add_gateway_v1:sign(
            AddGatewayTx02,
            OwnerSigFun
        ),
    SignedGatewayAddGatewayTx02 =
        blockchain_txn_add_gateway_v1:sign_request(
            SignedOwnerAddGatewayTx02,
            GatewaySigFun
        ),
