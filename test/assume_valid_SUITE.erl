...
all() ->
    [
        ...
        wrong_height,
        blockchain_restart,
        blockchain_almost_synced,
        blockchain_crash_while_absorbing,
        blockchain_crash_while_absorbing_and_assume_valid_moves,
        overlapping_streams,
        blockchain_crash_while_absorbing_then_resync
    ].
...
wrong_height(Config) ->
    ...
    {ok, Chain} = blockchain:new(
        ...
        Genesis,
        assumed_valid,
        {blockchain_block:hash_block(LastBlock), blockchain_block:height(LastBlock) + 1}
    ),
...
blockchain_crash_while_absorbing(Config) ->
    ...
    meck:expect(blockchain_txn, unvalidated_absorb_and_commit, fun (B, C, BC, R) ->
    ...
    blockchain:close(Chain),
    %% re-open with the assumed-valid hash supplied, like if we got an OTA
    {ok, Chain1} = blockchain:new(SimDir, Genesis, assumed_valid, undefined),
    ...
    %% check the hashes
    ?assertEqual(
        blockchain:head_hash(Chain1),
        {ok, blockchain_block:prev_hash(ExplodeBlock)}
    ),
    ?assertEqual(
        blockchain:sync_hash(Chain1),
        {ok, blockchain_block:prev_hash(ExplodeBlock)}
    ),
    ...
    ok = blockchain_ct_utils:wait_until(
        fun () -> {ok, 101} =:= blockchain:sync_height(Chain2) end
    ),
    ok = blockchain_ct_utils:wait_until(fun () ->
        {ok, 101} =:= blockchain:height(Chain2)
    end),
    ?assertEqual(
        blockchain:sync_hash(Chain2),
        {ok, blockchain_block:hash_block(LastBlock)}
    ),
    ?assertEqual(
        blockchain:head_hash(Chain2),
        {ok, blockchain_block:hash_block(LastBlock)}
    ),
    ok.
...
blockchain_crash_while_absorbing_and_assume_valid_moves(Config) ->
    ...
    {ok, Chain1} = blockchain:new(
        ...
        Genesis,
        assumed_valid,
        {blockchain_block:hash_block(FinalLastBlock),
            blockchain_block:height(FinalLastBlock)}
    ),
    ...
    ?assertEqual(
        blockchain:sync_hash(Chain1),
        {ok, blockchain_block:hash_block(FinalLastBlock)}
    ),
    ?assertEqual(
        blockchain:head_hash(Chain1),
        {ok, blockchain_block:hash_block(FinalLastBlock)}
...
blockchain_crash_while_absorbing_then_resync(Config) ->
    ...
    {ok, _GenesisMembers, _, ConsensusMembers, _} =
        test_utils:init_chain(Balance, {PrivKey, PubKey}),
    Chain0 = blockchain_worker:blockchain(),
    {ok, Genesis} = blockchain:genesis_block(Chain0),
    ...
    application:set_env(
        blockchain,
        assumed_valid_block_height,
        blockchain_block:height(LastBlock)
    ),
...
overlapping_streams(Config) ->
    ...
    %, blockchain_block:hash_block(LastBlock)),
    {ok, Chain1} = blockchain:new(SimDir, Genesis, undefined, undefined),
    ...
    ok = blockchain:add_blocks(
        lists:sublist(Blocks, 16, length(Blocks)) -- [LastBlock],
        Chain
    ),
    ?assertEqual({ok, 16}, blockchain:height(Chain)),
    ?assertEqual({ok, 100}, blockchain:sync_height(Chain)),
    ...
    ?assertEqual(
        1,
        length(lists:usort([blockchain:sync_hash(Chain) || _ <- lists:seq(1, 100)]))
    ),
